...
private:
  void log(const log_level &level, const std::string &message) const {
    logger::log(level, "[Instance " + hostname + ":" + std::to_string(pid) +
                           "] " + message);
  }
  ...
  void
  handle_message_udp_dp_filter(mtl_udp_dp_filter_message_t *udp_dp_filter_msg,
                               bool add);
  ...
  int send_response(int response,
                    mtl_message_type_t type = MTL_MSG_TYPE_RESPONSE) {
    mtl_message_t msg;
...
public:
  mtl_instance(int conn_fd)
      : conn_fd(conn_fd), is_registered(false), pid(-1), uid(-1),
        hostname("unknown") {}
  ~mtl_instance() {
    log(log_level::INFO, "Remove client.");
    for (const auto &lcore_id : lcore_ids)
...
void mtl_instance::handle_message(const char *buf, int len) {
  if ((size_t)len < sizeof(mtl_message_t))
    return;
...
void mtl_instance::handle_message_register(
  ...
  for (int i = 0; i < num_if; i++) {
    ...
    if (interface == nullptr) {
      log(log_level::ERROR,
          "Could not get interface " + std::to_string(ifindex));
      send_response(-1);
      return;
    }
  }
...
std::shared_ptr<mtl_interface>
mtl_instance::get_interface(const unsigned int ifindex) {
  ...
  if (g_it != g_interfaces.end()) {
    if (auto g_interface = g_it->second.lock()) {
      log(log_level::INFO,
          "Acquiring global interface " + std::to_string(ifindex));
  ...
  log(log_level::INFO,
      "Initializing a new interface " + std::to_string(ifindex));
  try {
    ...
    log(log_level::ERROR,
        "Failed to initialize interface: " + std::string(e.what()));
    return nullptr;
  }
}
...
void mtl_instance::handle_message_if_get_queue(mtl_if_message_t *if_msg) {
  ...
  if (ret > 0)
    if_queue_ids[ifindex].insert(ret);
...
void mtl_instance::handle_message_if_put_queue(mtl_if_message_t *if_msg) {
  ...
  if (ret == 0)
    if_queue_ids[ifindex].erase(queue_id);
  send_response(ret);
}
...
void mtl_instance::handle_message_if_add_flow(mtl_if_message_t *if_msg) {
  ...
  int ret = interface->add_flow(
      ntohs(if_msg->queue_id), ntohl(if_msg->flow_type), ntohl(if_msg->src_ip),
      ntohl(if_msg->dst_ip), ntohs(if_msg->src_port), ntohs(if_msg->dst_port));
  if (ret > 0)
    if_flow_ids[ifindex].insert(ret);
...
void mtl_instance::handle_message_if_del_flow(mtl_if_message_t *if_msg) {
  ...
  if (ret == 0)
    if_flow_ids[ifindex].erase(flow_id);
  send_response(ret);
}
