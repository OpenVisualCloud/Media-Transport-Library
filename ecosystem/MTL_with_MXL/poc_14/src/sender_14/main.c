...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt_long(argc, argv, "c:d:s:h", long_opts, NULL)) != -1) {
    switch (opt) {
      ...
      case 'd':
        ...
        break;
      case 's':
        cpu_start_override = atoi(optarg);
        break;
      case 'h':
      default:
        printf(
            "Usage: %s --config <streams_14.json> --target-info-dir <dir> [--cpu-start "
            "N]\n",
            argv[0]);
        return (opt == 'h') ? 0 : 1;
    }
  ...
  for (uint32_t i = 0; i < num_streams; i++) {
    ...
    if (poc_mxl_bridge_init(&ss->bridge, &per_cfg, mtl, &ss->queue, &ss->stats,
                            &g_running) != 0) {
    ...
    if (sc->camera_src_ip[0] || sc->camera_src_ip[1] || sc->camera_src_ip[2] ||
        sc->camera_src_ip[3]) {
      memcpy(ops.mcast_sip_addr[MTL_SESSION_PORT_P], sc->camera_src_ip, MTL_IP_ADDR_LEN);
  ...
  while (g_running) {
    ...
    for (int i = 0; i < active; i++) {
      ...
      printf(" s%d:rx=%lu/br=%lu/fab=%lu", ss->stream_id, (unsigned long)rx,
  ...
  for (int i = 0; i < active; i++) {
    ...
    if (ss->rx_handle) st20_rx_free(ss->rx_handle);
    poc_mxl_bridge_destroy(&ss->bridge);
  }
