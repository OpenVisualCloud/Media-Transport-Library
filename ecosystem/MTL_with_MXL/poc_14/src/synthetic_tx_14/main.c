...
static void *tx_thread_fn(void *arg)
{
    ...
    if (ctx->pin_cpu >= 0) {
        ...
        if (rc == 0)
            ...
            fprintf(stderr, "[TX-%d] Failed to pin to CPU %d: %s\n",
                    ctx->stream_id, ctx->pin_cpu, strerror(rc));
    }
    ...
    printf("[TX-%d] Thread stopped, frames=%lu\n",
           ctx->stream_id, (unsigned long)ctx->frames_sent);
    return NULL;
}
...
int main(int argc, char **argv)
{
        ...
        if (g_yuv_total_frames > 0) {
            ...
            if (fp) {
                ...
                if (g_yuv_mmap) {
                    ...
                    printf("[SYNTH_TX_16] YUV file loaded: %s\n"
                           "              %lu frames, %zu bytes/frame (%.2f MB total)\n",
                           yuv_file, (unsigned long)g_yuv_total_frames,
                           g_yuv_frame_size, (double)g_yuv_file_size / (1024.0 * 1024.0));
                } else {
    ...
    for (int si = 0; si < synth_count; si++) {
        ...
        memcpy(ops.port.dip_addr[MTL_SESSION_PORT_P],
               sc->multicast_ip, MTL_IP_ADDR_LEN);
        ...
        if (g_yuv_total_frames > 0)
            ctx->yuv_frame_idx = (uint64_t)si * (g_yuv_total_frames / synth_count)
                                 % g_yuv_total_frames;
        ...
        /* Thumbnail */
        if (poc14_thumb_init(&ctx->thumb, sc->id, "tx",
                             (int)cfg.video_width, (int)cfg.video_height) == 0) {
        ...
        printf("[SYNTH_TX_16] Stream %d → %s:%u  frame_size=%zu\n",
               sc->id, dip_str, sc->udp_port, ctx->frame_size);
    }
    ...
    while (g_running) {
        ...
        for (int si = 0; si < synth_count; si++) {
            ...
            printf(" s%d:%lu(+%lu)", streams[si].stream_id,
                   (unsigned long)f, (unsigned long)(f - last_frames[si]));
            last_frames[si] = f;
        }
        printf("\n");
...
cleanup:
    ...
    for (int si = 0; si < synth_count; si++) {
        ...
        if (streams[si].tx_handle)
            st20p_tx_wake_block(streams[si].tx_handle);
    }
    for (int si = 0; si < synth_count; si++) {
        if (streams[si].tx_handle)
            pthread_join(tids[si], NULL);
    }

    printf("\n[SYNTH_TX_16] Final frame counts:");
    for (int si = 0; si < synth_count; si++) {
        printf(" s%d:%lu", streams[si].stream_id,
               (unsigned long)streams[si].frames_sent);
    }
    printf("\n");

    for (int si = 0; si < synth_count; si++) {
        if (streams[si].thumb_enabled)
            poc14_thumb_cleanup(&streams[si].thumb);
        if (streams[si].tx_handle)
            st20p_tx_free(streams[si].tx_handle);
    }
    free(g_yuv_mmap);
    g_yuv_mmap = NULL;
    mtl_uninit(mtl);
