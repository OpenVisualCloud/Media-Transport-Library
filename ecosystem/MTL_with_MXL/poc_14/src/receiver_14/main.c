...
int main(int argc, char **argv)
{
    ...
    for (uint32_t i = 0; i < num_streams; i++) {
        ...
        if (pthread_create(&rs->consumer_tid, NULL, consumer_thread_fn,
                           &rs->consumer_ctx) != 0) {
        ...
        if (sc->consumer_cpu >= 0) {
            ...
            printf("[RECEIVER-%d] Consumer pinned to CPU %d (+HT %d)\n",
                   sc->id, sc->consumer_cpu, ht);
        }

        /* Thumbnail (receiver side) */
        if (poc14_thumb_init(&rs->thumb, sc->id, "rx",
        ...
        printf("[RECEIVER-%d] Stream %d active on fabrics port %s\n",
               sc->id, sc->id, sc->fabrics_local_port);
    ...
    while (g_running) {
        ...
        for (int i = 0; i < active; i++) {
            ...
            printf(" s%d:ev=%lu/con=%lu",
                   rs->stream_id, (unsigned long)te, (unsigned long)cf);
    ...
    for (int i = 0; i < active; i++) {
        receiver_stream_t *rs = &streams[i];
        if (rs->consumer_running)
            pthread_join(rs->consumer_tid, NULL);
        if (rs->thumb_enabled)
            poc14_thumb_cleanup(&rs->thumb);
        poc_fabrics_target_destroy(&rs->ft);
        poc_mxl_sink_destroy(&rs->sink);
    }
