...
int poc14_config_parse(const char *json_path, poc14_config_t *out)
{
    ...
    out->sip[0] = 192; out->sip[1] = 168; out->sip[2] = 2; out->sip[3] = 10;
    ...
    if (global) {
        ...
        out->accept_incomplete = json_get_bool(global, "accept_incomplete", out->accept_incomplete);
    ...
    for (int i = 0; i < count; i++) {
        ...
        strncpy(sc->source, json_get_string(item, "source", "synthetic"), sizeof(sc->source) - 1);
        ...
        if (fj[0]) {
            ...
            snprintf(sc->flow_json_path, sizeof(sc->flow_json_path),
                     "config/flow_%d.json", sc->id);
        }

        strncpy(sc->fabrics_local_port,
                json_get_string(item, "fabrics_local_port", "5000"),
        ...
        if (sc->worker_cpu < 0)
            sc->worker_cpu = next_worker_cpu++;
        if (sc->consumer_cpu < 0)
            sc->consumer_cpu = next_consumer_cpu++;
    }
...
void poc14_config_print(const poc14_config_t *cfg)
{
    ...
    printf("  Video:          %ux%u @ %u/%u\n",
           cfg->video_width, cfg->video_height, cfg->fps_num, cfg->fps_den);
    ...
    for (uint32_t i = 0; i < cfg->num_streams; i++) {
        ...
        printf("  [%2d] %-20s  %s  mcast=%s:%u  fabrics=:%s  cpu=%d  con_cpu=%d",
               s->id, s->label, s->source, mip, s->udp_port,
               s->fabrics_local_port, s->worker_cpu, s->consumer_cpu);
