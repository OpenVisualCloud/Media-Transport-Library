 ...
 */
static inline void rfc4175_get_pixel(const uint8_t* line, int x, int* y_out, int* cb_out,
                                     int* cr_out) {
  int pair = x / 2;
  int pos = x & 1;
...
static inline void ycbcr10_to_rgb(int y10, int cb10, int cr10, uint8_t* r, uint8_t* g,
                                  uint8_t* b) {
...
static void* thumb_encode_thread(void* arg) {
  ...
  while (1) {
    pthread_mutex_lock(&ctx->mutex);
    while (!ctx->pending && ctx->running) pthread_cond_wait(&ctx->cond, &ctx->mutex);
...
int poc14_thumb_init(poc14_thumb_ctx_t* ctx, int stream_id, const char* role, int src_w,
                     int src_h) {
  ...
  snprintf(ctx->path, sizeof(ctx->path), POC14_THUMB_DIR "/thumb_%s_%d.jpg", role,
           stream_id);
...
int poc14_thumb_write(poc14_thumb_ctx_t* ctx, const uint8_t* frame_data,
                      size_t frame_len) {
  if (!ctx->hot_buf || !frame_data) return -1;
  ...
  if (frame_len < expected) return -1; /* truncated frame, skip */
...
int poc14_thumb_write_planar(poc14_thumb_ctx_t* ctx, void* const planes[3]) {
  if (!ctx->hot_buf || !planes[0]) return -1;
  ...
  for (int dy = 0; dy < ctx->dst_h; dy++) {
    ...
    for (int dx = 0; dx < ctx->dst_w; dx++) {
      ...
      uint8_t* pixel = ctx->hot_buf + (dy * ctx->dst_w + dx) * 3;
      ycbcr10_to_rgb(y_val, cb_val, cr_val, &pixel[0], &pixel[1], &pixel[2]);
    }
  }
