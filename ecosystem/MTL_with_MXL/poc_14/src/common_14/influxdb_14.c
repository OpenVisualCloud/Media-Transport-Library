...
bool poc14_influxdb_init(void) {
  ...
  snprintf(s_url, sizeof(s_url), "%s/api/v2/write?org=%s&bucket=%s&precision=ns", url,
           org, bucket);
...
void poc14_influxdb_push(poc_stats_t* stats, const char* role, int stream_id) {
  ...
  off +=
      snprintf(body + off, sizeof(body) - off,
               ...
               "consumer_frames=%luu"
               " %lu\n",
               role, s_host_tag, stream_id, (unsigned long)atomic_load(&stats->rx_frames),
               ...
               (unsigned long)atomic_load(&stats->consumer_frames), (unsigned long)ts_ns);

  /* Latency stages — with stream_id tag + percentiles */
  struct {
    const char* name;
    const poc_latency_t* lat;
  } stages[] = {
  ...
  for (size_t i = 0; i < sizeof(stages) / sizeof(stages[0]); i++) {
    ...
    off += snprintf(body + off, sizeof(body) - off,
                    ...
                    (unsigned long)l->max_us, (unsigned long)poc_latency_avg(l),
                    (unsigned long)p50, (unsigned long)p95, (unsigned long)p99,
                    (unsigned long)l->count, (unsigned long)ts_ns);
  }
