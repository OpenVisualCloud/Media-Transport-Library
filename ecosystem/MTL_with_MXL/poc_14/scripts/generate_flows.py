...
def generate_flow_json(
    stream_id: int, label: str, width: int, height: int, fps_num: int, fps_den: int
) -> dict:
    """Build an MXL flow descriptor for a single stream."""
    # Deterministic UUID based on stream id (reproducible, no random jitter)
    flow_id = str(uuid.uuid5(uuid.NAMESPACE_DNS, f"mtl-mxl-poc14-stream-{stream_id}"))
...
def main():
    ...
    print(
        f"[generate_flows] {active} flow files → {out_dir}/  "
