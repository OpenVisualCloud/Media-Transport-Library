...
static int write_jpeg(const char* path, const uint8_t* rgb, int width, int height,
                      int quality) {
  char tmp_path[512];
...
static int thumb_init(thumb_ctx_t* tc, const char* dir) {
  ...
  if (!tc->hot || !tc->ready || !tc->encode_buf) return -1;
  ...
  if (pthread_create(&tc->thread, NULL, thumb_encode_thread, tc) != 0) return -1;

  return 0;
}

static void thumb_submit(thumb_ctx_t* tc, const uint16_t* y_plane,
                         const uint16_t* u_plane, const uint16_t* v_plane, int src_w,
                         int src_h) {
...
static int mxl_init(receiver_t* r) {
  ...
  bool created = false;
  st = mxlCreateFlowWriter(r->mxl_instance, flow_json, NULL, &r->writer, &r->config_info,
                           &created);
  free(flow_json);
  if (st != MXL_STATUS_OK) {
  ...
  printf("[MXL] FlowWriter ready (grainCount=%u, sliceSize[0]=%u)\n",
         r->config_info.discrete.grainCount, r->config_info.discrete.sliceSizes[0]);
  ...
  if (st != MXL_STATUS_OK) {
    ...
    for (int retry = 1; retry <= 10 && st != MXL_STATUS_OK; retry++) {
      fprintf(stderr,
              "[MXL] mxlFabricsTargetSetup failed (attempt %d/10), "
              "retrying in %ds...\n",
              retry, retry);
      sleep(retry);
      st = mxlFabricsTargetSetup(r->target, &target_cfg, &r->target_info);
...
static int decoder_init_from_codestream(receiver_t* r, const uint8_t* bitstream,
  ...
  CPU_ZERO(&pin_set);
  for (int c = 56; c < 56 + n_dec; c++) CPU_SET(c, &pin_set);
  ...
  SvtJxsErrorType_t err = svt_jpeg_xs_decoder_init(
      SVT_JPEGXS_API_VER_MAJOR, SVT_JPEGXS_API_VER_MINOR, dec, bitstream, size, &img_cfg);
  ...
  printf("[DEC] Initialized: %ux%u %dbit %dcomp, threads=%d\n", img_cfg.width,
         img_cfg.height, img_cfg.bit_depth, img_cfg.components_num,
         r->cfg.decoder_threads);
...
static int decoder_decode(receiver_t* r, const uint8_t* bitstream, size_t size) {
  ...
  if (fserr != SvtJxsErrorNone || frame_size == 0 || frame_size > size) {
    if (log_count < 3) {
      fprintf(stderr,
              "[DEC] get_single_frame_size failed: err=%d frame_size=%u buf_size=%zu\n",
...
static void dq_cleanup(decode_queue_t* dq) {
  for (int i = 0; i < DECODE_QUEUE_DEPTH; i++) free(dq->slots[i].data);
  ...
  while (dq->running) {
    ...
    if (rc == 0) {
      ...
      if (slot->hdr.encode_duration_us > 0) {
        atomic_fetch_add(&r->encode_time_us, (uint64_t)slot->hdr.encode_duration_us);
      }

      /* Generate thumbnail (downscale is CPU-bound but <2 ms at 8K→960×540) */
      thumb_submit(&r->thumb, r->decoded_y, r->decoded_u, r->decoded_v, SRC_W, SRC_H);
    } else {
      atomic_fetch_add(&r->decode_errors, 1);
    }
  }
...
static void http_serve_stats(int client_fd, receiver_t* r) {
  ...
  int len = snprintf(body, sizeof(body),
                     ...
                     avg_dec, avg_enc, avg_rdma, real_rdma, transit,
                     transit_shm_to_comptx, transit_comptx_stage, transit_st22_path,
                     sender_fps_delta, sender_fps_cum, avg_e2e, fps);
...
static void* mjpeg_stream_thread(void* arg) {
  ...
  while (r->running) {
    ...
    if (send(fd, part_hdr, plen, MSG_NOSIGNAL) < 0) {
      free(buf);
      goto done;
    }
    if (send(fd, buf, sz, MSG_NOSIGNAL) < 0) {
      free(buf);
      goto done;
    }
    if (send(fd, "\r\n", 2, MSG_NOSIGNAL) < 0) {
...
static int parse_args(int argc, char** argv, receiver_config_t* cfg) {
  static struct option long_opts[] = {{"mxl-domain", required_argument, NULL, 1001},
  ...
  while ((opt = getopt_long(argc, argv, "h", long_opts, NULL)) != -1) {
    switch (opt) {
      ...
      case 1002:
        ...
        break;
      case 1003:
        strncpy(cfg->provider, optarg, sizeof(cfg->provider) - 1);
        break;
      case 1004:
        strncpy(cfg->bind_ip, optarg, sizeof(cfg->bind_ip) - 1);
        break;
      case 1005:
        strncpy(cfg->bind_port, optarg, sizeof(cfg->bind_port) - 1);
        break;
      case 1006:
        cfg->decoder_threads = atoi(optarg);
        break;
      case 1007:
        strncpy(cfg->thumb_dir, optarg, sizeof(cfg->thumb_dir) - 1);
        break;
      case 1008:
        cfg->http_port = atoi(optarg);
        break;
      case 1009:
...
int main(int argc, char** argv) {
  ...
  /* Parse config */
  set_defaults(&r->cfg);
  if (parse_args(argc, argv, &r->cfg) != 0) return 1;
  ...
  while (r->running) {
    ...
    for (;;) {
      ...
      mxlStatus st =
          mxlFabricsTargetReadGrainNonBlocking(r->target, &entry_idx, &slice_idx);

      if (st == MXL_ERR_NOT_READY) break;
      if (st != MXL_STATUS_OK) {
      ...
      if (qrc != 0) {
        ...
        if (queue_drops <= 5 || (queue_drops % 100) == 0)
          fprintf(stderr,
                  "[CONSUMER] Decode queue full, dropped frame "
                  "(total drops: %lu)\n",
    ...
    if (now_stat_us - last_stats_us >= 2000000) {
      ...
      if (d_dec > 0) {
        ...
        r->win.avg_rdma_ms =
            (r->win.avg_e2e_ms > e + d) ? r->win.avg_e2e_ms - e - d : 0.0;
        ...
        if (sf) {
          ...
          if (fgets(sbuf, sizeof(sbuf), sf)) {
            ...
            if (rdma_val > 0.0) r->win.real_rdma_ms = rdma_val;
      ...
      printf(
          "[RECEIVER] %lds: rdma=%lu decoded=%lu errors=%lu drops=%lu "
          "fps=%.1f avg_decode=%.1fms rdma=%.1fms transit=%.1fms "
          "(shm->comptx=%.1f comptx=%.1f st22=%.1f) avg_e2e=%.1fms\n",
          (long)(time(NULL) - start_time), (unsigned long)rdma, (unsigned long)dec,
          (unsigned long)errs, (unsigned long)queue_drops, r->win.fps,
          r->win.avg_decode_ms, r->win.real_rdma_ms, r->win.transit_ms,
          r->win.shm_to_comptx_ms, r->win.comptx_stage_ms, r->win.st22_path_ms,
          r->win.avg_e2e_ms);
