...
static int parse_args(int argc, char** argv, comptx_config_t* cfg) {
  static struct option long_opts[] = {{"tx-port", required_argument, NULL, 1001},
  ...
  while ((opt = getopt_long(argc, argv, "h", long_opts, NULL)) != -1) {
    switch (opt) {
      ...
      case 1002:
        ...
        break;
      case 1003:
        parse_ip(optarg, cfg->tx_multicast);
        break;
      case 1004:
      ...
      case 1008:
        ...
        break;
      case 1009:
        strncpy(cfg->shm_name, optarg, sizeof(cfg->shm_name) - 1);
        break;
      case 1010:
...
int main(int argc, char** argv) {
  ...
  if (parse_args(argc, argv, &ctx->cfg) != 0) return 1;
  ...
  printf("  TX port: %s (SIP: %u.%u.%u.%u) → %u.%u.%u.%u:%u\n", cfg->tx_port,
         ...
         cfg->tx_multicast[0], cfg->tx_multicast[1], cfg->tx_multicast[2],
         cfg->tx_multicast[3], cfg->tx_udp_port);
  printf("  Lcores: %s\n", cfg->lcores);
  printf("  SHM ring: %s\n", cfg->shm_name);
  printf("══════════════════════════════════════════════════════\n\n");
  ...
  memcpy(tx_ops.dip_addr[MTL_SESSION_PORT_P], cfg->tx_multicast, MTL_IP_ADDR_LEN);
  snprintf(tx_ops.port[MTL_SESSION_PORT_P], MTL_PORT_MAX_LEN, "%s", cfg->tx_port);
  ...
  printf("[COMPTX] ST22 TX created: %dx%d@%u/%u → %u.%u.%u.%u:%u\n", OUT_W, OUT_H,
         cfg->fps_num, cfg->fps_den, cfg->tx_multicast[0], cfg->tx_multicast[1],
         cfg->tx_multicast[2], cfg->tx_multicast[3], cfg->tx_udp_port);
  ...
  while (ctx->running) {
    ...
    while (ctx->running) {
      uint64_t tail =
          atomic_load_explicit(&ctx->shm_ring.hdr->tail, memory_order_relaxed);
      uint64_t head =
          atomic_load_explicit(&ctx->shm_ring.hdr->head, memory_order_acquire);
    ...
    if (now - last_stats >= 2) {
      ...
      printf("[COMPTX] %lds: read=%lu sent=%lu stale_drop=%lu fps=%.1f\n",
             (long)(now - start_time), (unsigned long)read_cnt, (unsigned long)sent_cnt,
             (unsigned long)stale_drop_cnt, fps);
      last_stats = now;
    }

    /* Duration check */
    if (cfg->duration_sec > 0 && (int)(now - start_time) >= cfg->duration_sec) {
  ...
  if (ctx->st22_tx) st22_tx_free(ctx->st22_tx);

  if (ctx->mtl) mtl_uninit(ctx->mtl);
