...
static int st22_rx_frame_ready(void* priv, void* frame, struct st22_rx_frame_meta* meta) {
  ...
  if (cs_size >= POC_8K_HEADER_SIZE) {
    ...
    uint64_t now = now_ns();
    bool valid =
        (hdr.shm_commit_ns > 0 && hdr.comptx_pop_ns > 0 && hdr.comptx_tx_ready_ns > 0 &&
         hdr.shm_commit_ns <= hdr.comptx_pop_ns &&
         hdr.comptx_pop_ns <= hdr.comptx_tx_ready_ns && hdr.comptx_tx_ready_ns <= now);
    if (valid) {
      atomic_fetch_add(&s->ingest_path_us, (now - hdr.shm_commit_ns) / 1000ULL);
      atomic_fetch_add(&s->shm_to_comptx_us,
                       (hdr.comptx_pop_ns - hdr.shm_commit_ns) / 1000ULL);
      atomic_fetch_add(&s->comptx_stage_us,
                       (hdr.comptx_tx_ready_ns - hdr.comptx_pop_ns) / 1000ULL);
...
static int mxl_init(sender_t* s) {
  ...
  /* Create FlowWriter */
  bool created = false;
  st = mxlCreateFlowWriter(s->mxl_instance, flow_json, NULL, &s->writer, &s->config_info,
                           &created);
  free(flow_json);
  if (st != MXL_STATUS_OK) {
  ...
  printf("[MXL] FlowWriter ready (grainCount=%u, sliceSize[0]=%u)\n", s->grain_count,
         s->config_info.discrete.sliceSizes[0]);
...
static void* bridge_thread(void* arg) {
  ...
  while (s->running) {
    ...
    t_rdma_start = now_us();
    st = mxlFabricsInitiatorTransferGrain(s->initiator, grain_idx, 0, ginfo.totalSlices);
    if (st != MXL_STATUS_OK) {
...
static int parse_args(int argc, char** argv, sender_config_t* cfg) {
  static struct option long_opts[] = {{"port", required_argument, NULL, 1001},
  ...
  while ((opt = getopt_long(argc, argv, "h", long_opts, NULL)) != -1) {
    switch (opt) {
      ...
      case 1002:
        ...
        break;
      case 1003:
        parse_ip(optarg, cfg->rx_multicast);
        break;
      case 1004:
      ...
      case 1009:
        ...
        break;
      case 1010:
        strncpy(cfg->local_ip, optarg, sizeof(cfg->local_ip) - 1);
        break;
      case 1011:
        strncpy(cfg->local_port, optarg, sizeof(cfg->local_port) - 1);
        break;
      case 1012:
        strncpy(cfg->target_info_file, optarg, sizeof(cfg->target_info_file) - 1);
        break;
      case 1013:
...
int main(int argc, char** argv) {
  ...
  /* Parse config */
  set_defaults(&s->cfg);
  if (parse_args(argc, argv, &s->cfg) != 0) return 1;
  ...
  if (cfg->sip[0] == 0 && cfg->sip[1] == 0 && cfg->sip[2] == 0 && cfg->sip[3] == 0) {
  ...
  if (cfg->lcores[0]) p.lcores = (char*)cfg->lcores;
  ...
  printf("[SENDER] ST22 RX created → %u.%u.%u.%u:%u\n", cfg->rx_multicast[0],
         cfg->rx_multicast[1], cfg->rx_multicast[2], cfg->rx_multicast[3],
         cfg->rx_udp_port);
  ...
  while (s->running) {
    ...
    if (d_rdma > 0) win_rdma_avg_ms = (double)d_us / d_rdma / 1000.0;
    prev_rdma = rdma;
    ...
    if (d_hdr > 0) {
      ...
      win_shm_to_comptx_ms =
          (double)(shm_to_comptx_us - prev_shm_to_comptx_us) / d_hdr / 1000.0;
      win_comptx_stage_ms =
          (double)(comptx_stage_us - prev_comptx_stage_us) / d_hdr / 1000.0;
    ...
    printf(
        "[SENDER] %lds: rx=%lu rdma=%lu drops=%lu fps_delta=%.1f fps_cum=%.1f "
        "rdma_avg=%.1fms "
        "ingest=%.1fms (shm->comptx=%.1f comptx=%.1f st22=%.1f)\n",
        (long)elapsed, (unsigned long)rx, (unsigned long)rdma, (unsigned long)drops,
        fps_delta, fps_cum, cum_rdma_avg, win_ingest_avg_ms, win_shm_to_comptx_ms,
    ...
    if (sf) {
      fprintf(sf,
              "{\"rdma_avg_ms\":%.2f,\"ingest_avg_ms\":%.2f,"
              "\"shm_to_comptx_ms\":%.2f,\"comptx_stage_ms\":%.2f,"
              "\"st22_path_ms\":%.2f,\"rx_frames\":%lu,"
              "\"rdma_frames\":%lu,\"hdr_frames\":%lu,"
              "\"drops\":%lu,\"fps\":%.1f,\"fps_cum\":%.1f}\n",
              win_rdma_avg_ms, win_ingest_avg_ms, win_shm_to_comptx_ms,
              win_comptx_stage_ms, win_st22_path_ms, (unsigned long)rx,
              (unsigned long)rdma, (unsigned long)hdr_frames, (unsigned long)drops,
              fps_delta, fps_cum);
      fclose(sf);
...
cleanup:
  if (s->st22_rx) st22_rx_free(s->st22_rx);
  if (s->mtl) mtl_uninit(s->mtl);
  mxl_cleanup(s);
  for (int i = 0; i < 4; i++) free(s->frame_ring[i].buf);
