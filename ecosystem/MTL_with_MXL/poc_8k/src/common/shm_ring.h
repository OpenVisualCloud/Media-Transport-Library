...
#define SHM_RING_TOTAL_SIZE \
  (sizeof(shm_ring_header_t) + SHM_RING_NUM_SLOTS * SHM_SLOT_SIZE)
...
static inline int shm_ring_producer_create(shm_ring_producer_t* p, const char* shm_name) {
  ...
  p->base = mmap(NULL, SHM_RING_TOTAL_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, p->fd, 0);
  ...
  printf("[SHM-RING] Producer created: %s (%zu bytes, %u slots × %u MB)\n", shm_name,
         (size_t)SHM_RING_TOTAL_SIZE, SHM_RING_NUM_SLOTS,
         SHM_RING_MAX_PAYLOAD / (1024 * 1024));
  return 0;
}
...
static inline int shm_ring_producer_try_acquire(shm_ring_producer_t* p) {
  ...
  if (head - tail >= p->hdr->num_slots) return -1; /* full */
...
static inline int shm_ring_consumer_open(shm_ring_consumer_t* c, const char* shm_name) {
  ...
  c->base = mmap(NULL, SHM_RING_TOTAL_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, c->fd, 0);
  ...
  printf("[SHM-RING] Consumer opened: %s (%u slots × %u MB)\n", shm_name,
         c->hdr->num_slots, c->hdr->max_payload / (1024 * 1024));
  return 0;
}
...
static inline bool shm_ring_consumer_done(shm_ring_consumer_t* c) {
  if (!atomic_load(&c->hdr->producer_done)) return false;
...
static inline void shm_ring_consumer_destroy(shm_ring_consumer_t* c) {
  if (c->base && c->base != MAP_FAILED) munmap(c->base, SHM_RING_TOTAL_SIZE);
