...
static void yuv422p10le_to_rgb_thumb(const uint16_t* y_plane, const uint16_t* u_plane,
                                     const uint16_t* v_plane, int src_w, int src_h,
                                     uint8_t* rgb_out, int dst_w, int dst_h) {
...
static int write_jpeg_thumb(const char* path, const uint8_t* rgb, int width, int height,
                            int quality) {
  char tmp_path[512];
  ...
  if (!f) {
    ...
    if (++err_cnt <= 3) fprintf(stderr, "[COMP-THUMB] Cannot open %s: %m\n", tmp_path);
    return -1;
  }
...
static int thumb_init(thumb_ctx_t* tc, const char* dir) {
  ...
  printf(
      "[COMP-THUMB] Initialised: %dx%d → %dx%d (triple-buffered, async) → %s/thumb.jpg\n",
      OUT_W, OUT_H, THUMB_W, THUMB_H, dir);
  return 0;
}

static void thumb_submit_frame(thumb_ctx_t* tc, const uint16_t* y_plane,
                               const uint16_t* u_plane, const uint16_t* v_plane,
                               int src_w, int src_h) {
  /* Downscale into hot buffer (off the encode critical path) */
  yuv422p10le_to_rgb_thumb(y_plane, u_plane, v_plane, src_w, src_h, tc->hot, THUMB_W,
                           THUMB_H);
...
static void tile_compose(compositor_t* c) {
  for (int t = 0; t < c->num_rx; t++) {
    ...
    if (!atomic_load_explicit(&rs->shadow_valid, memory_order_acquire)) continue;
    ...
    for (int y = 0; y < TILE_H; y++) {
      ...
      memcpy(c->tile_y + dst_row * OUT_Y_STRIDE + dst_y_off, src_y + y * TILE_Y_STRIDE,
             TILE_W * BYTES_PER_SAMPLE);
      memcpy(c->tile_u + dst_row * OUT_U_STRIDE + dst_u_off, src_u + y * TILE_U_STRIDE,
             (TILE_W / 2) * BYTES_PER_SAMPLE);
      memcpy(c->tile_v + dst_row * OUT_V_STRIDE + dst_v_off, src_v + y * TILE_V_STRIDE,
             (TILE_W / 2) * BYTES_PER_SAMPLE);
    }
  }
}
...
static int tile_workers_init(compositor_t* c) {
  ...
  for (char* tok = strtok(lbuf, ","); tok; tok = strtok(NULL, ",")) {
    int core = atoi(tok);
    if (core >= 0 && core < CPU_SETSIZE) CPU_SET(core, &exclude_set);
  }
  for (int i = 0; i < CPU_SETSIZE; i++) CPU_SET(i, &full_set);
...
static void tile_workers_destroy(compositor_t* c) {
  ...
  for (int i = 0; i < TILE_WORKERS; i++) atomic_store(&c->tile_workers[i].running, false);

  pthread_barrier_wait(&c->tile_start_bar); /* wake workers for exit */

  for (int i = 0; i < TILE_WORKERS; i++) pthread_join(c->tile_workers[i].thread, NULL);
...
static int encoder_init(compositor_t* c) {
  ...
  printf("[COMP] SVT-JPEG-XS encoder initialized: %dx%d, bpp=%d/%d, threads=%d\n", OUT_W,
         OUT_H, enc->bpp_numerator, enc->bpp_denominator, enc->threads_num);
  return 0;
}

static int encoder_encode_buf(compositor_t* c, uint8_t* enc_y, uint8_t* enc_u,
                              uint8_t* enc_v, size_t* out_size) {
  ...
  SvtJxsErrorType_t err =
      svt_jpeg_xs_encoder_send_picture(&c->enc_api, &in_frame, 1 /* blocking */);
  if (err != SvtJxsErrorNone) {
  ...
  if (out_frame.bitstream.buffer != c->bitstream_buf) {
    memmove(c->bitstream_buf, out_frame.bitstream.buffer, out_frame.bitstream.used_size);
  }
  ...
  while (atomic_load(&w->running) && c->running) {
    ...
    for (int i = w->stream_start; i < w->stream_start + w->stream_count; i++) {
      ...
      for (;;) {
        ...
        if (!f) break;
        if (newest) st20p_rx_put_frame(rs->rx, newest);
        newest = f;
      }
...
static int ingest_workers_init(compositor_t* c) {
  ...
  for (int i = 0; i < INGEST_WORKERS; i++) {
    ...
    if (pthread_create(&w->thread, NULL, ingest_worker_fn, w) != 0) {
      ...
      for (int j = 0; j < i; j++) atomic_store(&c->ingest_workers[j].running, false);
      for (int j = 0; j < i; j++) pthread_join(c->ingest_workers[j].thread, NULL);
...
static void ingest_workers_stop(compositor_t* c) {
  if (!c->ingest_pool_active) return;
  for (int i = 0; i < INGEST_WORKERS; i++)
...
static int count_valid_shadows(compositor_t* c) {
  ...
  for (int i = 0; i < c->num_rx; i++) {
    if (atomic_load_explicit(&c->rx[i].shadow_valid, memory_order_acquire)) valid++;
  }
  return valid;
}
...
static void print_usage(const char* prog) {
  printf(
      "Usage: %s [options]\n"
      "  --rx-port <BDF>       VF for RX port P, streams 0-7 (required, e.g. "
      "0000:xx:01.6)\n"
      "  --rx-sip <IP>         Source IP for RX port P (required, e.g. 192.168.1.81)\n"
      "  --tx-port <BDF>       VF for RX port R, streams 8-15 (required, e.g. "
      "0000:xx:01.7)\n"
...
static int parse_args(int argc, char** argv, comp_config_t* cfg) {
  static struct option long_opts[] = {{"rx-port", required_argument, NULL, 1001},
  ...
  while ((opt = getopt_long(argc, argv, "h", long_opts, NULL)) != -1) {
    switch (opt) {
      case 1001:
        strncpy(cfg->rx_port, optarg, sizeof(cfg->rx_port) - 1);
        break;
      case 1002:
        parse_ip(optarg, cfg->rx_sip);
        break;
      case 1003:
        strncpy(cfg->tx_port, optarg, sizeof(cfg->tx_port) - 1);
        break;
      case 1004:
        parse_ip(optarg, cfg->tx_sip);
        break;
      case 1008:
        strncpy(cfg->lcores, optarg, sizeof(cfg->lcores) - 1);
        break;
      case 1009:
        cfg->codec_threads = atoi(optarg);
        break;
      case 1010:
        cfg->compression_ratio = atoi(optarg);
        break;
      case 1011:
...
int main(int argc, char** argv) {
  ...
  /* ── 1. Parse config ── */
  set_defaults(&c->cfg);
  if (parse_args(argc, argv, &c->cfg) != 0) return 1;
  ...
  if (cfg->rx_sip[0] == 0 && cfg->rx_sip[1] == 0 && cfg->rx_sip[2] == 0 &&
      cfg->rx_sip[3] == 0) {
  ...
  if (cfg->tx_sip[0] == 0 && cfg->tx_sip[1] == 0 && cfg->tx_sip[2] == 0 &&
      cfg->tx_sip[3] == 0) {
  ...
  printf("  port P (RX): %s (SIP: %u.%u.%u.%u) — streams 0-7\n", cfg->rx_port,
         cfg->rx_sip[0], cfg->rx_sip[1], cfg->rx_sip[2], cfg->rx_sip[3]);
  printf("  port R (RX): %s (SIP: %u.%u.%u.%u) — streams 8-15\n", cfg->tx_port,
         cfg->tx_sip[0], cfg->tx_sip[1], cfg->tx_sip[2], cfg->tx_sip[3]);
  ...
  printf("  Encoder: SVT-JPEG-XS, ratio=%d:1, threads=%d\n", cfg->compression_ratio,
         cfg->codec_threads);
  printf("  Lcores: %s\n", cfg->lcores);
  if (cfg->thumb_dir[0]) printf("  Thumbnail: %s/thumb.jpg\n", cfg->thumb_dir);
  printf("══════════════════════════════════════════════════════\n\n");
  ...
  for (int bi = 0; bi < 2; bi++) {
    ...
    if (c->tile_bufs[bi] == MAP_FAILED) {
      fprintf(stderr,
              "[COMP] Hugepage mmap failed for buf[%d] (%m), "
    ...
    for (size_t s = 0; s < chroma_samples; s++) u16[s] = 512;
   ...
   */
  {
    ...
    for (char* tok = strtok(lbuf, ","); tok; tok = strtok(NULL, ",")) {
      int core = atoi(tok);
      if (core >= 0 && core < CPU_SETSIZE) CPU_SET(core, &exclude);
    }

    /* Build the restricted set: all online cores minus lcores */
    for (int i = 0; i < CPU_SETSIZE; i++) CPU_SET(i, &full);
  ...
  for (int i = 0; i < c->num_rx; i++) {
    ...
    ops.port.num_port = 1;
    memcpy(ops.port.ip_addr[MTL_SESSION_PORT_P], cfg->streams[i].multicast_ip,
           MTL_IP_ADDR_LEN);
    const char* vf_bdf = (i < half) ? cfg->rx_port : cfg->tx_port;
    snprintf(ops.port.port[MTL_SESSION_PORT_P], MTL_PORT_MAX_LEN, "%s", vf_bdf);
    ...
    if (!rs->rx) {
      ...
      inet_ntop(AF_INET, cfg->streams[i].multicast_ip, ip_str, sizeof(ip_str));
      fprintf(stderr, "[COMP] st20p_rx_create failed for stream %d (%s:%u)\n", i, ip_str,
              cfg->streams[i].udp_port);
  ...
  while (c->running) {
    ...
    if (next_frame_time < t_now) next_frame_time = t_now + (uint64_t)target_interval_us;
    ...
    if (c->thumb_enabled) {
      thumb_submit_frame(&c->thumb, (const uint16_t*)c->tile_y,
                         (const uint16_t*)c->tile_u, (const uint16_t*)c->tile_v, OUT_W,
                         OUT_H);
    }
    ...
    if (now_stat_us - last_stats_us >= 2000000) {
      ...
      printf(
          "[COMP] %lds: composed=%lu encoded=%lu written=%lu "
          ...
          (long)(time(NULL) - start_time), (unsigned long)composed,
          (unsigned long)encoded, (unsigned long)written, fps,
    ...
    /* Duration check */
    if (cfg->duration_sec > 0 && (int)(time(NULL) - start_time) >= cfg->duration_sec) {
...
cleanup:
  ...
  for (int i = 0; i < c->num_rx; i++) {
    if (c->rx[i].rx) st20p_rx_free(c->rx[i].rx);
    for (int bi = 0; bi < 2; bi++) {
  ...
  /* Destroy MTL */
  if (c->mtl) mtl_uninit(c->mtl);

  /* Cleanup encoder */
  encoder_close(c);

  /* Cleanup thumbnail */
  if (c->thumb_enabled) thumb_cleanup(&c->thumb);
