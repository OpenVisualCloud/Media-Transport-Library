...
def parse_ip_link_stats(iface: str) -> dict:
    ...
    try:
        out = subprocess.check_output(
            ["ip", "-s", "link", "show", iface], stderr=subprocess.STDOUT, timeout=5
    ...
    while i < len(lines):
        ...
        if m:
            ...
            key = f"vf{vf_id}"
            result.setdefault(
                key, {"tx_bytes": 0, "rx_bytes": 0, "tx_pkts": 0, "rx_pkts": 0}
            )
...
def parse_ethtool_stats(iface: str) -> dict:
    ...
    try:
        out = subprocess.check_output(
            ["ethtool", "-S", iface], stderr=subprocess.STDOUT, timeout=5
        ).decode(errors="replace")
    except (
        subprocess.CalledProcessError,
        subprocess.TimeoutExpired,
...
def main():
    ...
    parser.add_argument(
        "--pf-iface", required=True, help="PF interface with VFs (e.g. eth0)"
    )
    parser.add_argument(
        "--vf-roles",
        default="",
    ...
    parser.add_argument(
        "--pf2-vf-roles", default="", help="VF→role map for PF2: 0=sender_8k_rx,..."
    )
    parser.add_argument(
        "--interval",
        type=float,
    ...
    parser.add_argument(
        "--quiet", action="store_true", help="Suppress per-poll stdout output"
    )
    ...
    if not all([url, token, org, bucket]):
        print(
            "[VF-STATS] ERROR: INFLUXDB_URL / TOKEN / ORG / BUCKET env vars required",
            file=sys.stderr,
        )
    ...
    if args.pf2_iface:
        print(
            f"[VF-STATS] PF2 iface: {args.pf2_iface}  VF roles: {pf2_roles}", flush=True
        )
    if args.extra_iface:
        print(
            f"[VF-STATS] Extra iface: {args.extra_iface}  role={args.extra_role}",
            flush=True,
        )
    ...
    while True:
        ...
        if prev_vf and dt > 0:
            vf_body = build_lines(
                host, args.pf_iface, cur_vf, prev_vf, dt, vf_roles, ts_ns
            )
            if vf_body:
        ...
        if prev_pf2 and dt > 0 and args.pf2_iface:
            pf2_body = build_lines(
                host, args.pf2_iface, cur_pf2, prev_pf2, dt, pf2_roles, ts_ns
            )
            if pf2_body:
            ...
            if not args.quiet:
                ...
                for entity, cur in cur_vf.items():
                    if entity in prev_vf and dt > 0:
                        ...
                        role = vf_roles.get(entity, entity)
                        print(
                            f"  {entity:4s} ({role:14s})  TX {tx_g:7.3f} Gbps  RX {rx_g:7.3f} Gbps"
                        )
                if args.extra_iface and "rdma_tx_gbps" in dir():
                    print(
                        f"  rdma ({args.extra_role:14s})  TX {rdma_tx_gbps:7.3f} Gbps  RX {rdma_rx_gbps:7.3f} Gbps"
                    )
                print(flush=True)
