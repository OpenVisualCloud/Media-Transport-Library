                                   ...
                                   struct st20_rx_frame_meta *meta)
{
    ...
    for (uint32_t i = 0; i < dma_count; i++) {
        ...
        if (atomic_load_explicit(&gs->in_use, memory_order_acquire))
            continue;
                        ...
                        volatile bool *running)
{
    ...
    /* ── Create FlowWriter ── */
    bool created = false;
    st = mxlCreateFlowWriter(bridge->mxl_instance, flow_json, NULL,
                              &bridge->writer, &bridge->config_info, &created);
    if (st != MXL_STATUS_OK) {
    ...
    if (bridge->grain_count >= 4) {
        ...
        bridge->pipeline_depth = (max_pipe < POC_PIPELINE_DEPTH) ? max_pipe : POC_PIPELINE_DEPTH;
    } else {
    ...
    if (bridge->grain_count > POC_MAX_GRAIN_SLOTS) {
        fprintf(stderr, "[BRIDGE] grain_count %u exceeds max %d\n",
                bridge->grain_count, POC_MAX_GRAIN_SLOTS);
        goto err;
    }
    ...
    for (uint32_t i = 0; i < bridge->grain_count; i++) {
        ...
        printf("[BRIDGE]   grain[%u]: addr=%p  size=%u\n",
               i, (void *)payload, ginfo.grainSize);
    }
     ...
     * contiguous pool, so they are adjacent in virtual memory. */
    {
        ...
        for (uint32_t i = 0; i < dma_count; i++) {
            ...
            printf("[BRIDGE]   grain[%u]: addr=%p  iova=0x%lx\n",
                   i, (void *)gs->addr, (unsigned long)gs->iova);
        }
    }

    printf("[BRIDGE] Zero-copy grain pool ready (grain_count=%u, dma_mapped=%u, grain_size=%zu)\n",
                           ...
                           const poc_config_t *cfg)
{
    ...
    if (!cfg->target_info_str[0]) {
        fprintf(stderr, "[BRIDGE] No target_info_str provided. "
...
static inline void drain_inflight(poc_mxl_bridge_t *bridge,
                                  poc_inflight_entry_t *ring,
                                  int *ring_head, int *inflight)
{
    ...
    while (*inflight > 0) {
        ...
        atomic_store_explicit(
            &bridge->grain_slots[e->slot].in_use,
...
static inline int try_drain(poc_mxl_bridge_t *bridge,
                            poc_inflight_entry_t *ring,
                            int *ring_head, int *inflight)
{
    mxlStatus dst = mxlFabricsInitiatorMakeProgressNonBlocking(
                        bridge->initiator);
    ...
    if (dst != MXL_ERR_NOT_READY) {
        fprintf(stderr,
                "[BRIDGE] MakeProgress error: %d — aborting\n", dst);
...
void poc_mxl_bridge_run(poc_mxl_bridge_t *bridge)
{
    ...
    while (*bridge->running) {
        ...
        if (inflight > 0) {
            if (try_drain(bridge, ring, &ring_head, &inflight) < 0)
                break;
        }
        ...
        while (inflight < pipe_depth && *bridge->running) {
            ...
            if (poc_queue_pop(bridge->queue, &entry) != 0)
                break;   /* queue empty */
            ...
            mxlStatus st = mxlFlowWriterOpenGrain(bridge->writer,
                                                   grain_index,
                                                   &grain_info, &payload);
            if (st != MXL_STATUS_OK) {
                fprintf(stderr, "[BRIDGE] OpenGrain(slot=%u) failed: %d\n",
                        slot, st);
                atomic_store_explicit(&bridge->grain_slots[slot].in_use,
                                      false, memory_order_release);
            ...
            st = mxlFabricsInitiatorTransferGrain(bridge->initiator,
                                                   grain_index, 0,
            ...
            if (st != MXL_STATUS_OK) {
                fprintf(stderr,
                        "[BRIDGE] TransferGrain(slot=%u) failed: %d\n",
                        (unsigned)slot, st);
                atomic_store_explicit(&bridge->grain_slots[slot].in_use,
                                      false, memory_order_release);
                /* put_framebuff already called above after dequeue */
        ...
        if (posted_any)
            idle_spins = 0;   /* reset back-off on activity */
    ...
    if (inflight > 0) {
        ...
        while (inflight > 0 && now_ms() < drain_deadline) {
            mxlStatus dst = mxlFabricsInitiatorMakeProgressBlocking(
                                bridge->initiator, 100);
            if (dst == MXL_STATUS_OK) {
                drain_inflight(bridge, ring, &ring_head, &inflight);
                break;
            }
        ...
        if (inflight > 0) {
            fprintf(stderr,
                    "[BRIDGE] Timed out draining %d transfers\n", inflight);
        }
    }

    printf("[BRIDGE] Bridge worker stopped (processed %lu frames, "
...
void poc_mxl_bridge_destroy(poc_mxl_bridge_t *bridge)
{
    ...
    if (bridge->mtl && bridge->dma_region_mapped) {
        mtl_dma_unmap(bridge->mtl, bridge->dma_region_base,
                      bridge->dma_region_iova, bridge->dma_region_size);
