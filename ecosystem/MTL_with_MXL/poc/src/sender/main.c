...
int main(int argc, char **argv)
{
    ...
    cfg.sip[0] = 192; cfg.sip[1] = 168; cfg.sip[2] = 1; cfg.sip[3] = 21;
    cfg.rx_ip[0] = 239; cfg.rx_ip[1] = 0; cfg.rx_ip[2] = 0; cfg.rx_ip[3] = 1;
    ...
    while ((opt = getopt_long(argc, argv, "b:s:r:u:y:W:H:N:D:d:f:p:i:P:T:F:c:q:Iw:L:t:Rh",
                               long_opts, NULL)) != -1) {
        switch (opt) {
        case 'b': strncpy(cfg.mtl_port, optarg, sizeof(cfg.mtl_port)-1); break;
        case 's': parse_ip(optarg, cfg.sip); break;
        case 'r': parse_ip(optarg, cfg.rx_ip); break;
        case 'u': cfg.rx_udp_port = (uint16_t)atoi(optarg); break;
        case 'y': cfg.payload_type = (uint8_t)atoi(optarg); break;
        case 'W': cfg.video.width = (uint32_t)atoi(optarg); break;
        case 'H': cfg.video.height = (uint32_t)atoi(optarg); break;
        case 'N': cfg.video.fps_num = (uint32_t)atoi(optarg); break;
        case 'D': cfg.video.fps_den = (uint32_t)atoi(optarg); break;
        ...
        case 'T': strncpy(cfg.target_info_str, optarg, sizeof(cfg.target_info_str)-1); break;
        case 'F': strncpy(target_info_file, optarg, sizeof(target_info_file)-1); break;
        case 'c': cfg.framebuff_cnt = (uint32_t)atoi(optarg); break;
        case 'q': cfg.queue_depth = (uint32_t)atoi(optarg); break;
        case 'I': cfg.accept_incomplete = true; break;
        case 'w': cfg.worker_cpu = atoi(optarg); break;
        case 'L': strncpy(cfg.mtl_lcores, optarg, sizeof(cfg.mtl_lcores)-1); break;
        case 't': cfg.duration_sec = atoi(optarg); break;
        case 'R': cfg.redundancy = true; break;
        case 1001: strncpy(cfg.mtl_port_r, optarg, sizeof(cfg.mtl_port_r)-1); break;
        case 1002: parse_ip(optarg, cfg.sip_r); break;
        case 1003: parse_ip(optarg, cfg.rx_ip_r); break;
        case 1004: cfg.rx_udp_port_r = (uint16_t)atoi(optarg); break;
        case 'h':
        default:
            print_usage(argv[0]);
            return (opt == 'h') ? 0 : 1;
    ...
    printf("  RX:            %d.%d.%d.%d:%u\n",
           cfg.rx_ip[0], cfg.rx_ip[1], cfg.rx_ip[2], cfg.rx_ip[3], cfg.rx_udp_port);
    if (cfg.redundancy) {
        printf("  RX (R):        %d.%d.%d.%d:%u  (ST 2022-7)\n",
               cfg.rx_ip_r[0], cfg.rx_ip_r[1], cfg.rx_ip_r[2], cfg.rx_ip_r[3],
               cfg.rx_udp_port_r);
        printf("  Port (R):      %s  SIP: %d.%d.%d.%d\n",
               cfg.mtl_port_r,
               cfg.sip_r[0], cfg.sip_r[1], cfg.sip_r[2], cfg.sip_r[3]);
    }
    printf("  Video:         %ux%u @ %u/%u fps, v210 stride=%u\n",
           cfg.video.width, cfg.video.height,
           cfg.video.fps_num, cfg.video.fps_den, cfg.video.v210_stride);
    printf("  MXL domain:    %s\n", cfg.mxl_domain);
    printf("  Flow JSON:     %s\n", cfg.flow_json_path);
    ...
    if (poc_mxl_bridge_init(&bridge, &cfg, mtl_rx.mtl,
                            &queue, &stats, &g_running) != 0) {
    ...
    if (poc_mtl_rx_create_session(&mtl_rx, &cfg, &queue, &stats,
                                  &bridge,
