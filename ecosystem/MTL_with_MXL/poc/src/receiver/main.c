...
int main(int argc, char** argv) {
  ...
  static struct option long_opts[] = {{"mxl-domain", required_argument, NULL, 'd'},
  ...
  while ((opt = getopt_long(argc, argv, "d:f:p:i:P:T:w:t:h", long_opts, NULL)) != -1) {
    switch (opt) {
      ...
      case 'P':
        ...
        break;
      case 'T':
        strncpy(thumb_dir, optarg, sizeof(thumb_dir) - 1);
        break;
      case 'w':
        cfg.consumer_cpu = atoi(optarg);
        break;
      case 't':
  ...
  /* ── Extract video dimensions from the flow JSON ── */
  {
    ...
    if (json) {
      ...
      printf("[CONFIG] Parsed from flow JSON: %ux%u @ %u/%u\n", cfg.video.width,
             cfg.video.height, cfg.video.fps_num, cfg.video.fps_den);
      free(json);
    }
  }
