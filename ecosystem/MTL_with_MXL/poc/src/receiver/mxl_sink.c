...
int poc_mxl_sink_init(poc_mxl_sink_t* sink, const poc_config_t* cfg) {
  ...
  mxlStatus st = mxlCreateFlowWriter(sink->instance, sink->flow_json, NULL, &sink->writer,
                                     &sink->config_info, &sink->created);
  if (st != MXL_STATUS_OK) {
  ...
  printf("[SINK] FlowWriter %s (grainCount=%u, sliceSize[0]=%u)\n",
         ...
         sink->config_info.discrete.grainCount, sink->config_info.discrete.sliceSizes[0]);
