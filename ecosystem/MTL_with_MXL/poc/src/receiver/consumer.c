...
void poc_consumer_run(poc_consumer_ctx_t* ctx) {
  ...
  while (*ctx->running) {
    ...
    for (;;) {
      ...
      mxlStatus st =
          mxlFabricsTargetReadGrainNonBlocking(ft->target, &entry_index, &slice_index);
      ...
      if (last_event_ns > 0) {
        ...
        if (gap_ns > batch_max_gap_ns) batch_max_gap_ns = gap_ns;
      ...
      uint8_t* recv_payload = NULL;
      {
        ...
        st = mxlFlowWriterOpenGrain(sink->writer, grain_info.index, &open_info,
                                    &recv_payload);
        if (st == MXL_STATUS_OK) {
      ...
      /* lat_consume: RDMA event → grain committed */
      {
        ...
        poc_latency_record(&stats->lat_consume, poc_ns_to_us(t_commit_ns - t_event_ns));
      }
        ...
        if (sender_ts) {
          if (t_event_ns > sender_ts) {
            ...
            /* ── E2E anomaly diagnostics ── */
            {
              ...
              if (prev_sender_ts > 0) {
                int64_t ts_delta_us = (int64_t)(sender_ts - prev_sender_ts) / 1000;
                int64_t ev_delta_us = (int64_t)(t_event_ns - prev_event_ns) / 1000;
                ...
                if (e2e_us > 5000 && diag_count < 30) {
                  fprintf(stderr,
                          ...
                          e2e_us, entry_index, ts_delta_us, ev_delta_us, sender_ts,
                          t_event_ns);
                  diag_count++;
                }

                /* Detect non-monotonic or stale sender_ts */
                if (!anomaly_seen && (ts_delta_us < 0 || ts_delta_us > 25000 ||
                                      (ts_delta_us < 10000 && ts_delta_us != 0))) {
                  fprintf(stderr,
                          "[E2E-TS-ANOMALY] ts_delta=%ldus "
                          "ev_delta=%ldus entry=%u e2e=%luus "
                          "raw[0..31]=",
                          ts_delta_us, ev_delta_us, entry_index, e2e_us);
    ...
    if (rd_tail != rd_head) {
      ...
      mxlStatus st = mxlFlowReaderGetGrainNonBlocking(sink->reader, pr->grain_index,
                                                      &read_info, &payload);
      if (st == MXL_STATUS_OK && payload) {
        ...
        if ((frame_count % 300) == 1) {
          printf(
              "[CONSUMER] Frame #%lu  slot=%u  index=%lu  "
              "slices=%u/%u  size=%u\n",
              frame_count, (unsigned)pr->entry_index, (unsigned long)pr->grain_index,
              pr->slice_index, read_info.totalSlices, read_info.grainSize);
        }
        ...
        if (ctx->thumb16) {
          poc14_thumb_write((poc14_thumb_ctx_t*)ctx->thumb16, payload,
                            read_info.grainSize);
        }
#endif
      }
    ...
    if (events_this_round > 0) {
      ...
      if (idle_spins > batch_max_idle) batch_max_idle = idle_spins;
    }

    /* ── Periodic batch report (every 10s) ── */
    {
      uint64_t now_ns = poc_now_realtime_ns();
      if (batch_report_ns == 0) batch_report_ns = now_ns + BATCH_REPORT_INTERVAL_NS;
      if (now_ns >= batch_report_ns) {
        double avg_batch =
            batch_total_rounds > 0 ? (double)batch_total_events / batch_total_rounds : 0;
        fprintf(stderr,
                "[BATCH] events=%lu rounds=%u avg_batch=%.1f max_batch=%u "
                "max_idle=%u max_gap_ms=%.1f\n",
                (unsigned long)batch_total_events, batch_total_rounds, avg_batch,
                batch_max_size, batch_max_idle, batch_max_gap_ns / 1e6);
  ...
  if (do_thumb) poc_thumbnail_cleanup();
