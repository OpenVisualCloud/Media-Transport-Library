...
static inline void poc_latency_record(poc_latency_t* lat, uint64_t us) {
  ...
  if (lat->ring_n < POC_LATENCY_SAMPLES_MAX) lat->ring_n++;
}
...
static inline uint64_t poc_latency_percentile(const poc_latency_t* lat, uint64_t* scratch,
                                              unsigned pct) {
...
static inline uint64_t poc_frame_ts_read(const uint8_t* payload) {
  ...
  if (ts->magic != POC_TS_MAGIC) return 0;
