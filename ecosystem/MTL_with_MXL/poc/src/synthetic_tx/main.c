...
{
    return ((uint32_t)(c2 & 0x3FF) << 20) |
           ((uint32_t)(c1 & 0x3FF) << 10) |
           ((uint32_t)(c0 & 0x3FF));
}
...
static void apply_dest_update(st20p_tx_handle h, const synth_config_t *cfg,
    ...
    if (ret < 0)
        fprintf(stderr, "[CTRL] st20p_tx_update_destination failed: %d\n", ret);
}
...
static void poll_ctrl_file(mtl_handle mtl, st20p_tx_handle tx_h,
    ...
    if (cur.corrupt != g_ctrl_prev.corrupt) {
        if (cur.corrupt) {
            ...
            if (cfg->redundancy)
                mtl_set_runtime_tx_packet_loss(mtl, MTL_PORT_R, true, 1, 2);
            printf("[CTRL] Packet corruption ENABLED (per-pkt redirect)\n");
        } else {
            mtl_set_runtime_tx_packet_loss(mtl, MTL_PORT_P, false, 0, 0);
            if (cfg->redundancy)
                mtl_set_runtime_tx_packet_loss(mtl, MTL_PORT_R, false, 0, 0);
...
static void *tx_thread_fn(void *arg)
{
    ...
    while (!ctx->stop && g_running) {
        ...
        if (ctx->thumb_dir[0]) {
            ...
            poc_thumb_tx_write(planes, g_ctrl_prev.corrupt,
                               g_ctrl_prev.mute_p, g_ctrl_prev.mute_r);
        }
...
int main(int argc, char **argv)
{
    ...
    cfg.sip[0] = 192; cfg.sip[1] = 168; cfg.sip[2] = 1; cfg.sip[3] = 22;
    cfg.tx_ip[0] = 239; cfg.tx_ip[1] = 0; cfg.tx_ip[2] = 0; cfg.tx_ip[3] = 1;
    ...
    while ((opt = getopt_long(argc, argv, "h", long_opts, NULL)) != -1) {
        switch (opt) {
        case 'b': strncpy(cfg.port, optarg, sizeof(cfg.port) - 1);   break;
        case 's': parse_ip(optarg, cfg.sip);                          break;
        case 't': parse_ip(optarg, cfg.tx_ip);                        break;
        case 'u': cfg.tx_udp_port  = (uint16_t)atoi(optarg);          break;
        case 'y': cfg.payload_type = (uint8_t)atoi(optarg);           break;
        case 'W': cfg.width  = (uint32_t)atoi(optarg);                break;
        case 'H': cfg.height = (uint32_t)atoi(optarg);                break;
        case 'N': cfg.fps_num = (uint32_t)atoi(optarg);               break;
        case 'D': cfg.fps_den = (uint32_t)atoi(optarg);               break;
        case 'F': cfg.framebuff_cnt = (uint32_t)atoi(optarg);         break;
        case 'P': strncpy(cfg.pattern, optarg, sizeof(cfg.pattern)-1);break;
        case 'd': cfg.duration_sec = atoi(optarg);                    break;
        case 'L': strncpy(cfg.lcores, optarg, sizeof(cfg.lcores)-1);   break;
        case 'Y': strncpy(cfg.yuv_file, optarg, sizeof(cfg.yuv_file)-1);  break;
        case 'R': cfg.redundancy = true;                                     break;
        case 1001: strncpy(cfg.port_r, optarg, sizeof(cfg.port_r)-1);       break;
        case 1002: parse_ip(optarg, cfg.sip_r);                              break;
        case 1003: parse_ip(optarg, cfg.tx_ip_r);                            break;
        case 1004: cfg.tx_udp_port_r = (uint16_t)atoi(optarg);               break;
        case 1005: strncpy(cfg.thumb_dir, optarg, sizeof(cfg.thumb_dir)-1);     break;
    ...
    size_t yuv_frame_size = (size_t)cfg.width * cfg.height * 2
                          + (size_t)(cfg.width / 2) * cfg.height * 2 * 2;
    ...
    printf("\n"
           ...
           "  Source IP P: %s\n"
           "  Dest P:      %s:%u\n",
           cfg.redundancy ? " [ST 2022-7]" : "",
           cfg.port, sip_str, dip_str, cfg.tx_udp_port);
    if (cfg.redundancy) {
        printf("  NIC port R:  %s\n"
               "  Source IP R: %s\n"
               "  Dest R:      %s:%u\n",
    ...
    printf("  Resolution:  %ux%u @ %.2f fps\n"
           "  Input:       %s\n"
           "  Transport:   YUV_422_10BIT (RFC 4175)\n"
           "  Duration:    %s\n"
           "══════════════════════════════════════════════\n\n",
           cfg.width, cfg.height, fps_val,
           use_yuv_file ? cfg.yuv_file : cfg.pattern,
    ...
    printf("[SYNTH_TX] Pipeline TX session created: %ux%u, YUV422P10LE→RFC4175, frame_size=%zu\n",
    ...
    if (use_yuv_file) {
        ...
        ctx.yuv_frame_idx = 0;
        printf("[SYNTH_TX] YUV file loaded: %s\n"
               "           %lu frames, %zu bytes/frame (%.2f MB total)\n",
               cfg.yuv_file, ctx.yuv_total_frames,
               ctx.yuv_frame_size, (double)ctx.yuv_file_size / (1024.0*1024.0));
    } else {
