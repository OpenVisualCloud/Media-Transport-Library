...
static inline void ycbcr_to_rgb(int y10, int cb10, int cr10,
...
static inline void rfc4175_get_pixel(const uint8_t *line, uint32_t x,
                                     int *y, int *cb, int *cr)
{
    uint32_t pair = x / 2;
    uint32_t pos  = x & 1;
...
static int write_jpeg(const uint8_t *rgb, uint32_t w, uint32_t h)
{
    ...
    if (!fp) {
        ...
        if (++err_count <= 3)
            fprintf(stderr, "[THUMB] Cannot open %s: %m\n", s_path_tmp);
        return -1;
    }
    ...
    if (rename(s_path_tmp, s_path_final) != 0) {
        static int err_count = 0;
        if (++err_count <= 3)
            fprintf(stderr, "[THUMB] rename failed: %m\n");
        return -1;
    }
    return 0;
}
                       ...
                       uint32_t src_width, uint32_t src_height)
{
    ...
    if (!s_hot_buf || !s_ready_buf || !s_enc_buf) {
        free(s_hot_buf); free(s_ready_buf); free(s_enc_buf);
    ...
    s_inited = true;
    fprintf(stderr, "[THUMB] Initialised: %ux%u → %ux%u (RFC 4175, stride=%u), "
...
int poc_thumbnail_write(const uint8_t *rfc4175_data, uint32_t frame_size)
{
    ...
    for (uint32_t ty = 0; ty < s_thumb_h; ty++) {
        ...
        for (uint32_t tx = 0; tx < s_thumb_w; tx++) {
            ...
            ycbcr_to_rgb(y_val, cb_val, cr_val,
                         &dst[tx * 3 + 0],
                         &dst[tx * 3 + 1],
                         &dst[tx * 3 + 2]);
        }
...
void poc_thumbnail_cleanup(void)
{
    ...
    free(s_hot_buf);   s_hot_buf   = NULL;
    free(s_ready_buf); s_ready_buf = NULL;
