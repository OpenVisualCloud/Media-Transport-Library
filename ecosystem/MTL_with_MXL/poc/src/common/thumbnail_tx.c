...
static int write_jpeg(const char* path_tmp, const char* path_final, const uint8_t* rgb,
                      uint32_t w, uint32_t h) {
  FILE* fp = fopen(path_tmp, "wb");
  if (!fp) {
    static int err_count = 0;
    if (++err_count <= 3) fprintf(stderr, "[TX_THUMB] Cannot open %s: %m\n", path_tmp);
    return -1;
  }
  ...
  if (rename(path_tmp, path_final) != 0) {
    static int err_count = 0;
    if (++err_count <= 3) fprintf(stderr, "[TX_THUMB] rename failed: %m\n");
    return -1;
  }
  return 0;
}
...
int poc_thumb_tx_init(const char* thumb_dir, uint32_t src_w, uint32_t src_h) {
  ...
  if (!s_hot_p || !s_ready_p || !s_enc_p || !s_hot_r || !s_ready_r || !s_enc_r) {
    free(s_hot_p);
    free(s_ready_p);
    free(s_enc_p);
    free(s_hot_r);
  ...
  s_inited = true;
  fprintf(stderr,
          "[TX_THUMB] Initialised: %ux%u → %ux%u, pgroups/pkt=%u, "
