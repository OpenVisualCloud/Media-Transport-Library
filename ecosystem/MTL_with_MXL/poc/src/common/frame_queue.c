...
int poc_queue_init(poc_frame_queue_t* q, uint32_t depth) {
  if (!q || depth == 0 || depth > POC_QUEUE_MAX_DEPTH) return -1;
  /* depth must be power of 2 */
  if ((depth & (depth - 1)) != 0) return -1;
...
int poc_queue_push(poc_frame_queue_t* q, const poc_frame_entry_t* entry) {
  ...
  /* full? */
  if (((head + 1) & q->mask) == (tail & q->mask)) return -1;

  q->entries[head & q->mask] = *entry;

  atomic_store_explicit(&q->head, (head + 1) & (q->depth * 2 - 1), memory_order_release);
  return 0;
}
...
int poc_queue_pop(poc_frame_queue_t* q, poc_frame_entry_t* out) {
  ...
  *out = q->entries[tail & q->mask];
  ...
  atomic_store_explicit(&q->tail, (tail + 1) & (q->depth * 2 - 1), memory_order_release);
  return 0;
}
