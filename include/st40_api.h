 ...
 * join/drop. Use st40_rx_get_queue_meta to get the queue meta(queue number etc)
 * info.
 */
...
MTL_PACK(struct st40_rfc8331_rtp_hdr {
  ...
  struct {
    ...
    /** signaling the field specified by the RTP timestamp in an interlaced SDI
     * raster */
    uint32_t f : 2;
    /** reserved */
...
MTL_PACK(struct st40_rfc8331_payload_hdr {
  union {
    struct {
      ...
      /** line number corresponds to the location (vertical) of the ANC data
       * packet */
...
struct st40_tx_ops {
  ...
  /**
   * Mandatory for ST40_TYPE_FRAME_LEVEL. callback when lib require a new frame
   * for sending. User should provide the next available frame index to
  ...
  /**
   * Optional for ST40_TYPE_FRAME_LEVEL. callback when lib finish sending one
  ...
  /**
   * Optional for ST40_TYPE_RTP_LEVEL. callback when lib finish the sending of
   * one rtp packet, And only non-block method can be used in this callback as
...
struct st40_rx_ops {
  ...
  uint32_t rtp_ring_size;
  /**
   * Optional. the callback when lib finish the sending of one rtp packet. And
...
/**
 * Get the mbuf pointer and usrptr of the mbuf from the tx st2110-40(ancillary)
 * session. For ST40_TYPE_RTP_LEVEL. Must call st40_tx_put_mbuf to return the
...
/**
 * Put back the mbuf which get by st40_tx_get_mbuf to the tx
 * st2110-40(ancillary) session. For ST40_TYPE_RTP_LEVEL.
 *
 * @param handle
 *   The handle to the tx st2110-40(ancillary) session.
 * @param mbuf
...
/**
 * Get the mbuf pointer and usrptr of the mbuf from the rx st2110-40(ancillary)
 * session. For ST40_TYPE_RTP_LEVEL. Must call st40_rx_put_mbuf to return the
...
/**
 * Put back the mbuf which get by st40_rx_get_mbuf to the rx
 * st2110-40(ancillary) session. For ST40_TYPE_RTP_LEVEL.
 *
 * @param handle
 *   The handle to the rx st2110-40(ancillary) session.
