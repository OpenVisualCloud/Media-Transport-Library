...
/**
 * A structure used to retrieve general statistics(I/O) for a st40 tx port.
 */
...
/**
 * A structure used to retrieve general statistics(I/O) for a st40 rx port.
 */
struct st40_rx_user_stats {
  struct st_rx_port_stats port[MTL_SESSION_PORT_MAX];
  uint64_t stat_pkts_wrong_interlace_dropped; /**< Base structure for rx port status */
};

/**
 * Retrieve the general statistics(I/O) for one tx st2110-40(ancillary) session port.
 *
 * @param handle
 *   The handle to the tx st2110-40(video) session.
 * @param port
 *   The port index.
 * @param stats
...
/**
 * Reset the general statistics(I/O) for one tx st2110-40(video) session port.
 *
 * @param handle
 *   The handle to the tx st2110-40(video) session.
 * @param port
...
/**
 * Retrieve the general statistics(I/O) for one rx st2110-40(ancillary) session port.
 *
 * @param handle
...
/**
 * Reset the general statistics(I/O) for one rx st2110-40(ancillary) session port.
 *
 * @param handle
 *   The handle to the rx st2110-40(ancillary) session.
 * @param port
