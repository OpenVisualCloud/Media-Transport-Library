 ...
 * It include a plugin layer to hide the covert/encode detail that application
 * can focus on the raw pixel handling.
 *
 */
...
enum st_frame_fmt {
   ...
   * This is used when user wants to directly transport none-RFC4175 formats
   * like I420/NV12. When this input/output format is set, the frame is
   * identical to transport frame without conversion. The frame should not have
   * lines padding.
   */
  ST_FRAME_FMT_YUV420CUSTOM8 = 12,
  /** Customized YUV 422 8bit, set transport format as ST20_FMT_YUV_422_8BIT.
   * This is used when user wants to directly transport none-RFC4175 formats
   * like YUY2. When this input/output format is set, the frame is identical to
   * transport frame without conversion. The frame should not have lines
   * padding.
   */
...
#define ST_FMT_CAP_YUV422RFC4175PG2BE10                                        \
  (MTL_BIT64(ST_FRAME_FMT_YUV422RFC4175PG2BE10))
...
/** ST format cap of ST_FRAME_FMT_JPEGXS_CODESTREAM, used in the st22_plugin
 * caps */
#define ST_FMT_CAP_JPEGXS_CODESTREAM (MTL_BIT64(ST_FRAME_FMT_JPEGXS_CODESTREAM))
/** ST format cap of ST_FRAME_FMT_H264_CBR_CODESTREAM, used in the st22_plugin
 * caps */
#define ST_FMT_CAP_H264_CBR_CODESTREAM                                         \
  (MTL_BIT64(ST_FRAME_FMT_H264_CBR_CODESTREAM))
...
/** ST format cap of ST_FRAME_FMT_H265_CBR_CODESTREAM, used in the st22_plugin
 * caps */
#define ST_FMT_CAP_H265_CBR_CODESTREAM                                         \
  (MTL_BIT64(ST_FRAME_FMT_H265_CBR_CODESTREAM))
...
struct st_frame {
  ...
  /**
   * The user meta data buffer for current frame of st20, the size must smaller
   * than MTL_PKT_MAX_RTP_BYTES. This data will be transported to RX with video
   * data and passed to user by user_meta also.
   */
...
enum st22p_tx_flag {
   ...
   * degrade the performance since the object enqueue/dequeue will be acted one
   * by one.
   */
  ...
  /** Enable the st22p_tx_get_frame block behavior to wait until a frame becomes
   available or timeout(default: 1s, use st22p_tx_set_block_timeout to
   customize) */
...
enum st22p_rx_flag {
   ...
   * join/drop. Use st22p_rx_get_queue_meta to get the queue meta(queue number
   * etc) info.
   */
   ...
   * callback(query_ext_frame in st22p_rx_ops) to let MTL can get the frame when
   * needed.
   */
  ...
  /** Enable the st22p_rx_get_frame block behavior to wait until a frame becomes
     available or timeout(default: 1s, use st22p_rx_set_block_timeout to
     customize) */
...
enum st20p_rx_flag {
  ...
  /**
   * Enable the dynamic external frame mode, and user must provide a query
   * callback(query_ext_frame in st20p_rx_ops) to let MTL can get the frame when
   * needed. Note to enable ST20P_RX_FLAG_RECEIVE_INCOMPLETE_FRAME also for
   * non-converter mode.
   */
...
enum st22_decoder_resp_flag {
  /** Enable the st22_decoder_get_frame block behavior to wait until a frame
     becomes available or timeout(default: 1s, use
     st22_decoder_set_block_timeout to customize)
   */
...
enum st22_encoder_resp_flag {
  /** Enable the st22_encoder_get_frame block behavior to wait until a frame
     becomes available or timeout(default: 1s, use
     st22_encoder_set_block_timeout to customize)
   */
...
struct st20_converter_dev {
  ...
  uint64_t output_fmt_caps;
  /** create session function */
  st20_convert_priv (*create_session)(void *priv,
...
struct st20p_tx_ops {
  ...
  /**
   * Mandatory. The frame buffer count requested for one st20 pipeline tx
   * session, should be in range [2, ST20_FB_MAX_COUNT],
   */
  uint16_t framebuff_cnt;
  ...
  void *priv;
  /** Optional. Flags to control session behaviors. See ST20P_TX_FLAG_* for
   * possible value
   */
  uint32_t flags;
  /**
  ...
  /**  Use this socket if ST20P_TX_FLAG_FORCE_NUMA is on, default use the NIC
   * numa */
  int socket_id;
};
...
struct st20p_rx_ops {
  ...
  enum st_plugin_device device;
  /**
   * Mandatory. The frame buffer count requested for one st20 pipeline rx
   * session, should be in range [2, ST20_FB_MAX_COUNT],
   */
  uint16_t framebuff_cnt;
  ...
  /** Optional. private data to the callback function */
  void *priv;
  /** Optional. Flags to control session behaviors. See ST20P_RX_FLAG_* for
   * possible value
   */
  uint32_t flags;
  ...
  /**
   * Optional. event callback, lib will call this when there is some event
   * happened. Only non-block method can be used in this callback as it run from
   * lcore routine. args point to the meta data of each event. Ex, cast to
   * struct st10_vsync_meta for ST_EVENT_VSYNC.
   */
  int (*notify_event)(void *priv, enum st_event event, void *args);
  /**
   * Optional with ST20_RX_FLAG_AUTO_DETECT. The callback when lib detected
   * video format. And only non-block method can be used in this callback as it
  ...
  /**  Use this socket if ST20P_RX_FLAG_FORCE_NUMA is on, default use the NIC
   * numa */
  int socket_id;
};
...
struct st22p_tx_ops {
  ...
  enum st22_quality_mode quality;
  /** Mandatory. codestream size, calculate as compress ratio. For interlaced,
   * it's the expected codestream size for each field */
  size_t codestream_size;
  /**
   *  Mandatory. the frame buffer count requested for one st22 pipeline tx
   * session, should be in range [2, ST22_FB_MAX_COUNT],
   */
  uint16_t framebuff_cnt;
  ...
  /** Optional. private data to the callback function */
  void *priv;
  /** Optional. Flags to control session behaviors. See ST22P_TX_FLAG_* for
   * possible value
   */
  uint32_t flags;
   ...
   */
  int (*notify_event)(void *priv, enum st_event event, void *args);
  /**  Use this socket if ST22P_TX_FLAG_FORCE_NUMA is on, default use the NIC
   * numa */
  int socket_id;
};
...
struct st22p_rx_ops {
  ...
  enum st_plugin_device device;
  /**
   * Mandatory. the frame buffer count requested for one st22 pipeline rx
   * session, should be in range [2, ST22_FB_MAX_COUNT],
   */
  uint16_t framebuff_cnt;
  ...
  /** Optional. private data to the callback function */
  void *priv;
  /** Optional. Flags to control session behaviors. See ST22P_RX_FLAG_* for
   * possible value
   */
  uint32_t flags;
  /** Optional. thread count for codec, leave to zero if not know */
   ...
   * For interlaced, it's the expected codestream size for each field */
  size_t max_codestream_size;
  ...
  int (*notify_event)(void *priv, enum st_event event, void *args);
  /**
   * Mandatory for ST22P_RX_FLAG_EXT_FRAME. Callback when the lib query next
   * external frame's data address. And only non-block method can be used within
   * this callback as it run from lcore tasklet routine.
   */
  int (*query_ext_frame)(void *priv, struct st_ext_frame *ext_frame,
                         struct st22_rx_frame_meta *meta);
  /**  Use this socket if ST22P_RX_FLAG_FORCE_NUMA is on, default use the NIC
   * numa */
  int socket_id;
};
 ...
 */
struct st22_encode_frame_meta *
st22_encoder_get_frame(st22p_encode_session session);

/**
 * Wake up the block wait on st22_encoder_get_frame if
 ...
 *   - <0: Error code.
 */
int st22_encoder_set_block_timeout(st22p_encode_session session,
 ...
 */
struct st22_decode_frame_meta *
st22_decoder_get_frame(st22p_decode_session session);

/**
 * Wake up the block wait on st22_decoder_get_frame if
 ...
 *   - <0: Error code.
 */
int st22_decoder_set_block_timeout(st22p_decode_session session,
                                   uint64_t timedwait_ns);
 ...
 */
st20_converter_dev_handle
st20_converter_register(mtl_handle mt, struct st20_converter_dev *dev);
 ...
 */
struct st20_convert_frame_meta *
st20_converter_get_frame(st20p_convert_session session);
 ...
 * Call st22p_tx_put_frame or st22p_tx_put_ext_frame(if ST22P_TX_FLAG_EXT_FRAME)
 * to return the frame to session.
 *
 * @param handle
 *   The handle to the tx st2110-22 pipeline session.
 * @return
 ...
 *   - <0: Error code.
 */
int st22p_tx_update_destination(st22p_tx_handle handle,
                                struct st_tx_dest_info *dst);

/**
 * Wake up the block wait on st22p_tx_get_frame if ST22P_TX_FLAG_BLOCK_GET is
 * enabled.
 *
 * @param handle
 *   The handle to the tx st2110-22(pipeline) session.
...
/**
 * Set the block timeout time on st22p_tx_get_frame if ST22P_TX_FLAG_BLOCK_GET
 * is enabled.
 *
 * @param handle
 *   The handle to the tx st2110-22(pipeline) session.
...
/**
 * Wake up the block wait on st22p_rx_get_frame if ST22P_RX_FLAG_BLOCK_GET is
 * enabled.
 *
 * @param handle
 *   The handle to the rx st2110-22(pipeline) session.
...
/**
 * Set the block timeout time on st22p_rx_get_frame if ST22P_RX_FLAG_BLOCK_GET
 * is enabled.
 *
 * @param handle
 *   The handle to the rx st2110-22(pipeline) session.
 ...
 * Call st20p_tx_put_frame/st20p_tx_put_ext_frame(if ST20P_TX_FLAG_EXT_FRAME) to
 * return the frame to session.
 *
 * @param handle
 *   The handle to the tx st2110-20 pipeline session.
 * @return
...
/**
 * Wake up the block wait on st20p_tx_get_frame if ST20P_TX_FLAG_BLOCK_GET is
 * enabled.
 *
 * @param handle
 *   The handle to the tx st2110-20(pipeline) session.
...
/**
 * Set the block timeout time on st20p_tx_get_frame if ST20P_TX_FLAG_BLOCK_GET
 * is enabled.
 *
 * @param handle
 *   The handle to the tx st2110-20(pipeline) session.
 ...
 *   - <0: Error code.
 */
int st20p_rx_reset_port_stats(st20p_rx_handle handle,
                              enum mtl_session_port port);
 ...
 *   - <0: Error code.
 */
int st20p_rx_update_source(st20p_rx_handle handle,
                           struct st_rx_source_info *src);
...
/**
 * Wake up the block wait on st20p_rx_get_frame if ST20P_RX_FLAG_BLOCK_GET is
 * enabled.
 *
 * @param handle
 *   The handle to the rx st2110-20(pipeline) session.
...
/**
 * Set the block timeout time on st20p_rx_get_frame if ST20P_RX_FLAG_BLOCK_GET
 * is enabled.
 *
 * @param handle
 *   The handle to the rx st2110-20(pipeline) session.
...
static inline bool st_frame_fmt_is_codestream(enum st_frame_fmt fmt) {
  if (fmt >= ST_FRAME_FMT_CODESTREAM_START &&
      fmt <= ST_FRAME_FMT_CODESTREAM_END)
    return true;
  else
    return false;
}
 ...
 *   size
 */
static inline size_t st_frame_plane_size(struct st_frame *frame,
                                         uint8_t plane) {
  /* no line size for codestream */
  if (st_frame_fmt_is_codestream(frame->fmt))
...
/**
 * This helper function retrieves the actual data height in one st frame. Please
 * note that for an interlaced frame, it will return half the height.
 *
 * @param frame
 *   The st_frame pointer.
 * @return
...
static inline void st_rxp_para_udp_port_set(struct st_rx_port *p,
                                            enum mtl_port port,
                                            uint16_t udp_port) {
...
static inline void st_txp_para_udp_port_set(struct st_tx_port *p,
                                            enum mtl_port port,
                                            uint16_t udp_port) {
  p->udp_port[port] = udp_port;
}
...
static inline mtl_cpuva_t st_frame_addr_cpuva(struct st_frame *frame,
                                              uint8_t plane) {
...
static inline struct st20_rx_tp_meta *
st_frame_tp_meta(struct st_frame *frame, enum mtl_session_port port) {
