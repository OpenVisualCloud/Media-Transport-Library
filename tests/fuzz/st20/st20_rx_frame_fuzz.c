...
static void st20_fuzz_log_printer(enum mtl_log_level level, const char* format, ...) {
...
#define ST20_FUZZ_POOL_SIZE 2048
#define ST20_FUZZ_POOL_NAME "st20_rx_fuzz_pool"
#define ST20_FUZZ_MAX_PKT_SIZE 4096
#define ST20_FUZZ_MIN_PKT_SIZE (sizeof(struct st20_rfc4175_rtp_hdr))
#define ST20_FUZZ_FRAME_COUNT 2
#define ST20_FUZZ_WIDTH 640
#define ST20_FUZZ_HEIGHT 480
#define ST20_FUZZ_FRAME_SIZE_BYTES (ST20_FUZZ_WIDTH * ST20_FUZZ_HEIGHT * 2)
#define ST20_FUZZ_BITMAP_SIZE ((ST20_FUZZ_FRAME_SIZE_BYTES / 800 / 8) + 8)
...
static struct st_frame_trans g_frames[ST20_FUZZ_FRAME_COUNT];
static uint8_t g_frame_storage[ST20_FUZZ_FRAME_COUNT][ST20_FUZZ_FRAME_SIZE_BYTES];
static uint8_t g_frame_user_meta[ST20_FUZZ_FRAME_COUNT][256];
static uint8_t g_slot_bitmaps[ST_VIDEO_RX_REC_NUM_OFO][ST20_FUZZ_BITMAP_SIZE];

static void st20_fuzz_enable_logging(void) {
  static bool logging_ready;
  if (logging_ready) return;

  mt_set_log_global_level(MTL_LOG_LEVEL_DEBUG);
  rte_log_set_global_level(RTE_LOG_DEBUG);
  mtl_set_log_printer(st20_fuzz_log_printer);
  if (mtl_openlog_stream(stderr) < 0) {
    fprintf(stderr, "st20 fuzz: failed to route MTL logs to stderr\n");
  }
  logging_ready = true;
}

static uint64_t st20_fuzz_ptp_time(struct mtl_main_impl* impl, enum mtl_port port) {
  MTL_MAY_UNUSED(port);
...
static void st20_fuzz_prepare_frames(void) {
  for (int i = 0; i < ST20_FUZZ_FRAME_COUNT; i++) {
...
static int st20_fuzz_notify_frame_ready(void* priv, void* frame,
...
static void st20_fuzz_init_eal(void) {
  st20_fuzz_enable_logging();
  if (g_pool) return;

  if (!g_eal_ready) {
    static char arg0[] = "st20_rx_fuzz";
  ...
  g_pool = rte_pktmbuf_pool_create(ST20_FUZZ_POOL_NAME, ST20_FUZZ_POOL_SIZE, 0, 0,
                                   RTE_MBUF_DEFAULT_BUF_SIZE, rte_socket_id());
  if (!g_pool) abort();
}

static void st20_fuzz_reset_context(void) {
  if (!g_pool) return;
  ...
  g_impl.inf[MTL_PORT_P].ptp_get_time_fn = st20_fuzz_ptp_time;
  ...
  g_session.st20_dst_port[MTL_SESSION_PORT_P] = 10000;
  g_session.detector.status = ST20_DETECT_STAT_DISABLED;

  g_session.ops.num_port = 1;
  g_session.ops.type = ST20_TYPE_FRAME_LEVEL;
  g_session.ops.width = ST20_FUZZ_WIDTH;
  g_session.ops.height = ST20_FUZZ_HEIGHT;
  g_session.ops.fps = ST_FPS_P59_94;
  g_session.ops.interlaced = false;
  g_session.ops.fmt = ST20_FMT_YUV_422_8BIT;
  g_session.ops.payload_type = 0;
  g_session.ops.packing = ST20_PACKING_BPM;
  g_session.ops.framebuff_cnt = ST20_FUZZ_FRAME_COUNT;
  g_session.ops.notify_frame_ready = st20_fuzz_notify_frame_ready;
  g_session.ops.priv = &g_session;
  g_session.ops.name = "st20_rx_fuzz";
  g_session.ops.udp_port[MTL_SESSION_PORT_P] = 10000;

  if (st20_get_pgroup(g_session.ops.fmt, &g_session.st20_pg) < 0) abort();

  g_session.st20_frames = g_frames;
  g_session.st20_frames_cnt = ST20_FUZZ_FRAME_COUNT;
  g_session.st20_frame_size = ST20_FUZZ_FRAME_SIZE_BYTES;
  ...
  g_session.st20_frame_bitmap_size = ST20_FUZZ_BITMAP_SIZE;

  size_t raw_bytes = (size_t)ST20_FUZZ_WIDTH * g_session.st20_pg.size;
...
int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
  if (!data || size < ST20_FUZZ_MIN_PKT_SIZE) return 0;

  size_t pkt_size = size;
  if (pkt_size > ST20_FUZZ_MAX_PKT_SIZE) pkt_size = ST20_FUZZ_MAX_PKT_SIZE;

  st20_fuzz_init_eal();
  st20_fuzz_reset_context();
