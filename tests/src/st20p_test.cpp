...
static void st20p_tx_ops_init(tests_context* st20, struct st20p_tx_ops* ops_tx) {
  ...
  strncpy(ops_tx->port.port[MTL_SESSION_PORT_P], ctx->para.port[MTL_PORT_P],
          MTL_PORT_MAX_LEN);
  ops_tx->port.udp_dst_port[MTL_SESSION_PORT_P] = ST20P_TEST_UDP_PORT + st20->idx;
...
static void st20p_rx_ops_init(tests_context* st20, struct st20p_rx_ops* ops_rx) {
  ...
  strncpy(ops_rx->port.port[MTL_SESSION_PORT_P], ctx->para.port[MTL_PORT_R],
          MTL_PORT_MAX_LEN);
  ops_rx->port.udp_dst_port[MTL_SESSION_PORT_P] = ST20P_TEST_UDP_PORT + st20->idx;
...
TEST(St20p, tx_create_free_multi) { pipeline_create_free_test(st20p_tx, 0, 1, 6); }
TEST(St20p, tx_create_free_mix) { pipeline_create_free_test(st20p_tx, 2, 3, 4); }
...
static void st20p_rx_digest_test(enum st_fps fps[], int width[], int height[],
  ...
  for (int i = 0; i < sessions; i++) {
    ...
    strncpy(ops_tx.port.port[MTL_SESSION_PORT_P], ctx->para.port[MTL_PORT_P],
            MTL_PORT_MAX_LEN);
    ops_tx.port.udp_dst_port[MTL_SESSION_PORT_P] = ST20P_TEST_UDP_PORT + i;
    ops_tx.port.payload_type = ST20P_TEST_PAYLOAD_TYPE;
  ...
  for (int i = 0; i < sessions; i++) {
    ...
    strncpy(ops_rx.port.port[MTL_SESSION_PORT_P], ctx->para.port[MTL_PORT_R],
            MTL_PORT_MAX_LEN);
    ops_rx.port.udp_dst_port[MTL_SESSION_PORT_P] = ST20P_TEST_UDP_PORT + i;
    ops_rx.port.payload_type = ST20P_TEST_PAYLOAD_TYPE;
