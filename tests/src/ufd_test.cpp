...
template <typename OPT_TYPE> static void socketopt_double(OPT_TYPE *i) {
  OPT_TYPE value = *i;
  *i = value * 2;
}

template <typename OPT_TYPE> static void socketopt_half(OPT_TYPE *i) {
  OPT_TYPE value = *i;
  *i = value / 2;
}
...
template <> void socketopt_double(struct timeval *i) {
  i->tv_sec *= 2;
  i->tv_usec *= 2;
}
template <> void socketopt_half(struct timeval *i) {
  i->tv_sec /= 2;
  i->tv_usec /= 2;
}
...
static int check_r_port_alive(struct mtl_init_params *p) {
  ...
  while (retry < max_retry) {
    if (mufd_sendto(tx_fd, send_buf, payload_len, 0,
...
GTEST_API_ int main(int argc, char **argv) {
  ...
  if (ctx->dhcp) {
    for (int i = 0; i < ctx->init_params.mt_params.num_ports; i++) {
      ...
      mufd_port_ip_info((enum mtl_port)i,
                        ctx->init_params.mt_params.sip_addr[i],
