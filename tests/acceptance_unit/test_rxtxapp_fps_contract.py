...
``parse_fps_to_pformat()`` is the only correct way to build the token.
...
from mtl_engine.media_files import parse_fps_to_pformat  # noqa: E402
...
# Known debt, named rather than silently skipped: the performance suite
# parametrizes exclusively over fractional rows (i1080p29/59, i2160p29/59,
# i4320p29/50/59), so every one of its cases carries this defect and none can
# run until the suite is converted or retired. Out of scope here.
UNCONVERTED = ("tests/single/performance/",)


def fractional_tables():
    ...
    for name, value in vars(media_files).items():
        ...
        rates = [
            ...
            if isinstance(entry, dict) and "/" in str(entry.get("fps", ""))
        ]
        if rates:
            tables[name] = rates
    return tables
...
class FpsTokenContractTests(unittest.TestCase):
    def setUp(self):
        self.fractional = fractional_tables()
    ...
    def test_no_test_module_interpolates_a_fractional_tables_fps(self):
        ...
        self.assertTrue(readers, "found no module reading a fractional media table")
        offenders = {
            ...
            for table, rates in self.fractional.items():
                for rate in rates:
                    with self.subTest(parser=parser, table=table, fps=rate):
                        self.assertIn(parse_fps_to_pformat(rate), tokens)
