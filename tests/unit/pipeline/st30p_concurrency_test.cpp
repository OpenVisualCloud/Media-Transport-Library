 ...
 *
 * ST30p (audio) pipeline-layer concurrency tests for the lock-free framebuffer
 * ring. The mutex that used to serialise the frame state machine has been
 * removed; claiming a slot is now a single atomic compare-exchange. These
 * tests hammer that claim from many threads and assert three properties:
 *
 *   1. single-ownership   — no buffer is ever handed to two actors at once
 *                           (a CAS regression to a plain store trips this).
 *   2. conservation       — every produced frame is consumed exactly once.
 *   3. deadlock-freedom   — the run completes within a wall-clock budget.
 *
 * Topologies (mirrors the two real deployments):
 *   TX: N app producers (get_frame/put_frame) + 1 transport consumer
 *       (next_frame/frame_done).
 *   RX: 1 transport producer (frame_ready) + N app consumers
 *       (get_frame/put_frame).
 */
...
#include "pipeline/st30p_harness.h"    /* RX role: ut30p_* */
#include "pipeline/st30p_tx_harness.h" /* TX role: ut30p_tx_* */
 ...
 * livelock/deadlock approaches this. The generous margin also covers a degraded
 * unpinned fallback on a multi-socket host. */
...
TEST(St30PipelineConcurrency, TxMultiProducerSingleConsumerNoDeadlock) {
  ...
  auto producer = [&](int id) {
    while (!stop.load(std::memory_order_relaxed)) {
      ...
      struct st30_frame* f = nullptr;
      while ((f = ut30p_tx_get_frame(ctx)) == nullptr) {
        if (stop.load(std::memory_order_relaxed)) return;
      }
      int idx = ut30p_tx_frame_idx(f);
      if (holder[idx].exchange(id) != 0) ownership_violation.store(true);
      dwell();
      if (holder[idx].exchange(0) != id) ownership_violation.store(true);
      if (ut30p_tx_put_frame(ctx, f) != 0) api_error.store(true);
  ...
  EXPECT_EQ(ut30p_tx_stat_frames_sent(ctx), (uint64_t)kTarget);
  EXPECT_TRUE(ut30p_tx_all_free(ctx)) << "framebuffer leaked (not back to FREE)";
...
TEST(St30PipelineConcurrency, RxSingleProducerMultiConsumerNoDeadlock) {
  ...
  ut30p_ctx* ctx = ut30p_ctx_create(kFrameCnt);
  ASSERT_NE(ctx, nullptr);
  ...
  auto producer = [&]() {
    ...
    while (produced.load(std::memory_order_relaxed) < kTarget) {
      if (stop.load(std::memory_order_relaxed)) break;
      if (ut30p_inject_frame(ctx, ST_FRAME_STATUS_COMPLETE, ts++) == 0)
        produced.fetch_add(1, std::memory_order_relaxed);
  ...
  auto consumer = [&](int id) {
    while (consumed.load(std::memory_order_relaxed) < kTarget) {
      if (stop.load(std::memory_order_relaxed)) break;
      struct st30_frame* f = ut30p_get_frame(ctx);
      if (!f) continue; /* nothing READY yet */
      int idx = ut30p_frame_idx(f);
      if (holder[idx].exchange(id) != 0) ownership_violation.store(true);
      dwell();
      if (holder[idx].exchange(0) != id) ownership_violation.store(true);
      if (ut30p_put_frame(ctx, f) != 0) api_error.store(true);
      consumed.fetch_add(1, std::memory_order_relaxed);
    }
  };
  ...
  EXPECT_EQ(produced.load(), kTarget);
  EXPECT_EQ(consumed.load(), kTarget) << "frames lost or duplicated";
  EXPECT_TRUE(ut30p_framebuff_cnt(ctx) == kFrameCnt);
...
/* tx_st30p_next_frame scans for a READY frame and writes IN_TRANSMITTING with a
...
TEST(St30PipelineConcurrency, TxConcurrentTransportNoDoubleClaim) {
  ASSERT_EQ(ut30p_tx_init(), 0) << "EAL init failed";
  ...
  /* holder[idx]: 0 = free/ready, >0 = owning transport id. */
  std::vector<std::atomic<int>> holder(kFrameCnt);
  for (auto& h : holder) h.store(0);
  ...
  /* Refiller: cycles FREE slots back to READY so transports always have work. */
  auto refiller = [&]() {
    while (!stop.load(std::memory_order_relaxed)) {
      for (int i = 0; i < kFrameCnt; i++) {
        if (ut30p_tx_frame_stat(ctx, i) == 0 /* FREE */) ut30p_tx_set_frame_ready(ctx, i);
      }
    }
  };
  ...
  auto transport = [&](int id) {
    while (claimed.load(std::memory_order_relaxed) < kTarget) {
      if (stop.load(std::memory_order_relaxed)) break;
      uint16_t idx = 0;
      if (ut30p_tx_next_frame(ctx, &idx) != 0) continue;
      if (holder[idx].exchange(id) != 0) ownership_violation.store(true);
      dwell();
      if (holder[idx].exchange(0) != id) ownership_violation.store(true);
      if (ut30p_tx_frame_done(ctx, idx) != 0) api_error.store(true);
  ...
  EXPECT_FALSE(ownership_violation.load())
      << "two transport threads held the same framebuffer simultaneously";
