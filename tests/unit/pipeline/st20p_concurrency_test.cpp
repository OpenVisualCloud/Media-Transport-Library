...
/* ---- TX: two-phase manual external-frame release lifecycle ---- */

/* Exercises the ST20P_TX_FLAG_EXT_FRAME_MANUAL_RELEASE path, the one TX
 * lifecycle the other cases skip: frame_done parks the slot in IN_USER instead
 * of returning it to FREE, and a *separate* thread later calls
 * notify_ext_frame_free (IN_USER -> FREE). This adds a second cross-thread
 * hand-off to the ring, so the full walk is:
 *
 *   producer:  FREE -> IN_USER -> CONVERTED         (get_frame / put_frame)
 *   consumer:  CONVERTED -> IN_TRANSMITTING -> IN_USER (next_frame / frame_done)
 *   releaser:  IN_USER -> FREE                       (notify_ext_frame_free)
 *
 * holder[] threads a single ownership token through every stage; any stage
 * observing the wrong predecessor token is a lost-slot / double-claim bug.
 * The consumer->releaser hand-off uses an explicit SPSC queue so each slot is
 * released exactly once (mirrors the app contract: one owner per frame). */
TEST(St20PipelineConcurrency, TxManualReleaseLifecycleNoDoubleClaim) {
  ASSERT_EQ(ut20p_tx_init(), 0) << "EAL init failed";
  ...
  /* holder[idx] ownership token:
   *   0   free/queued in the ring
   *   >0  owning producer id
   *  -1   consumer (transmitting)
   *  -2   parked IN_USER, awaiting manual release */
  std::vector<std::atomic<int>> holder(kFrameCnt);
  for (auto& h : holder) h.store(0);
  ...
  /* consumer -> releaser hand-off of parked (IN_USER) slot indices. */
  std::mutex q_mtx;
  std::queue<uint16_t> parked;
  ...
  auto consumer = [&]() {
    while (transmitted.load(std::memory_order_relaxed) < kTarget) {
      if (stop.load(std::memory_order_relaxed)) break;
      uint16_t idx = 0;
  ...
  auto releaser = [&]() {
    while (released.load(std::memory_order_relaxed) < kTarget) {
      if (stop.load(std::memory_order_relaxed)) break;
      uint16_t idx = 0;
      {
        std::lock_guard<std::mutex> lk(q_mtx);
      ...
      if (holder[idx].exchange(0) != -2) ownership_violation.store(true);
      if (ut20p_tx_notify_ext_frame_free(ctx, idx) != 0) api_error.store(true);
  ...
  if (timed_out) {
    fprintf(stderr,
            "[TX manual-release timeout] produced=%d transmitted=%d released=%d\n",
            produced.load(), transmitted.load(), released.load());
    for (int i = 0; i < kFrameCnt; i++)
      fprintf(stderr, "  buf %d stat=%d holder=%d\n", i, ut20p_tx_frame_stat(ctx, i),
              holder[i].load());
  }

  EXPECT_FALSE(timed_out) << "deadlock/livelock: released " << released.load() << "/"
                          << kTarget;
  EXPECT_FALSE(ownership_violation.load())
      << "two actors held the same framebuffer during the manual-release lifecycle";
  EXPECT_FALSE(api_error.load()) << "put_frame/frame_done/notify_ext_frame_free error";
  EXPECT_EQ(produced.load(), kTarget);
  EXPECT_EQ(transmitted.load(), kTarget) << "frames lost or duplicated on transmit";
  EXPECT_EQ(released.load(), kTarget) << "frames lost or duplicated on release";
  EXPECT_EQ(ut20p_tx_stat_frames_sent(ctx), (uint64_t)kTarget);
  EXPECT_TRUE(ut20p_tx_all_free(ctx)) << "framebuffer leaked (not back to FREE)";
