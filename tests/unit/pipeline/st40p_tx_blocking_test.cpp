 ...
 *
 * ST40p (ancillary) TX pipeline blocking get_frame() wake-loss regression
 * test. See st20p_tx_blocking_test.cpp for the equivalent video-pipeline test
 * and design rationale.
 *
 * Bug background: an app running st40p_tx in blocking mode
 * (ST40P_TX_FLAG_BLOCK_GET) sets a block_timeout,
 * e.g. 1 second, expecting st40p_tx_get_frame() to wait up to that long for a
...
TEST(St40PipelineTxBlocking, BlockedGetFrameWakesPromptlyOnGenuineFree) {
  ...
  struct st40_frame_info* held = ut40p_tx_get_frame(ctx);
  ...
  std::thread producer([&]() {
     ...
     * FREE. tx_st40p_frame_done() is what calls
     * tx_st40p_notify_frame_available(), the only source of a genuine wake --
     * ut40p_tx_put_frame_abort() skips it entirely. */
    release_ret[0] = ut40p_tx_put_frame(ctx, held);
    uint16_t idx;
    release_ret[1] = ut40p_tx_next_frame(ctx, &idx);
    release_ret[2] = ut40p_tx_frame_done(ctx, idx);
  });

  const auto t0 = std::chrono::steady_clock::now();
  struct st40_frame_info* frame = ut40p_tx_get_frame(ctx);
  ...
  ASSERT_EQ(ut40p_tx_put_frame_abort(ctx, frame), 0);
 ...
 */
TEST(St40PipelineTxBlocking, StaleWakeFromEarlierFastPathDoesNotShortCircuitBlock) {
  ASSERT_EQ(ut40p_tx_init(), 0) << "EAL init failed";
  ...
  struct st40_frame_info* held0 = ut40p_tx_get_frame(ctx);
  ...
  ASSERT_EQ(ut40p_tx_put_frame_abort(ctx, held0), 0);
  ASSERT_EQ(ut40p_tx_put_frame_abort(ctx, held1), 0);
  ut40p_tx_ctx_destroy(ctx);
}
 ...
 */
TEST(St40PipelineTxBlocking, DestroyDuringWaitNeverHandsOutAClaimedFrame) {
  ASSERT_EQ(ut40p_tx_init(), 0) << "EAL init failed";
  ...
  for (int rep = 0; rep < kReps; rep++) {
    ...
    std::thread racer([&]() {
      ...
      ut40p_tx_put_frame(ctx, held);
      uint16_t idx;
      ut40p_tx_next_frame(ctx, &idx);
      ut40p_tx_frame_done(ctx, idx);
      ...
      while (ut40p_tx_frame_stat(ctx, 0) != 1 /* ST40P_TX_FRAME_IN_USER */) {
    ...
    if (frame == nullptr) {
      EXPECT_EQ(ut40p_tx_frame_stat(ctx, 0), 0 /* ST40P_TX_FRAME_FREE */)
          << "rep " << rep << ": slot left stuck IN_USER after a NULL return";
    } else {
      EXPECT_EQ(ut40p_tx_frame_stat(ctx, 0), 1 /* ST40P_TX_FRAME_IN_USER */)
          << "rep " << rep
 ...
 */
TEST(St40PipelineTxBlocking, ContendedLosingConsumerHonorsAbsoluteDeadline) {
  ASSERT_EQ(ut40p_tx_init(), 0) << "EAL init failed";
  ...
  std::thread decoy([&, decoy_deadline]() {
    while (!stop.load(std::memory_order_relaxed) &&
      ...
      struct st40_frame_info* f = ut40p_tx_get_frame(ctx);
      if (!f) continue;
      ...
      ut40p_tx_put_frame(ctx, f);
      uint16_t idx;
      if (ut40p_tx_next_frame(ctx, &idx) == 0) ut40p_tx_frame_done(ctx, idx);
    }
  });
  ...
  ASSERT_EQ(ut40p_tx_put_frame_abort(ctx, held), 0);
  ut40p_tx_wake_block(ctx);
  std::this_thread::sleep_for(std::chrono::milliseconds(20));

  const auto t0 = std::chrono::steady_clock::now();
  struct st40_frame_info* measured_frame =
      ut40p_tx_get_frame(ctx); /* loses the claim race repeatedly */
  const auto elapsed = std::chrono::steady_clock::now() - t0;
  ...
  if (measured_frame) ut40p_tx_put_frame_abort(ctx, measured_frame);
  ut40p_tx_ctx_destroy(ctx);
}
