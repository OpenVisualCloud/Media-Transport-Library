...
_Static_assert(
    __builtin_types_compatible_p(typeof(((struct st_rx_anc_frame_slot*)0)->state),
                                 _Atomic enum st_rx_anc_slot_state),
    "frame_slots[].state must be a C11 _Atomic enum: it is read/written from "
    "both the RX tasklet and the app thread (st40_rx_put_framebuff)");
...
static int g_notify_rtp_calls; /* count to verify dispatch suppression */

/* ── PTP time stub ────────────────────────────────────────────────────── */

static uint64_t ut40_ptp_time(struct mtl_main_impl* impl, enum mtl_port port) {
  (void)port;
  impl->ptp_usync += 1000;
...
ut_test_ctx* ut40_ctx_create(int num_port) {
  ...
  for (int i = 0; i < MTL_PORT_MAX; i++) {
    ...
    ctx->impl.inf[i].ptp_get_time_fn = ut40_ptp_time;
  }
...
void ut40_set_frame_level(ut_test_ctx* ctx) {
  ...
  /* Dispatch-only stub; no frame_slots pool needed since it does not touch
   * them. Use ut40_setup_frame_pool() for tests that need the slot pool. */
}
...
uint64_t ut40_captured_timestamp_first_pkt(int i) {
  return (i >= 0 && i < g_captured_count) ? g_captured[i].timestamp_first_pkt : 0;
}
...
void ut40_release_frame(ut_test_ctx* ctx, void* addr) {
  struct st_rx_ancillary_session_handle_impl handle = {
      .type = MT_HANDLE_RX_ANC,
      .impl = &ctx->session,
  };
  st40_rx_put_framebuff(&handle, addr);
}
...
int ut40_feed_multi_anc_pkt(ut_test_ctx* ctx, uint16_t seq, uint32_t ts, int marker,
                            ...
                            uint8_t anc_count) {
  if (!udw_sizes || !anc_count || anc_count > ST40_MAX_META) return -1;

  size_t hdr_offset = sizeof(struct st_rfc3550_hdr) - sizeof(struct st_rfc3550_rtp_hdr);
  size_t payload_len = 0;
  for (uint8_t anc_idx = 0; anc_idx < anc_count; anc_idx++)
    payload_len += ut40_anc_payload_bytes(udw_sizes[anc_idx]);
  size_t total = hdr_offset + sizeof(struct st40_rfc8331_rtp_hdr) + payload_len;
  ...
  rtp->base.tmstamp = htonl(ts);
  rtp->base.marker = marker ? 1 : 0;
  uint32_t chunk = ((uint32_t)anc_count) << 24;
  rtp->swapped_first_hdr_chunk = htonl(chunk);
