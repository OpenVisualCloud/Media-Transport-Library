...
def check_yuv_mount_path():
    if platform.system().lower() == "windows":
        ...
        mount_yuv_cmd = (
            r"net use z: \\10.67.116.200\datadisk\streams intel123 /user:media"
        )
    else:
    ...
    check_prcoess = subprocess.Popen(
        check_yuv_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True
    )
    ...
    if "streams" in str(check_result):
        ...
        mount_prcocess = subprocess.Popen(
            mount_yuv_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True
        )
