...
def parse_args():
    parser = argparse.ArgumentParser(description="Upload a PCAP file to the EBU server.")
    parser.add_argument("--pcap_file_path", type=str, required=True, help="Full path to the PCAP file to upload.")
    parser.add_argument("--ip", type=str, required=True, help="IP address to the EBU LIST service.")
    parser.add_argument("--login", type=str, required=True, help="Login to the EBU LIST service.")
    parser.add_argument("--password", type=str, required=True, help="Password to the EBU LIST service.")
    parser.add_argument("--proxy", type=str, required=False, help="Proxy used to upload to the EBU LIST service.")
...
def upload_pcap(file_path, ip, login, password, proxies):
    ...
    uploader = PcapComplianceClient(pcap_file=file_path, config_path="", proxies=proxies)
    uploader.ebu_ip = ip
    uploader.user = login
