...
class VideoIntegritor:
    def __init__(self, logger: logging.Logger, src_url: str, out_name: str, resolution: str, file_format: str = "yuv422p10le",
                 out_path: str = "/mnt/ramdisk", delete_file: bool = True):
        self.logger = logger
        self.logger.info(
    ...
    def shift_src_chunk_by_first_frame_no(self, out_file):
        ...
        cmd = ["ffmpeg", "-s", self.resolution, "-pix_fmt", self.file_format, "-i", str(out_file), "-vframes", "1", "-update", "1", FIRST_FRAME_NAME, "-y"]
...
class VideoStreamIntegritor(VideoIntegritor):
    ...
    def __init__(self, logger: logging.Logger, src_url: str, out_name: str, resolution: str, file_format: str = "yuv422p10le",
                 out_path: str = "/mnt/ramdisk", segment_duration: int = 3, workers_count=5, delete_file: bool = True):
...
You can change them by modifying the constants at the beginning of the script.""",
    ...
    def add_common_arguments(parser):
        ...
        parser.add_argument('--output_path', type=str, default='/mnt/ramdisk',
                           help='Output path (default: /mnt/ramdisk)')
        parser.add_argument('--delete_file', action='store_true', default=True,
                           help='Delete output files after processing (default: True)')
        parser.add_argument('--no_delete_file', action='store_false', dest='delete_file',
...
It performs frame-by-frame integrity checking using MD5 checksums."""
    ...
    if args.mode == 'stream':
        integrator = VideoStreamIntegritor(logger, args.src, args.out, args.res, args.fmt, args.output_path,
                                         args.segment_duration, args.workers, args.delete_file)
    elif args.mode == 'file':
        integrator = VideoFileIntegritor(logger, args.src, args.out, args.res, args.fmt, args.output_path, args.delete_file)
    else:
        parser.print_help()
        return
