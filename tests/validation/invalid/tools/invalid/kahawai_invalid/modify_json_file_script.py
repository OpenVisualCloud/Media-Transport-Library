...
def init_json_file(json_file_list, json_file_mode, port_dict, ip_dict, yuv):
...
def modify_json_file(
    json_file_list, json_file_mode, modify_mode, parameter_name, value_list
):
    line_index = 0
    ...
    if json_file_mode == "1_json_file":
        ...
        if len(parameter_index_list) == 1:
            ...
            value_dict = {
                parameter_index_list[0]: value_list[0],
                parameter_index_list[1]: value_list[1],
            }
        else:
        ...
        actually_modify_json_file(json_file_name, modify_mode, parameter, value_dict)
    else:
        for json_file_name in json_file_list:
            ...
            if len(parameter_index_list) == 0:
                ...
                if "tx.json" in json_file_name:
                    ...
                    actually_modify_json_file(
                        json_file_name, modify_mode, parameter, value_dict
                    )
                else:
                    value_dict = {
                        0: value_list[0],
                        parameter_index_list[0]: value_list[1],
                    }
                    actually_modify_json_file(
                        json_file_name, modify_mode, parameter, value_dict
                    )
...
def actually_modify_json_file(json_file_name, modify_mode, parameter, value_dict):
    line_index = 0
    next_tag = 0
    ...
    for line in json_content_list:
        ...
        if "parameter" in modify_mode:
            if parameter in line:
                print(line)
                print(
                    "=======================Above name of '%s' parameter was modified to =============="
                    % parameter
                )
                if line_index in value_dict.keys():
                    if '"' in parameter:
                        line = re.sub(
                            parameter, '"' + value_dict[line_index] + '"', line
                        )
                    else:
            ...
            if line_index in value_dict.keys():
                if parameter in line or next_tag == 1:
                    print(line)
                    print(
                        "=================Above value of '%s' parameter was modified to ================"
                        % parameter
                    )
                    if next_tag == 0:
    ...
    with open(json_file_name, "r") as tmp_json_file_handle_r:
        json_content_dict = json.load(
            tmp_json_file_handle_r, object_pairs_hook=OrderedDict
        )
...
if __name__ == "__main__":
    ...
    json_file_list_g = [
        "wrong_valueson-video_format-rx_video_formatrx-tx_video_formatx-2_json_file_tx.json",
        "wrong_valueson-video_format-rx_video_formatrx-tx_video_formatx-2_json_file_rx.json",
    ]
    json_file_mode_g = "2_json_file"
    value_list_g = ["tx", "rx"]
    init_json_file(json_file_list_g, json_file_mode_g, port_dict_g, ip_dict_g)
    modify_json_file(
        json_file_list_g, json_file_mode_g, "parameter", "replicas", value_list_g
    )
