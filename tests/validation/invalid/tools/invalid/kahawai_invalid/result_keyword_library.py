def get_keyword(test_mode, parameter):
    if test_mode == "wrong_parameter_in_json":
        ...
        special_dict = dict(
            ...
            packing="video_result",
            payload_type="video_result",
            start_port="invalid start port",
            interfaces_ip="invalid ip",
        )
        ...
        special_dict = dict(
            packing="invalid video packing mode",
            start_port="invalid start port",
            pacing="invalid video pacing",
            interfaces_name="get ip fail",
            interfaces_ip="invalid ip",
            type="invalid video type",
            tr_offset="invalid video tr_offset",
            sessions_ip="invalid ip",
            pg_format="invalid pixel group format",
            payload_type="invalid payload",
            video_format="invalid video format",
            video_url_nonexistent="open fail",
            video_url_txt_file="file size small then a frame",
        )
