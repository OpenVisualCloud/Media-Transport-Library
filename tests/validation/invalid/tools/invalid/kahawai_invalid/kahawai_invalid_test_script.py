...
ip_dict = dict(tx_interfaces='192.168.17.101',
...
if test_mode == 'wrong_value_in_json' and parameter == 'video_url':
    yuv_dict = dict(tx_i720p59_yuv='ParkJoy_1280x720_10bit_50Hz_P422_To_yuv422p10be_To_yuv422YCBCR10be.yuv',
                    tx_1080p_h264='H264_1920x1080_walking_couple_25Hz_main_IBBP_f200_29M.h264',
                    tx_txt_file='test.txt', tx_nonexistent_yuv='nonexistent.yuv')
    yuv = yuv_path + slash + yuv_dict[value_list[0]]


def gather_result_log():
    if json_file_mode == "1_json_file":
        os.rename(per_case_log_path + slash + log_file_name, per_case_log_path + slash + final_log_name)
    else:
...
def prepare_json_test_command():
    ...
    if json_file_mode == '1_json_file':
        ...
        test_command = (cmd_prefix  + '--config_file ' + per_case_log_path + slash
                        + json_file_name + ' --test_time 60 > ' + per_case_log_path + slash + log_file_name + ' 2>&1')
        ...
        tx_test_command = (cmd_prefix + '--config_file ' + per_case_log_path + slash
                           + tx_json_file_name + ' --test_time 60 > ' + per_case_log_path + slash + tx_log_file_name
                           + ' 2>&1')
        rx_test_command = (cmd_prefix + '--config_file ' + per_case_log_path + slash
                           + rx_json_file_name + ' --test_time 60 > ' + per_case_log_path + slash + rx_log_file_name
                           + ' 2>&1')
...
def prepare_commandline_test_command():
    ...
    tmp_test_command = ""
    shutil.move(json_file_name, per_case_log_path)
    init_command = (cmd_prefix + '--config_file ' + per_case_log_path + slash +
                    json_file_name + ' --test_time 60')
    if 'parameter' in test_mode:
        ...
        if '--' + parameter in init_command:
            test_command = (init_command.replace('--' + parameter, '--' + value_list[0]) + ' > ' + per_case_log_path +
                            slash + log_file_name + ' 2>&1')
            test_command_list.append(test_command)
        else:
            if len(value_list) == 1:
                test_command = (init_command + ' --' + value_list[0] + ' > ' + per_case_log_path + slash +
                                log_file_name + ' 2>&1')
            else:
                test_command = (init_command + ' --' + value_list[1] + ' ' + value_list[0] + ' > ' +
                                per_case_log_path + slash + log_file_name + ' 2>&1')
            test_command_list.append(test_command)
    else:
        ...
        test_command = (init_command.split('--')[0] + tmp_test_command + ' > ' + per_case_log_path + slash +
                        log_file_name + ' 2>&1')
        test_command_list.append(test_command)
    return test_command_list
