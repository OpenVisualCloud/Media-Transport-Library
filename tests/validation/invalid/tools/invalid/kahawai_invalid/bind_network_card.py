...
def bind_card(dpdk_path, build_path):
    ...
    for line in p1.stdout.readlines():
        if "E810" in str(line):
            if "if=" in str(line):
                not_bound_list.append(str(line).encode("utf-8").split(" ")[0])
            else:
                bound_list.append(str(line).encode("utf-8").split(" ")[0])

        elif "ioatdma" in str(line):
            if "drv=vfio-pci unused=ioatdma" in str(line):
                dma_bound_list.append(str(line).encode("utf-8").split(" ")[0])
            else:
                dma_not_bound_list.append(str(line).encode("utf-8").split(" ")[0])
...
if __name__ == '__main__':
    dpdk_path_g = "/home/media/caixuan/workspace/dpdk/usertools/dpdk-devbind.py"
    build_path_g = "/home/media/caixuan/workspace/libraries.media.st2110.kahawai/script/nicctl.sh"
