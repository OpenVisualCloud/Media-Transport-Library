...
def init_ip_address_pools(test_config: dict[Any, Any]) -> None:
    # Support session_id at top level or in metadata, with default of 1
    session_id = test_config.get("session_id") or test_config.get("metadata", {}).get("session_id", 1)
    ip_pools.init(session_id=int(session_id))
