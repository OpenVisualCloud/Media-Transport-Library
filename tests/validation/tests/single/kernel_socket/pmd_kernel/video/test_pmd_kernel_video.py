...
def test_pmd_kernel_video_format(
    ...
    capture_cfg["test_name"] = f"test_pmd_kernel_video_{test_mode}_{video_format}_replicas{replicas}"
    ...
    rxtxapp.execute_test(config=config, build=build, test_time=test_time * 2, host=host, capture_cfg=capture_cfg)
