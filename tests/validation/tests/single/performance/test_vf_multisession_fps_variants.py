...
def test_vf_tx_fps_variants_single_core(
    hosts, build, media, test_time, nic_port_list, fps, num_sessions, use_dsa
) -> None:
    """
    ...
    # Configure DSA if enabled - uses mtl_engine.dsa for detection and NUMA validation
    if use_dsa:
        dsa_config = get_host_dsa_config(host)  # Get from topology config
        dsa_device = setup_host_dsa(host, tx_vf, dsa_config, role="TX")
        if dsa_device is None:
            pytest.skip(f"DSA device not available on host ({host.name})")
    else:
        dsa_device = None
    ...
    rx_process = host.connection.start_process(f"{rx_app.command} > {companion_log} 2>&1", cwd=build, shell=True)
    time.sleep(10)
    ...
    try:
        ...
        display_session_results("TX", dsa_label, num_sessions, fps, fps_details, tx_frame_counts, rx_frame_counts)
...
def test_vf_rx_fps_variants_single_core(
    hosts, build, media, test_time, nic_port_list, fps, num_sessions, use_dsa
) -> None:
    """
    ...
    if use_dsa:
        dsa_config = get_host_dsa_config(host)  # Get from topology config
        dsa_device = setup_host_dsa(host, rx_vf, dsa_config, role="RX")
        if dsa_device is None:
            pytest.skip(f"DSA device not available on host ({host.name})")
    else:
        dsa_device = None
    ...
    tx_process = host.connection.start_process(f"{tx_app.command} > {companion_log} 2>&1", cwd=build, shell=True)
    time.sleep(10)
    ...
    try:
        ...
        # Display session results
        display_session_results("RX", dsa_label, num_sessions, fps, rx_fps_details, tx_frame_counts, rx_frame_counts)
...
def test_vf_tx_fps_variants_multi_core(
    hosts, build, media, test_time, nic_port_list, fps, num_sessions, use_dsa
) -> None:
    """
    ...
    try:
        ...
        logger.log(
            TEST_PASS, f"TX FPS variant test (Multi-Core){dsa_label} passed: {num_sessions} sessions @ {fps} fps"
        )
...
def test_vf_rx_fps_variants_multi_core(
    hosts, build, media, test_time, nic_port_list, fps, num_sessions, use_dsa
) -> None:
    """
    ...
    try:
        ...
        logger.log(
            TEST_PASS, f"RX FPS variant test (Multi-Core){dsa_label} passed: {num_sessions} sessions @ {fps} fps"
        )
