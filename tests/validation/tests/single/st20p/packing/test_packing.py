...
def test_packing(
    ...
    (``GPM_SL`` and ``GPM``) for multiple resolutions to confirm negotiated
    layout and packetization succeed. This aims to catch regressions in
    packing-specific code paths that may not surface when using default
    settings.
