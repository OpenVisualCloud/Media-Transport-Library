...
def test_packing(
    ...
    """
    Exercise unicast ST20P sessions using alternative packet packing modes
    (``GPM_SL`` and ``GPM``) for multiple resolutions to confirm negotiated layout
    and packetization succeed. This aims to catch regressions in packing-specific
    code paths that may not surface when using default settings.
    ...
    :param packing: Packing mode supplied to the ST20P session.
