...
def test_resolutions_refactored(
    ...
    """
    Test different video resolutions using Application class refactored interface
    """
    ...
    capture_cfg["test_name"] = (
        f"test_resolutions_refactored_{media_file_info['filename']}"  # Set a unique pcap file name
    )
    ...
    width = media_file_info.get("width", 0)
    
    if height >= 2160:  # 4K and above
        config_params.update({
            "pacing": "linear",     # Linear pacing for 4K
            "packing": "GPM_SL",    # Single line packing for efficiency
            "tx_no_chain": True,    # Optimize for 4K performance
        })
    elif height >= 1080:  # Full HD
        config_params.update({
            "pacing": "wide",       # Wide pacing for 1080p
            "packing": "GPM",       # Standard GPM packing
            "tx_no_chain": False,   # Keep chain for 1080p
        })
    ...
    # Use adaptive test time based on resolution for accurate measurement
    actual_test_time = test_time
    if height >= 2160:  # 4K content
        actual_test_time = max(test_time, 15)  # Minimum 15 seconds for 4K accuracy
    elif height >= 1080:  # Full HD content  
        actual_test_time = max(test_time, 10)  # Minimum 10 seconds for 1080p accuracy
    else:  # HD 720p and below
        actual_test_time = max(test_time, 8)   # Minimum 8 seconds for HD accuracy
