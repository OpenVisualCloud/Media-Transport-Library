...
def test_integrity(hosts, build, media, nic_port_list, test_time, st20p_file, fps, test_config, prepare_ramdisk):
    ...
    capture_cfg["test_name"] = f"test_integrity_{os.path.splitext(os.path.basename(st20p_file['filename']))[0]}_{fps}" # Set a unique pcap file name
