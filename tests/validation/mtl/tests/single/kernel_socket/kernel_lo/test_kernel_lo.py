...
def test_kernello_mixed_format(
    hosts, build, media, test_time, test_mode, video_format, replicas, test_config, prepare_ramdisk
):
    ...
    capture_cfg["test_name"] = f"test_kernel_lo_{test_mode}_{video_format}_replicas{replicas}"
    ...
    rxtxapp.execute_test(config=config, build=build, test_time=test_time * replicas * 3, host=host, capture_cfg=capture_cfg)
