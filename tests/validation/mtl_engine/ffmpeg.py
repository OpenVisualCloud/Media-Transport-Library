...
class FFmpeg(Application):
    ...
    def execute_test(  # type: ignore[override]
        ...
        FFmpeg tests always run RX and TX on the same host with RX started
        first (DPDK init time), so the dual-host signature is rejected.
        """
        if tx_host is not None or rx_host is not None or rx_app is not None:
            raise NotImplementedError(
                "FFmpeg adapter does not support dual-host execution"
            )
        if not host:
        ...
        try:
            ...
            # All TX paths now stream indefinitely (FFmpeg with
            # ``-stream_loop -1`` or RxTxApp without ``--test_time``); RX is
            # the time-bounded process. Match legacy: wall-clock sleep then
            # tear down. Validation runs against captured RX state.
            time.sleep(test_time)
        except Exception as e:
            logger.error("[FFmpeg] Error during test execution: %s", e)
            raise
        finally:
            logger.info("[FFmpeg] Stopping processes")
            for idx, p in enumerate(tx_procs):
                ffmpeg_app.stop_process(p, f"TX{idx + 1}", timeout=5, host=host)
            if rx_proc is not None:
                ffmpeg_app.stop_process(rx_proc, "RX", timeout=5, host=host)
            time.sleep(1)
            ffmpeg_app.kill_orphaned_processes(host, "ffmpeg")
            ffmpeg_app.kill_orphaned_processes(host, "RxTxApp")
            self._rx_output = ffmpeg_app.capture_stdout(rx_proc, "RX")
            for idx, p in enumerate(tx_procs):
                ffmpeg_app.capture_stdout(p, f"TX{idx + 1}")
