...
static void app_rx_st20p_consume_frame(struct st_app_rx_st20p_session* s,
  ...
  if (s->num_port > 1) {
    ...
    if (s->stat_frame_total_received % 60 == 0 && frame->pkts_recv[MTL_SESSION_PORT_P] < (frame->pkts_total / 2))
      dbg("%s(%d): P port only receive %u pkts while total pkts is %u\n", __func__, idx,
           frame->pkts_recv[MTL_SESSION_PORT_P], frame->pkts_total);
    if (s->stat_frame_total_received % 60  && frame->pkts_recv[MTL_SESSION_PORT_R] < (frame->pkts_total / 2))
