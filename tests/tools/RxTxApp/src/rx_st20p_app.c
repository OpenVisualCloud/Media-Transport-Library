...
static void app_rx_st20p_consume_frame(struct st_app_rx_st20p_session* s,
  ...
  if (s->num_port > 1) {
    ...
    if (s->stat_frame_total_received % 60 == 0 &&
        buf->video.pkts_recv[MTL_SESSION_PORT_P] < (buf->video.pkts_total / 2))
    ...
    if (s->stat_frame_total_received % 60 &&
        buf->video.pkts_recv[MTL_SESSION_PORT_R] < (buf->video.pkts_total / 2))
...
static int app_rx_st20p_init(struct st_app_context* ctx,
  ...
  if (ctx->rx_video_multi_thread) config.base.flags |= MTL_SESSION_FLAG_USE_MULTI_THREADS;
...
static int app_rx_st20p_result(struct st_app_rx_st20p_session* s) {
  ...
  if (s->session) {
    ...
    if (mtl_session_io_stats_get(s->session, &wire, sizeof(wire)) == 0) {
