...
> **In-process component tests for the Media Transport Library that find real bugs without requiring a NIC, hugepages, root, or PTP hardware.**
...
A `gtest` binary (`ComponentTest`) that links the **real production `.c`** of
an RX session or pipeline and drives it end-to-end with hand-crafted DPDK
mbufs. No NIC, no PTP, no hugepages, no root.

### Why component tests, not pure unit tests

Most bugs in this code don't live in any single function — they live in the
interaction between the packet handler, the slot allocator, the bitmap, the
stats, the redundancy filter, and the per-port wrapper. A unit test that
mocks four of those out can't see the bug.

These tests run **all** of those layers together against a real session
struct, so a regression in any of them shows up as a failing assertion on a
user-visible counter or returned frame. That's the niche: catching
integration defects in RX state machines without the cost, flakiness, and
hardware setup of a full validation run.

For pure-function logic (sequence math, header parsing, format tables) prefer
true unit tests in [`tests/unit/`](../unit/) — they're cheaper and pinpoint
faults better. Reach for a component test when behaviour depends on
accumulated session state.

### How to use it

1. Run the suite to see the contract MTL is expected to satisfy
   (`--gtest_list_tests`).
2. When changing RX session/pipeline code, run the relevant filter
   (`--gtest_filter='St20Rx*'`) — a red test means you broke a contract.
3. When fixing a reported RX bug, add a test here first that reproduces it,
   then fix the lib until the test goes green.

See [Quick start](#quick-start) for the exact commands.
...
meson setup build_component -Denable_component_tests=true

# 2. build
ninja -C build_component

# 3. run
LD_PRELOAD=/lib/x86_64-linux-gnu/libasan.so.6 \
    ./build_component/tests/component/ComponentTest
```
...
```bash
# one suite
./build_component/tests/component/ComponentTest --gtest_filter='St40RxRedundancyTest.*'
...
./build_component/tests/component/ComponentTest --gtest_filter='*ErrPackets*'

# list everything without running
./build_component/tests/component/ComponentTest --gtest_list_tests
```

> **Why `LD_PRELOAD=libasan.so`?** `libmtl.so` is built with AddressSanitizer
> when `enable_component_tests=true`. Preloading the runtime first prevents
...
1. Create `tests/component/<layer>/<suite>_harness.{h,c}` and a matching
   `*_test.cpp`.
2. In the harness `.c`:
...
5. Add the new sources to `component_sources` in
   [`tests/component/meson.build`](meson.build).
...
| New test green but suite total didn't grow | The `.cpp` was not added to `component_sources` | Add to [`tests/component/meson.build`](meson.build) |
