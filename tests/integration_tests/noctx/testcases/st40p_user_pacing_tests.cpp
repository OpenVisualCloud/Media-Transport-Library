...
TEST_F(NoCtxTest, st40p_user_pacing_offset_jitter) {
  ...
  /* Use offsets that cross the half-frame boundary to force snapping to adjacent epochs */
  std::vector<double> jitterMultipliers = {0.6, -0.6, 0.55, -0.55, 0.0, 0.49, -0.49};
...
TEST_F(NoCtxTest, st40p_exact_user_pacing) {
  ...
  /* Use asymmetric offsets that remain monotonic to stress exact pacing */
  std::vector<double> exactOffsets = {0.2, 0.7, -0.1, 0.9, -0.05, 0.33, -0.25, 0.51};
