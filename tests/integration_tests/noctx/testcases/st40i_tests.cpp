...
class NonSplitAncStrategy : public FrameTestStrategy {
 public:
  explicit NonSplitAncStrategy(std::vector<uint16_t> anc_sizes)
      : FrameTestStrategy(nullptr, true, true), anc_sizes_(std::move(anc_sizes)) {
  }
  ...
  void txTestFrameModifier(void* frame, size_t /*frame_size*/) override {
    ...
    ASSERT_LE(offset, info->udw_buffer_size);
    info->udw_buffer_fill = offset;
  }
  ...
  void rxTestFrameModifier(void* frame, size_t /*frame_size*/) override {
    ...
    EXPECT_FALSE(info->seq_discont);
    EXPECT_EQ(info->seq_lost, 0u);
    /* Non-split: all ANC blocks packed into one RTP packet */
    EXPECT_EQ(info->pkts_total, 1u);

    uint32_t offset = 0;
    for (size_t i = 0; i < std::min(info->meta_num, (uint32_t)anc_sizes_.size()); i++) {
      const uint16_t udw_size = anc_sizes_[i];
      const auto& meta = info->meta[i];
      EXPECT_EQ(meta.udw_size, udw_size);
      EXPECT_EQ(meta.udw_offset, offset);
...
/* Non-split mode: TX packs all ANC blocks into one RTP packet.
 * UDW sizes {4,4,4} all have total_bits % 8 == 0, so no floor/ceil divergence.
 * This test isolates the ntohl bug: anc_count must be byte-swapped for parsing. */
TEST_F(NoCtxTest, st40i_nonsplit_multi_anc_parses) {
  initDefaultContext();

  auto bundle = createSt40pHandlerBundle(
      /*createTx=*/true, /*createRx=*/true,
      /*strategyFactory=*/
      [](St40pHandler*) { return new NonSplitAncStrategy({4, 4, 4}); },
      [](St40pHandler* handler) {
        /* No ST40P_TX_FLAG_SPLIT_ANC_BY_PKT — all ANC in one RTP packet */
        handler->sessionsOpsRx.interlaced = false;
        handler->sessionsOpsTx.framebuff_cnt = 4;
        handler->sessionsOpsRx.framebuff_cnt = 4;
      });
...
/* Non-split mode with UDW sizes that trigger the floor-division bug.
 * UDW=6 → total_bits=100, 100%8=4≠0 → TX ceil(100/8)=13 vs RX floor(100/8)=12.
 * After 4-byte alignment: TX=16 vs RX=12 → 4-byte offset drift at ANC 2.
 * This reproduces "UDW parity failure packet 2" from issue #1506. */
TEST_F(NoCtxTest, st40i_nonsplit_multi_anc_roundtrip) {
  initDefaultContext();

  auto bundle = createSt40pHandlerBundle(
      /*createTx=*/true, /*createRx=*/true,
      /*strategyFactory=*/
      [](St40pHandler*) { return new NonSplitAncStrategy({8, 6, 4}); },
      [](St40pHandler* handler) {
        /* No ST40P_TX_FLAG_SPLIT_ANC_BY_PKT — all ANC in one RTP packet */
        handler->sessionsOpsRx.interlaced = false;
        handler->sessionsOpsTx.framebuff_cnt = 4;
