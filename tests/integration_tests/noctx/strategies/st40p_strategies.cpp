...
void St40pUserTimestamp::verifyReceiveTiming(uint64_t frame_idx, uint64_t receive_time_ns,
  ...
  const int64_t tolerance_ns = 80 * NS_PER_US;
...
void St40pExactUserPacing::txTestFrameModifier(void* frame, size_t /*frame_size*/) {
  auto* info = static_cast<st40_frame_info*>(frame);
  info->tfmt = ST10_TIMESTAMP_FMT_MEDIA_CLK;
  const uint64_t planned_ns = plannedTimestampNs(idx_tx);
  info->timestamp = st10_tai_to_media_clk(planned_ns, VIDEO_CLOCK_HZ);
  idx_tx++;
}
...
void St40pExactUserPacing::verifyReceiveTiming(uint64_t frame_idx,
                                               uint64_t receive_time_ns,
                                               uint64_t expected_transmit_time_ns) const {
  const int64_t delta_ns = static_cast<int64_t>(receive_time_ns) -
                           static_cast<int64_t>(expected_transmit_time_ns);
  /* allow modest pipeline latency while ensuring we still launch near the requested ts */
  const int64_t tolerance_ns = 80 * NS_PER_US;

  EXPECT_GE(delta_ns, 0) << "st40p_exact_user_pacing frame " << frame_idx
