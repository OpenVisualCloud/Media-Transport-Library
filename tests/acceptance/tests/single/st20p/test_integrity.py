...
def test_integrity(
    ...
    # Record onto the media ramdisk, where the other st20p tests stage their
    # media: root-disk writeback can stall the writer long enough to run the
    # RX out of framebuffers and drop a frame.
    _, ramdisk = media_file
    out_file_url = output_files.register(os.path.join(ramdisk, "out.yuv"))
