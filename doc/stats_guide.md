...
| RX log: `Warn: RX_{VIDEO,AUDIO,ANC}_SESSION(...): back-pressure: ... dropped N ... [(sustained Kx)]` | video: `stat_pkts_pool_empty` delta; audio: `stat_slot_get_frame_fail` delta; anc: `stat_pkts_enqueue_fail` delta | Unified periodic back-pressure summary. Video/audio gauge is framebuf pool free count (raise `framebuff_cnt` / drain via `st{20,30}_rx_put_framebuff`); anc gauge is `rte_ring` usage (raise `rtp_ring_size`). `(sustained Kx)` suffix after 3+ consecutive starved intervals |
