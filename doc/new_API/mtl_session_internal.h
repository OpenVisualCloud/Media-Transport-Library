 ...
 * Architecture Note:
 * -----------------
 * This new unified API REPLACES the pipeline API (st_pipeline_api.h).
 * It directly uses the low-level session structures:
 *   - st_tx_video_session_impl / st_rx_video_session_impl (from st_header.h)
 *   - st_frame_trans (the actual frame buffer structure)
 * 
 * The pipeline layer (st20p_*, st_frame, etc.) will be REMOVED.
 */

#ifndef _MTL_SESSION_INTERNAL_H_
#define _MTL_SESSION_INTERNAL_H_

#include "mtl_session_api_improved.h"

/* Internal MTL headers - these stay */
#include "../mt_main.h"
#include "st_header.h"
...
typedef struct mtl_session_vtable {
    ...
    /* Online updates */
    int (*update_destination)(struct mtl_session_impl* s, const struct st_tx_dest_info* dst);
...
struct mtl_buffer_impl {
     ...
     * This is NOT st_frame from pipeline - that's going away.
     */
 ...
 * The pipeline wrapper layer (st20p_tx_handle, etc.) is being REMOVED.
 *************************************************************************/
...
struct mtl_session_impl {
     ...
     * 
     * NOT the pipeline handles (st20p_tx_handle) - those are being removed.
     */
    union {
    ...
    void (*notify_buffer_ready)(void* priv);
    void* notify_priv;
...
#define MTL_SESSION_VALID(s) \
    ((s) && ((s)->magic == MTL_SESSION_MAGIC_VIDEO_TX || \
             ...
             (s)->magic == MTL_SESSION_MAGIC_ANC_TX || \
             (s)->magic == MTL_SESSION_MAGIC_ANC_RX))

/** Get implementation from public handle */
#define MTL_SESSION_IMPL(pub) \
    ((struct mtl_session_impl*)(pub))

/** Get public handle from implementation */
#define MTL_SESSION_PUB(impl) \
    ((mtl_session_t*)(impl))

/** Get buffer implementation from public handle */
#define MTL_BUFFER_IMPL(pub) \
    ((struct mtl_buffer_impl*)((char*)(pub) - offsetof(struct mtl_buffer_impl, pub)))
...
int mtl_video_tx_session_init(struct mtl_session_impl* s,
                               struct mtl_main_impl* impl,
                               const mtl_video_config_t* config);

/** Create video RX session */
int mtl_video_rx_session_init(struct mtl_session_impl* s,
                               struct mtl_main_impl* impl,
                               const mtl_video_config_t* config);
...
/*************************************************************************
 * Internal Functions - Audio Session
 *************************************************************************/

/** Create audio TX session */
int mtl_audio_tx_session_init(struct mtl_session_impl* s,
                               struct mtl_main_impl* impl,
                               const mtl_audio_config_t* config);

/** Create audio RX session */
int mtl_audio_rx_session_init(struct mtl_session_impl* s,
                               struct mtl_main_impl* impl,
                               const mtl_audio_config_t* config);

/** Cleanup audio TX session */
void mtl_audio_tx_session_uinit(struct mtl_session_impl* s);

/** Cleanup audio RX session */
void mtl_audio_rx_session_uinit(struct mtl_session_impl* s);
...
/*************************************************************************
 * Internal Functions - Ancillary Session
 *************************************************************************/

/** Create ancillary TX session */
int mtl_ancillary_tx_session_init(struct mtl_session_impl* s,
                                   struct mtl_main_impl* impl,
                                   const mtl_ancillary_config_t* config);

/** Create ancillary RX session */
int mtl_ancillary_rx_session_init(struct mtl_session_impl* s,
                                   struct mtl_main_impl* impl,
                                   const mtl_ancillary_config_t* config);

/** Cleanup ancillary TX session */
void mtl_ancillary_tx_session_uinit(struct mtl_session_impl* s);

/** Cleanup ancillary RX session */
void mtl_ancillary_rx_session_uinit(struct mtl_session_impl* s);
...
#endif /* _MTL_SESSION_INTERNAL_H_ */
