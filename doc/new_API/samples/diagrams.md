...
Visual representation of the session API patterns. See [README.md](README.md) for
code samples and [../CURRENT_STATE.md](../CURRENT_STATE.md) for the feature
matrix. The real samples referenced below live in
[app/sample/new_api/](../../../app/sample/new_api/).

> Diagrams marked **(NOT YET IMPLEMENTED)** describe the *intended* API for
> features that are not present in the current build (slice mode, ST22). They
> are kept as design references.
...
sequenceDiagram
    ...
    Note over App,Network: Library-Owned Receiver (rx_video_lib_owned_sample.c)
...
sequenceDiagram
    ...
    Note over App,Network: Library-Owned Transmitter (tx_video_lib_owned_sample.c)
...
sequenceDiagram
    ...
    Note over App,Network: User-Owned Receiver (rx_video_user_owned_sample.c)
...
sequenceDiagram
    ...
    Note over Producer,Network: User-Owned Transmitter (tx_video_user_owned_sample.c)
...
## Slice-Level TX Flow (Ultra-Low Latency) — NOT YET IMPLEMENTED

> `slice_ready`/`slice_query` currently return `-ENOTSUP`. Design reference only.
...
sequenceDiagram
    ...
    Note over App,Network: Slice Mode TX (design reference)
...
## Slice-Level RX Flow (Ultra-Low Latency) — NOT YET IMPLEMENTED

> `MTL_EVENT_SLICE_READY` is defined but not emitted. Design reference only.
...
sequenceDiagram
    ...
    Note over Network,App: Slice Mode RX (design reference)
...
## ST22 Plugin Flow — NOT YET IMPLEMENTED

> ST22 compressed-video plugin wiring is not present. Design reference only.
...
sequenceDiagram
    ...
    Note over App,Network: ST22 TX (design reference)
...
sequenceDiagram
    ...
    Note over Signal,Session: Graceful Shutdown (see GRACEFUL_SHUTDOWN.md)
