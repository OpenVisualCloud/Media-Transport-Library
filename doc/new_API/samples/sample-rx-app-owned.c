...
int main(void) {
    ...
    uint8_t* registered_memory = malloc(NUM_BUFFERS * BUFFER_SIZE);
    if (!registered_memory) {
        printf("Failed to allocate memory\n");
        return -1;
    }
    mtl_dma_mem_handle* dma_mem = NULL;
    if (media_lib_mem_register(session, registered_memory, NUM_BUFFERS * BUFFER_SIZE, &dma_mem) != MEDIA_LIB_SUCCESS) {
    ...
    for (int i = 0; i < NUM_BUFFERS; i++) {
        ...
        if (media_lib_post_rx(session, buffers[i]->data, buffers[i]->size, (void*)buffers[i]) != MEDIA_LIB_SUCCESS) {
    ...
    while (1) {
        if (media_lib_poll(session, &event, TIMEOUT_MS) == MEDIA_LIB_SUCCESS) {
            if (event.type == MEDIA_LIB_EVENT_BUFFER_RECEIVED) {
                // In app-owned mode, event.buffer is the application context.
                app_buffer_t* buf = (app_buffer_t*)event.buffer;
                printf("Received buffer id %d, size %zu\n", buf->id, buf->size);
                // Process the data in buf->data as needed...
                
                // After processing, repost the buffer back to the library.
                if (media_lib_post_rx(session, buf->data, buf->size, (void*)buf) != MEDIA_LIB_SUCCESS) {
