...
static volatile int g_frames_sent = 0;
static volatile int g_frames_completed = 0;
...
void* producer_thread(void* arg) {
    ...
    while (g_frames_sent < MAX_FRAMES) {
        app_buffer_t* buf = &g_buffers[next_buf];
        
        /* Wait for buffer to be free */
        while (buf->in_use && g_frames_sent < MAX_FRAMES) {
            usleep(1000);
        }
        if (g_frames_sent >= MAX_FRAMES) break;
...
void* event_thread(void* arg) {
    ...
    while (g_frames_completed < MAX_FRAMES) {
...
int main(void) {
    ...
    printf("Transmitting %d frames (zero-copy mode)...\n", MAX_FRAMES);
...
cleanup_session:
    ...
    return 0;
        ...
        /* Post buffer for transmission */
        buf->in_use = 1;
        err = mtl_session_buffer_post(ctx->session, buf->data, buf->size, buf);
        if (err < 0) {
            printf("buffer_post failed: %d\n", err);
            buf->in_use = 0;
    ...
    printf("Producer thread exiting cleanly\n");
    return NULL;
}
...
void* event_thread(void* arg) {
    ...
    while (ctx->running) {
        err = mtl_session_event_poll(ctx->session, &event, 100);
        if (err == -EAGAIN) {
    ...
    printf("Event thread exiting cleanly\n");
    return NULL;
}
...
int main(void) {
    ...
    /* Start worker threads */
    pthread_create(&prod_tid, NULL, producer_thread, &g_ctx);
    pthread_create(&event_tid, NULL, event_thread, &g_ctx);

    /* Wait for shutdown signal */
    printf("Transmitting frames... Press Ctrl+C to stop.\n");
    while (g_ctx.running) {
        usleep(100000);
    }
    
    /* Graceful shutdown:
     * 1. g_ctx.running = 0 (set by signal handler)
     * 2. mtl_session_stop() (called by signal handler)
     * 3. pthread_join() - workers see -EAGAIN, exit
