...
void* frame_generator_thread(void* arg) {
    while (1) {
        ...
        app_buffer_t* buf = dequeue(&free_queue);
        if (!buf)
            continue;
        ...
        if (media_lib_post_tx(session, buf->data, buf->size, (void*)buf) != MEDIA_LIB_SUCCESS) {
...
int main(void) {
    ...
    if (media_lib_video_session_create(instance, &tx_config, &session) != MEDIA_LIB_SUCCESS) {
    ...
    for (int i = 0; i < NUM_BUFFERS; i++) {
        app_buffer_t* buf = malloc(sizeof(app_buffer_t));
        if (!buf) {
