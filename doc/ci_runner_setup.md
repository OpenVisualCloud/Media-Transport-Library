...
already the cached one). The local harness deliberately does not replace a module
in a developer's running kernel, so it asks instead —
`.github/ci-local/jobs/validate-host.sh` runs `activate-ice.sh --check` and fails
the run with the command to fix it, rather than letting the suite crash.
