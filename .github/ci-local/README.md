...
# run the smoke suite on this machine, as the i225 leg of the matrix
.github/ci-local/run-job.sh smoke-tests --nic i225
...
`test-pr-locally.sh` stops before them: cache restore, JPEG XS and ICE artifact
validation, and library and plugin resolution run here. ICE activation does not:
it replaces a module in the running kernel, which a local run has no business
doing to a developer's machine. To run a test job itself, run it on the machine
that owns the card — see below.

## Hardware jobs: `--runner host`

A container cannot bind a PCI device, reserve hugepages or move ST 2110 packets,
so simulating `smoke-tests` or `gtest` in one proves nothing. On the machine that
owns the NIC those jobs are not a simulation: `--runner host` (the default for
both) runs the same `jobs/<job>.sh` on this host, with `run-job.sh` playing only
GitHub's part.

```sh
.github/ci-local/run-job.sh smoke-tests --nic i225   # the 2.5 GbE, low-bandwidth leg
.github/ci-local/run-job.sh gtest --nic e810
NIGHTLY=1 .github/ci-local/run-job.sh gtest --nic e810   # the nightly-gtest leg
```

Host mode differs from container mode in three ways:

- caches are validated where they are, never restored, deleted or saved: the
  tree in `.local_install` is what the restore would have produced, and
  discarding it to model a miss would break the run for local reasons CI does
  not have;
- the NIC label is resolved against the cards actually present, which also
  decides the datapath — `VF` on an SR-IOV card, `PF` on a two-port card without
  VFs, `KERNEL` (MTL's kernel socket) on a single-port one;
- the matrix leg is read out of `.github/workflows/smoke-tests.yml` rather than
  copied here, so the suite, the traffic duration and whether the leg captures
  cannot drift from the workflow.

Two host prerequisites are not the job's to fix, and each fails with the command
that closes it: the acceptance virtualenv (`task ci:pytest-setup -- install`) and
the ST 2110 media assets (`task ci:media-assets -- list`, then `-- generate` for
a host with no lab share). `doc/ci_runner_setup.md` is the full contract.
...
1. **A cached DPDK loads no drivers on a host it was not built on.** meson bakes
   `RTE_EAL_PMD_PATH` — `<prefix>/lib/<arch>/dpdk/pmds-<abi>` — into
   `librte_eal`, and EAL loads every driver from that one absolute path. MTL
   builds a fixed EAL argv, so there is no `-d` to override it with. Build the
   cache in the container (prefix under `/github/workspace`), run the tests on
   the host, and nothing registers: `mtl_init` fails at its first mempool,
   `mt_mempool_create_by_ops, fail(Invalid argument) for T_P0_SYS`, because even
   the mempool ops MTL asks for are plugins. The same applies in CI whenever the
   build runner's workspace path differs from the test runner's.
   `task ci:configure-host -- dpdk-plugins` points the baked path at the
   restored tree, and `validate-host` runs it after the restore.
