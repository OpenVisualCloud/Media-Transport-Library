...
def main():
    ...
    # The outer cap governs one state only -- queued -- because the action bounds
    # the in_progress and the never-created cases on its own budget. It therefore
    # has to be far larger than that budget: the queue in front of the fleet's
    # single build host is routinely longer than the build, and a cap near it
    # fails the gate on commits whose build has not started yet.
    if wait_job_seconds < 3600 or wait_job_seconds <= wait_seconds:
        errors.append(
            ".github/workflows/pr-gate.yml: wait-for-build needs an outer cap above"
            " an hour, to sit out the queue in front of the build host"
        )
