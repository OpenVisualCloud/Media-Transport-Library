...
- **Source media from `tests/acceptance/mtl_engine/media_files.py`** (`yuv_files`, `yuv_files_422rfc10`, `audio_files`, `anc_files`, …) via the `media_file` fixture. Never hard-code an NFS or developer-home path; mount points are per-host in `topology_config.yaml` / `test_config.yaml`.
- **Do not edit the top-level `tests/acceptance/conftest.py`, `common/`, or `mtl_engine/`** — those are framework. A **per-category `conftest.py` next to your tests is fine** (examples: `tests/single/xdp/conftest.py`, `tests/dual/performance/conftest.py`).
