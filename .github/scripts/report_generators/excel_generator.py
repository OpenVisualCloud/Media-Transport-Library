...
def _create_pytest_sheet(wb, pytest_data):
    ...
    _add_section_title(
        ws, "DETAILED TEST CASES", f"A{detail_title_row}:F{detail_title_row}"
    )
    ws.append([])  # Blank row
    ...
    for data in sorted_pytest:
        if "test_cases" in data and data["test_cases"]:
            _add_category_separator(
                ws, f"Category: {data.get('nic', '')} - {data.get('category', '')}", 6
            )
...
def _create_summary_sheet(wb, pytest_data, gtest_data, system_info_list, test_metadata):
    ...
    if pytest_data or gtest_data:
        summary_data.extend(
            _build_test_summary("Overall Summary (Pytest + GTest)", stats["combined"])
        )
...
def _add_category_separator(ws, text, col_count):
    ...
    for col in range(1, col_count + 1):
        ...
        cell.fill = PatternFill(
            start_color=COLOR_SEPARATOR, end_color=COLOR_SEPARATOR, fill_type="solid"
        )
...
def _color_code_result(ws, row_num, col_num, result):
    ...
    if "PASS" in result_upper and "FAIL" not in result_upper:
        cell.fill = PatternFill(
            start_color=COLOR_PASSED, end_color=COLOR_PASSED, fill_type="solid"
        )
    elif "FAIL" in result_upper or "ERROR" in result_upper:
        cell.fill = PatternFill(
            start_color=COLOR_FAILED, end_color=COLOR_FAILED, fill_type="solid"
        )
    elif "SKIP" in result_upper:
        cell.fill = PatternFill(
            start_color=COLOR_SKIPPED, end_color=COLOR_SKIPPED, fill_type="solid"
        )
