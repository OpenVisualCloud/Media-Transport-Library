...
def generate_html_report(pytest_data, gtest_data, output_file, system_info_list=None, test_metadata=None):
    ...
    system_info_section = _generate_system_info(system_info_list) if system_info_list else ""
...
def _calculate_statistics(pytest_data, gtest_data):
    ...
    return {
        'pytest': {'total': pytest_total, 'passed': pytest_passed, 'failed': pytest_failed, 'skipped': pytest_skipped, 'pass_rate': pytest_pass_rate},
        'gtest': {'total': gtest_total, 'passed': gtest_passed, 'failed': gtest_failed, 'skipped': gtest_skipped, 'pass_rate': gtest_pass_rate},
        'combined': {'total': combined_total, 'passed': combined_passed, 'failed': combined_failed, 'skipped': combined_skipped, 'pass_rate': combined_pass_rate}
    }
...
def _generate_test_run_info(test_metadata):
    ...
    html += '<tr><th>Test Suite</th><th>Run</th><th>Branch</th><th>Date & Time</th></tr>'
    ...
    if test_metadata.get('pytest_run_number') and test_metadata.get('pytest_branch'):
        ...
        run_link = f'<a href="{test_metadata["pytest_run_url"]}" target="_blank">#{test_metadata["pytest_run_number"]}</a>' if test_metadata.get('pytest_run_url') else f'#{test_metadata["pytest_run_number"]}'
        html += f'<tr><td>Pytest</td><td>{run_link}</td><td>{test_metadata["pytest_branch"]}</td><td>{run_date} {run_time}</td></tr>'
    ...
    if test_metadata.get('gtest_run_number') and test_metadata.get('gtest_branch'):
        ...
        run_link = f'<a href="{test_metadata["gtest_run_url"]}" target="_blank">#{test_metadata["gtest_run_number"]}</a>' if test_metadata.get('gtest_run_url') else f'#{test_metadata["gtest_run_number"]}'
        html += f'<tr><td>GTest</td><td>{run_link}</td><td>{test_metadata["gtest_branch"]}</td><td>{run_date} {run_time}</td></tr>'
...
def _generate_pytest_table(pytest_data):
    ...
    html = '<table><thead><tr><th>NIC</th><th>Category</th><th>Passed</th><th>Failed</th><th>Skipped</th><th>Error</th><th>XPassed</th><th>XFailed</th><th>Total</th></tr></thead><tbody>'
    ...
    for idx, data in enumerate(sorted(pytest_data, key=lambda x: (x['nic'], x['category']))):
        ...
        if 'test_cases' in data and data['test_cases']:
            ...
            html += f'<h4>Detailed Test Cases for {data["category"]}</h4><table><thead><tr><th>Test Name</th><th>Result</th><th>Duration</th></tr></thead><tbody>'
            ...
            for tc in sorted(data['test_cases'], key=lambda x: x['test_name']):
                ...
                html += f'<tr><td>{tc["test_name"]}</td><td class="{result_class}">{tc["result"]}</td><td>{tc["duration"]}</td></tr>'
...
def _generate_gtest_table(gtest_data):
    ...
    html = '<table><thead><tr><th>NIC</th><th>Test Category</th><th>Passed</th><th>Failed</th><th>Skipped</th><th>Total</th></tr></thead><tbody>'
...
def _generate_system_info(system_info_list):
    ...
    html = '<div class="summary"><h2>Test Environment Information</h2><table><thead><tr>'
    html += '<th>Hostname</th><th>Platform</th><th>CPU</th><th>Cores</th><th>RAM</th><th>HugePages</th><th>OS</th><th>Kernel</th><th>NICs</th>'
    html += '</tr></thead><tbody>'
