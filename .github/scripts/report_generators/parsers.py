...
def parse_gtest_log(log_file):
    ...
    # If still no results, try summary format
    if not results:
...
def parse_system_info(info_file):
    ...
    try:
        ...
        nic_list.extend([nic for nic in broadcom_matches if 'NetXtreme' in nic and nic not in nic_list])
        
        info['nics'] = ', '.join(nic_list) if nic_list else 'Unknown'
        
        # Extract platform
        platform_match = re.search(r'Product Name:\s+(.+)', content)
        info['platform'] = platform_match.group(1).strip() if platform_match else 'Unknown'
