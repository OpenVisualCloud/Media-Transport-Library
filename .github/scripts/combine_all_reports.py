...
def parse_pytest_html(html_file):
    ...
    match = re.match(r'nightly-test-report-([^-]+)-(dell|broadcom|mellanox|intel|cisco)-(.+)', filename)
    if match:
...
def parse_system_info(info_file):
    ...
    try:
        ...
        if hugepages_match and hugepagesize_match:
            ...
            info['hugepages'] = f"{total_pages} x {page_size_kb//1024//1024}GB = {total_gb:.0f}GB"
...
def create_excel_report(pytest_data, gtest_data, output_file, system_info_list=None, test_metadata=None):
    ...
    if test_metadata:
        ...
        if test_metadata.get('pytest_run_number') and test_metadata.get('pytest_branch'):
            ...
            summary_data.append(['Pytest Run:', f"#{test_metadata['pytest_run_number']} (branch: {test_metadata['pytest_branch']})"])
        ...
        if test_metadata.get('gtest_run_number') and test_metadata.get('gtest_branch'):
            ...
            summary_data.append(['GTest Run:', f"#{test_metadata['gtest_run_number']} (branch: {test_metadata['gtest_branch']})"])
    ...
    if system_info_list:
        ...
        ws_summary.cell(row=system_info_title_row, column=1).font = Font(bold=True, size=12)
...
def create_html_report(pytest_data, gtest_data, output_file, system_info_list=None, test_metadata=None):
    ...
    if test_metadata:
        ...
        if test_metadata.get('pytest_run_number') and test_metadata.get('pytest_branch'):
            ...
            test_run_info += f'<tr><td>Pytest</td><td>{run_link}</td><td>{test_metadata["pytest_branch"]}</td><td>{run_date} {run_time}</td></tr>'
        ...
        if test_metadata.get('gtest_run_number') and test_metadata.get('gtest_branch'):
            ...
            test_run_info += f'<tr><td>GTest</td><td>{run_link}</td><td>{test_metadata["gtest_branch"]}</td><td>{run_date} {run_time}</td></tr>'
...
def main():
    ...
    parser.add_argument('--system-info-dir', type=Path, help='Directory containing system info reports')
    ...
    parser.add_argument('--pytest-run-number', type=str, help='Pytest workflow run number')
    ...
    parser.add_argument('--gtest-run-number', type=str, help='GTest workflow run number')
    ...
    # Create reports
    create_excel_report(pytest_data, gtest_data, args.output_excel, system_info_list, test_metadata)
    create_html_report(pytest_data, gtest_data, args.output_html, system_info_list, test_metadata)
