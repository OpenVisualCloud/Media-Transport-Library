...
def _valid_nics() -> tuple[str, ...]:
    """The NIC labels the fleet advertises, read from the one table that has
    them: .github/scripts/ci/pytest-setup.sh. A copy here would be a second
    list to forget when a card is added -- which is how i225 and i226 came to
    be rejected by this server months after the workflows had legs for them.
    """
    rc, out = _run_rc(
        ["bash", str(REPO_ROOT / ".github/scripts/ci/pytest-setup.sh"), "nic-labels"],
        timeout=30,
    )
    labels = tuple(line.strip() for line in out.splitlines() if line.strip())
    return labels if rc == 0 and labels else ("e810", "e830", "e835", "e825")


VALID_NICS = _valid_nics()
VALID_RUNNERS = ("runner", "baremetal", "host")
...
mcp = FastMCP(
    ...
    instructions=textwrap.dedent(
        ...
        Common workflows:
        • Just pushed:           ci_watch_run(pr=1676) -- waits, then names the
                                 failed job, step and error line
        ...
        • On real hardware:      ci_run_job("smoke-tests", nic="i225", timeout_s=3600)
                                 -- runs on this machine, not in a container
...
def ci_watch_run(
    ...
    """Wait for the Actions runs of a commit, then say which job, step and error failed.

    This is the tool for "did the change I just pushed work". ci_pr_checks
    answers for a commit whose runs have finished; this one waits for them,
    which on this repository is the normal case: the bare-metal legs run on
    NIC-owning hosts whose availability comes in windows, so a push is followed
    by a queue measured in hours.

    It returns what a fix needs -- the failed job, its failed step and the error
    line -- rather than a log to read. While waiting it distinguishes the two
    kinds of queue that look identical in the Actions UI: a job queued with no
    runner name has not been handed to a host, so it is waiting on fleet
    availability and not on itself.

    Args:
        run: watch exactly this run id. Otherwise every run of the commit.
        pr: watch the head commit of this pull request.
        branch: watch the pushed head of this branch (default: the current one).
        workflow: keep only runs whose name matches, e.g. "smoke-tests".
        job: keep only jobs whose name matches, e.g. "i225".
        timeout_min: give a bare-metal leg room -- the queue in front of the
             build host is routinely longer than the build, and a smoke leg runs
             for half an hour after that.
        interval_s: poll interval.
        repo: optional owner/repo; defaults to the origin remote.

    Read-only: it never pushes, re-runs, cancels or comments.
    """
    script = REPO_ROOT / ".github" / "scripts" / "ci" / "watch-run.sh"
    ...
    # --quiet drops the per-poll state trace: the caller sees the summary, which
    # is the verdict. The trace is for a terminal that is being watched.
    cmd = [
        "bash",
        str(script),
        "--quiet",
        "--timeout",
        str(max(1, timeout_min)),
        "--interval",
        str(max(5, interval_s)),
    ]
    for flag, value in (
        ("--run", str(run) if run > 0 else ""),
        ("--pr", str(pr) if pr > 0 else ""),
        ("--branch", branch),
        ("--workflow", workflow),
        ("--job", job),
        ("--repo", repository),
    ...
    # The script polls until the runs finish, so it may legitimately run for the
    # whole timeout. Give the subprocess a margin over it rather than killing it
    # just before it would have reported.
    rc, out = _run_rc(cmd, timeout=max(1, timeout_min) * 60 + 120)
    verdict = {
        0: "PASS -- every job succeeded",
        1: "FAIL -- a job failed",
        2: f"TIMEOUT -- still outstanding after {timeout_min}m",
        3: "ERROR -- nothing to watch",
    }.get(rc, f"exit {rc}")
    return f"**{verdict}**\n\n```\n{out.strip()}\n```"
...
def ci_run_job(
    ...
    Args:
        ...
        nic: matrix NIC to simulate for bare-metal jobs (see ci_list_jobs for
             the labels). Selects the bare-metal runner image and sets
             PCI_DEVICE; in host mode it is resolved against the cards this
             machine actually has.
        runner: runner kind -- "runner", "baremetal", or "host". Empty lets
             run-job.sh choose, which is what you normally want: the test jobs
             pick host, because a container cannot bind a PCI device, reserve
             hugepages or move ST 2110 packets. Pass "host" explicitly to run a
             job on this machine that would otherwise be simulated.
        ...
        timeout_s: give a cold build room; it compiles DPDK, MTL and FFmpeg.
             A hardware suite needs more: the i225 smoke leg alone runs for
             half an hour.
    ...
    if runner and runner not in VALID_RUNNERS:
        return f"ERROR: unknown runner '{runner}'. Valid: {', '.join(VALID_RUNNERS)}"
...
@mcp.tool()
def ci_check_ebpf() -> str:
    """Check this host's eBPF/XDP prerequisites, naming the package to install
    for each one that is missing.

    Same script as `task ebpf:check` and as the dependency validation every
    hardware job runs, so its verdict is the one CI will reach.
    """
    script = REPO_ROOT / "script" / "build_ebpf_xdp.sh"
    if not script.is_file():
        return f"ERROR: {script} not found."
    rc, out = _run_rc(["bash", str(script), "--check"], timeout=120)
...
def ci_run_task(task: str, args: str = "", timeout_s: int = 1800) -> str:
    ...
    Args:
        ...
        args: extra arguments, e.g. "NIC=e830" or "COMPONENT=dpdk".
