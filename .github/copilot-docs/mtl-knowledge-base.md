...
- The early release also comes off the RTP timestamp: `tv_rl_rtp_shift_ticks()` subtracts `RL_RTP_SHIFT_PKTS` (5) packet intervals as whole 90kHz ticks, floored at one, off the media-clock image of `ptp_time_cursor` (plus `rtp_timestamp_delta_us`), at all three sites: `tv_update_rtp_time_stamp()` and both `ST20_TYPE_RTP_LEVEL` builders. Without it the receiver sees a negative latency every frame.
- Exempt: `warm_pkts == 0` (RL planned no pad train at all), `USER_TIMESTAMP` (the app owns that value) and `RTP_TIMESTAMP_EPOCH` (the bare epoch already sits a whole `tr_offset` before any packet leaves). `EXACT_USER_PACING` is *not* exempt -- the pads release early whoever picked the launch instant.
- `warm_pkts` is the plan, not the outcome: `video_trs_rl_warm_up()` sends `video_trs_rl_pkts_needed()` pads and bails, counting `stat_trans_troffset_mismatch`, when that count falls outside `[0, warm_pkts]`. An already-overdue frame gets no pads yet is still shifted -- by that bail, or silently where the count truncates to 0. Over-shifting is the safe direction, so the shift does not track it.
- The shift is capped at the epoch lead of the instant actually being converted, as tick counts: `st10_tai_to_media_clk(tai_for_rtp_ts)` less `st10_tai_to_media_clk(tai_from_frame_count(cur_epochs))`. Read off `ptp_time_cursor` instead, a negative `rtp_timestamp_delta_us` -- which moves the timestamp but not the schedule -- gets a cap that does not bind.
- The lead is not the `rtp_offset` the receiver reports: `rv_tp_on_packet()` floors the epoch's exact tick value, while TX floors it to whole *nanoseconds* first and rounds that to the nearest tick. `E_tx >= E_rx` always, since `round_closest(v) >= floor(v)` and the ns floor moves `v` under 9e-5 of a tick, nowhere near the half tick that would cross. A lead at or below zero is not shifted.
- The cap is therefore what keeps `rtp_offset >= 0`, with a tick to spare where the epoch's remainder exceeds half a tick. Saturating it costs no `rtp_ts_delta`: the emitted tick is `E_tx + max(lead - cap, 0)`, and `max(x - cap, 0)` is monotone and 1-Lipschitz, so every inter-frame step lands between the unshifted step and the epoch's own -- never past it, never wider.
- Not `tr_offset - vrx * trs` -- that is the ordinary path's lead only. `EXACT_USER_PACING` sets `start_time_tai = required_tai` verbatim and `calc_frame_count_since_epoch()` picks the epoch *nearest* it, so an app launching just after one has little lead. `warm_pkts` bounds nothing either: `height <= 576` forces it to `8` whatever `tr_offset` holds (480x270p30: eight pads, only 23 ticks of lead).
- Subtract in the tick domain, never as ns before the conversion: `1e9/90000` is not an integer, so an ns amount resolves to different tick counts either side of a tick boundary and the receiver sees a wrong `rtp_ts_delta`.
- The *lead* is a tick-domain difference for the same reason, and easy to get wrong: the epoch's own sub-tick position is arbitrary at every frame rate, because `tai_from_frame_count()` truncates `cur_epochs * frame_time` to whole ns. Converting the ns lead once rounds the timestamp a tick *past* the epoch wherever that remainder exceeds half a tick -- 40% of lead-capped frames.
- Below `trs` 1111ns five intervals round to no ticks at all and the one-tick floor answers instead -- 2160p60 is the only rx_timing format there, where a tick buys 11.4 intervals. Deliberate: EBU LIST's `packet_ts_vs_rtp_ts` has min 0, no negative slack at all, and EBU runs *before* the parser in the acceptance suite, so the parser's own tick of latency slack buys nothing there.
- `frame->timestamp` keeps the *uncompensated* launch instant: it reports the schedule the library resolved, while the shift is a wire-protocol correction belonging only to the RTP header value. So it and `frame->rtp_timestamp` stop being each other's `st10_tai_to_media_clk()` image.
