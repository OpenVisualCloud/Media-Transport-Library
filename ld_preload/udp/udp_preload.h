...
struct upl_ctx {
  ...
  /*
   * All rte_malloc memory is shared by processes as it's mapped with shared
   * during DPDK memory init, and MTL always use DPDK memory. The easiest way
   * for multi-process sharing is parent process to create/free the resource,
   * child only access the data path */
  pid_t pid;
