...
static inline struct upl_ufd_entry *upl_get_ufd_entry(struct upl_ctx *ctx,
                                                      int kfd) {
...
static inline struct upl_efd_entry *upl_get_efd_entry(struct upl_ctx *ctx,
                                                      int kfd) {
...
static struct upl_ctx *upl_create_ctx(bool child) {
  ...
  ctx->upl_entires =
      upl_zmalloc(sizeof(*ctx->upl_entires) * ctx->upl_entires_nb);
  if (!ctx->upl_entires) {
...
static int upl_stat_dump(void *priv) {
  ...
  if (entry->stat_epoll_cnt || entry->stat_epoll_revents_cnt) {
    notice("%s(%d), epoll %d revents %d\n", __func__, kfd,
           entry->stat_epoll_cnt, entry->stat_epoll_revents_cnt);
  ...
  if (entry->stat_select_cnt || entry->stat_select_revents_cnt) {
    notice("%s(%d), select %d revents %d\n", __func__, kfd,
           entry->stat_select_cnt, entry->stat_select_revents_cnt);
...
static int upl_efd_epoll_query(void *priv) {
  ...
  if (entry->sigmask)
    ret = LIBC_FN(epoll_pwait, efd, entry->events, entry->maxevents, 0,
                  entry->sigmask);
  else
...
static int upl_select_query(void *priv) {
  ...
  if (select_ctx->sigmask)
    ret = LIBC_FN(pselect, select_ctx->nfds, select_ctx->readfds,
                  select_ctx->writefds, select_ctx->exceptfds, &zero_spec,
                  select_ctx->sigmask);
  else
    ret = LIBC_FN(select, select_ctx->nfds, select_ctx->readfds,
                  select_ctx->writefds, select_ctx->exceptfds, &zero);
  dbg("%s, ret %d\n", __func__, ret);
  return ret;
}
...
static int upl_poll_query(void *priv) {
  ...
  if (poll_ctx->tmo_p || poll_ctx->sigmask) {
    ...
    zero.tv_nsec = 0;
    ret =
        LIBC_FN(ppoll, poll_ctx->fds, poll_ctx->nfds, &zero, poll_ctx->sigmask);
  } else {
...
/* reuse mufd_poll now */
static int upl_efd_epoll_pwait(struct upl_efd_entry *entry,
  ...
  if (kfd_cnt > 0) {
    ...
    ret = mufd_poll_query(p_fds, p_fds_cnt, timeout_ms, upl_efd_epoll_query,
                          entry);
  } else {
...
static int upl_pselect(struct upl_ctx *ctx, int nfds, fd_set *readfds,
  ...
  if (!poll_ufds_cnt) {
    if (sigmask)
      return LIBC_FN(pselect, nfds, readfds, writefds, exceptfds, timeout_spec,
                     sigmask);
    else
  ...
  int ret = mufd_poll_query(poll_ufds, poll_ufds_cnt, timeout_ms,
                            upl_select_query, &priv);
  if (ret < 0)
    return ret;

  FD_ZERO(readfds);
  for (nfds_t i = 0; i < poll_ufds_cnt; i++) {
    if (!poll_ufds[i].revents)
      continue;
...
static int upl_ppoll(struct upl_ctx *ctx, struct pollfd *fds, nfds_t nfds,
                     int timeout, const struct timespec *tmo_p,
                     const sigset_t *sigmask) {
  if (nfds <= 0) {
...
int socket(int domain, int type, int protocol) {
  ...
  if (!ctx)
    return LIBC_FN(socket, domain, type, protocol);
  ...
  if (kfd < 0) {
    err("%s, create kfd fail %d for domain %d type %d protocol %d\n", __func__,
        kfd, domain, type, protocol);
    return kfd;
  }
  if (kfd > ctx->upl_entires_nb) {
    err("%s, kfd %d too big, consider enlarge entires space %d\n", __func__,
        kfd, ctx->upl_entires_nb);
    return kfd;
  }

  ret = mufd_socket_check(domain, type, protocol);
  if (ret < 0)
    return kfd; /* not support by mufd */

  if (ctx->child) {
    err("%s, kfd %d, child not allow to create a ufd, domain %d type %d "
        "protocol %d\n",
...
int bind(int sockfd, const struct sockaddr *addr, socklen_t addrlen) {
  ...
  struct upl_ctx *ctx = upl_get_ctx();
  if (!ctx)
    return LIBC_FN(bind, sockfd, addr, addrlen);
...
ssize_t sendmsg(int sockfd, const struct msghdr *msg, int flags) {
  ...
  uint8_t *ip = (uint8_t *)&addr_in->sin_addr.s_addr;
  dbg("%s(%d), dst ip %u.%u.%u.%u\n", __func__, sockfd, ip[0], ip[1], ip[2],
      ip[3]);
  int ufd = entry->ufd;
...
ssize_t send(int sockfd, const void *buf, size_t len, int flags) {
  struct upl_ctx *ctx = upl_get_ctx();
  if (!ctx)
    return LIBC_FN(send, sockfd, buf, len, flags);
...
int ppoll(struct pollfd *fds, nfds_t nfds, const struct timespec *tmo_p,
  ...
  int timeout =
      (tmo_p == NULL) ? -1 : (tmo_p->tv_sec * 1000 + tmo_p->tv_nsec / 1000000);
...
int select(int nfds, fd_set *readfds, fd_set *writefds, fd_set *exceptfds,
  ...
  return upl_pselect(ctx, nfds, readfds, writefds, exceptfds, timeout, NULL,
                     NULL);
}
...
int pselect(int nfds, fd_set *readfds, fd_set *writefds, fd_set *exceptfds,
  ...
  struct upl_ctx *ctx = upl_get_ctx();
  if (!ctx)
    return LIBC_FN(pselect, nfds, readfds, writefds, exceptfds, timeout,
                   sigmask);

  return upl_pselect(ctx, nfds, readfds, writefds, exceptfds, NULL, timeout,
                     sigmask);
}
...
int epoll_create(int size) {
  struct upl_ctx *ctx = upl_get_ctx();
  if (!ctx)
    return LIBC_FN(epoll_create, size);
...
int epoll_create1(int flags) {
  struct upl_ctx *ctx = upl_get_ctx();
  if (!ctx)
    return LIBC_FN(epoll_create1, flags);
...
int epoll_ctl(int epfd, int op, int fd, struct epoll_event *event) {
  struct upl_ctx *ctx = upl_get_ctx();
  if (!ctx)
    return LIBC_FN(epoll_ctl, epfd, op, fd, event);
