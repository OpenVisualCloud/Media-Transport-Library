...
#endif
    dbg("%s(%d), ip %u.%u.%u.%u\n", __func__, s->idx, ip[0], ip[1], ip[2],
        ip[3]);

    ssize_t send = mufd_sendto(
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < session_num; i++) {
    ...
    if (ctx.udp_tx_bps)
      mufd_set_tx_rate(app[i]->socket, ctx.udp_tx_bps);
    ...
    tv.tv_sec = 0;
    tv.tv_usec = 1000;
    ret = mufd_setsockopt(app[i]->socket, SOL_SOCKET, SO_RCVTIMEO, &tv,
                          sizeof(tv));
    if (ret < 0) {
    ...
    if (mcast) {
      ...
      memcpy(&mreq.imr_interface.s_addr, ctx.param.sip_addr[MTL_PORT_P],
             MTL_IP_ADDR_LEN);
      ret = mufd_setsockopt(app[i]->socket, IPPROTO_IP, IP_ADD_MEMBERSHIP,
                            &mreq, sizeof(mreq));
      if (ret < 0) {
    ...
    if (ctx.udp_mode == SAMPLE_UDP_TRANSPORT) {
      ret = pthread_create(&app[i]->thread, NULL, ufd_server_transport_thread,
                           app[i]);
    } else if (ctx.udp_mode == SAMPLE_UDP_TRANSPORT_POLL) {
      ret = pthread_create(&app[i]->thread, NULL,
                           ufd_server_transport_poll_thread, app[i]);
    } else if (ctx.udp_mode == SAMPLE_UDP_TRANSPORT_UNIFY_POLL) {
      ctxs.apps[i] = app[i];
      if ((i + 1) == session_num) {
        ret =
            pthread_create(&ctxs.thread, NULL, ufd_servers_poll_thread, &ctxs);
      }
    } else {
