...
static int create_display_context(struct st_display *d) {
  ...
  d->window = SDL_CreateWindow(
      title, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, d->window_w,
...
static int
display_thread_create(struct st_v4l2_tx_video_session *tx_video_session,
  ...
  if (video_set_realtime(tx_video_session->display.display_thread, priority,
                         cpu) < 0) {
...
static void
display_consume_frame(struct st_v4l2_tx_video_session *tx_video_session,
                      void *frame) {
...
static int app_init_display(struct st_display *d, int idx, int width,
  ...
  if (video_is_mplane(dev)) {
    ...
    for (i = 0; i < v_fmt.fmt.pix_mp.num_planes; i++) {
      dev->plane_fmt[i].bytesperline =
          v_fmt.fmt.pix_mp.plane_fmt[i].bytesperline;
...
static int video_set_format(struct device *dev, unsigned int w, unsigned int h,
  ...
  if (video_is_mplane(dev)) {
    printf("Video attributes, pixel format: %s (%08x), resolution: %ux%u "
           "field: %s, number "
           "of planes: %u\n",
...
static int video_buffer_mmap(struct device *dev, struct buffer *buffer,
  ...
  for (i = 0; i < dev->num_planes; i++) {
    ...
    buffer->mem[i] =
        mmap(0, length, PROT_READ | PROT_WRITE, MAP_SHARED, dev->fd, offset);
...
static int video_buffer_alloc_userptr(struct device *dev, struct buffer *buffer,
  ...
  for (i = 0; i < dev->num_planes; i++) {
    ...
    ret = posix_memalign(&buffer->mem[i], page_size,
                         length + offset + padding + page_size);
    if (ret < 0) {
    ...
    printf("Buffer %u/%u allocated at address %p length %u page %u.\n",
           buffer->idx, i, buffer->mem[i], length, page_size);
  }

  return 0;
}

static void video_buffer_free_userptr(struct device *dev,
  ...
  for (i = 0; i < dev->num_planes; i++) {
    if (video_is_output(dev)) {
      ...
      if (fill & BUFFER_FILL_PADDING)
        memset(dev->buffers[buf.index].mem[i] + dev->buffers[index].size[i],
  ...
  for (plane = 0; plane < dev->num_planes; plane++) {
    ...
    if (filename != NULL) {
      ...
      if (dev->plane_fmt[plane].bytesperline == 0) {
        printf("Compressed format detected for plane %u and no test pattern "
               "filename "
               "given.\n"
...
static int tx_video_next_frame(void *priv, uint16_t *next_frame_idx,
  ...
  if (TX_FRAME_READY ==
      framebuff_ctl->buffs[framebuff_ctl->transmit_idx].status) {
...
static int tx_video_frame_done(void *priv, uint16_t frame_idx,
  ...
  if (frame_idx != framebuff_ctl->receive_idx) {
    ...
    printf("%s, receive_idx %d != frame_done %d\n", __func__,
           framebuff_ctl->receive_idx, frame_idx);
    return ret;
  }

  pthread_mutex_lock(&(framebuff_ctl->wake_mutex));

  if (TX_FRAME_TRANSMITTING !=
  ...
  ret =
      video_queue_buffer(&(st_v4l2_tx->dev), frame_idx, st_v4l2_tx->fill_mode);
  if (ret < 0) {
...
static void tx_video_debug_output(void) {
  struct st_v4l2_tx_video_session *tx_video_session =
      g_st_v4l2_tx->tx_video_sessions;
...
static int
tx_video_verify_buffer(struct st_v4l2_tx_video_session *tx_video_session,
                       struct v4l2_buffer *buf) {
  struct st_v4l2_tx_context *st_v4l2_tx = tx_video_session->ctx;
  ...
  if (dev->plane_fmt[0].sizeimage !=
      (length + dev->plane_fmt[0].bytesperline)) {
...
static int
tx_video_copy_frame(struct st_v4l2_tx_video_session *tx_video_session,
                    struct v4l2_buffer *buf) {
  struct st_v4l2_tx_context *st_v4l2_tx = tx_video_session->ctx;
  struct device *dev = &(st_v4l2_tx->dev);
  struct tx_frame_buff_ct *framebuff_ctl = &(tx_video_session->framebuff_ctl);
  ...
  if (TX_FRAME_RECEIVING !=
      framebuff_ctl->buffs[framebuff_ctl->ready_idx].status) {
    /* buff full */
  ...
  if (tx_video_session->ops_tx.flags & ST20_TX_FLAG_EXT_FRAME) {
    st20_tx_set_ext_frame(
        tx_video_session->handle, framebuff_ctl->ready_idx,
  ...
  framebuff_ctl->buffs[framebuff_ctl->ready_idx].size =
      tx_video_session->framebuff_size;

  clock_gettime(CLOCK_MONOTONIC,
                &(framebuff_ctl->buffs[framebuff_ctl->ready_idx].app_ts));
  framebuff_ctl->buffs[framebuff_ctl->ready_idx].v4l2_ts.tv_sec =
...
static void *tx_video_thread_capture(void *arg) {
  ...
  for (i = 0; i < st_v4l2_tx->nframes; ++i) {
    ...
    /*
       ...
       v4l2_field_name(buf.field), buf.sequence, buf.m.planes[0].bytesused,
    ...
    */
    ...
    if (!st_v4l2_tx->skip) {
      ...
      ret = video_queue_buffer(&(st_v4l2_tx->dev), buf.index,
                               st_v4l2_tx->fill_mode);
      if (ret < 0) {
        printf("%s Unable to requeue buffer: %d\n", __func__, ret);
        break;
      }
...
static int
tx_video_thread_create(struct st_v4l2_tx_video_session *tx_video_session,
                       unsigned int priority, unsigned int cpu) {
  int ret = 0;

  if (pthread_create(&(tx_video_session->st20_app_thread), NULL,
                     tx_video_thread_capture, tx_video_session)) {
    printf("%s pthread_create Failed: %s (%d)\n", __func__, strerror(errno),
           errno);
    ret = -EIO;
    return ret;
  ...
  if (video_set_realtime(tx_video_session->st20_app_thread, priority, cpu) <
      0) {
    printf("%s video_set_realtime Failed\n", __func__);
    ret = -EIO;
    return ret;
...
int main(int argc, char *argv[]) {
  ...
  st_v4l2_tx =
      (struct st_v4l2_tx_context *)malloc(sizeof(struct st_v4l2_tx_context));
  if (!st_v4l2_tx) {
  ...
  if (video_set_format(&(st_v4l2_tx->dev), width, height, V4L2_PIX_FMT_UYVY, 0,
                       0, V4L2_FIELD_ANY, 0) < 0) {
    video_close(&(st_v4l2_tx->dev));
    free(st_v4l2_tx);
    return -EIO;
  }
  ...
  memcpy(st_v4l2_tx->param.sip_addr[MTL_PORT_P], g_tx_video_local_ip,
         MTL_IP_ADDR_LEN);
  ...
  st_v4l2_tx->param.log_level =
      MTL_LOG_LEVEL_INFO;        // log level. ERROR, INFO, WARNING
  ...
  if (!tx_video_session) {
    printf("%s struct st_v4l2_tx_video_session is not correctly malloc",
           __func__);
  ...
  for (int i = 0; i < session_num; i++) {
    ...
    tx_video_session->ops_tx.priv =
        tx_video_session; // app handle register to lib
    ...
    if (tx_video_session->ops_tx.flags & ST20_TX_FLAG_EXT_FRAME) {
      ...
      tx_video_session->ext_frames = (struct st20_ext_frame *)malloc(
          sizeof(struct st20_ext_frame) * nbufs);
      if (!tx_video_session->ext_frames) {
        printf("%s[%d], ext_frames malloc fail\n", __func__, i);
        ret = -EIO;
        goto error;
      }
      memset(tx_video_session->ext_frames, 0,
             sizeof(struct st20_ext_frame) * nbufs);

      for (int j = 0; j < nbufs; ++j) {
        tx_video_session->ext_frames[j].buf_addr =
            st_v4l2_tx->dev.buffers[j].mem[0];
    ...
    if (st_v4l2_tx->has_sdl) {
      ret = app_init_display(&(tx_video_session->display), i,
                             st_v4l2_tx->dev.width, st_v4l2_tx->dev.height,
                             st_v4l2_tx->ttf_file);
      if (ret < 0) {
  ...
  for (int i = 0; i < session_num; i++) {
    tx_video_session = &(st_v4l2_tx->tx_video_sessions[i]);
    ret = tx_video_thread_create(tx_video_session, v4l2_thread_priority,
                                 v4l2_thread_cpu);
    if (ret < 0) {
    ...
    if (tx_video_session->ext_frames) {
      for (int j = 0; j < nbufs; ++j) {
        if ((tx_video_session->ext_frames[j].buf_iova != MTL_BAD_IOVA) &&
          ...
          ret = mtl_dma_unmap(st_v4l2_tx->st,
                              tx_video_session->ext_frames[j].buf_addr,
