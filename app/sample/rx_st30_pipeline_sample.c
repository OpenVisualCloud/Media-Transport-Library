...
static int rx_st30p_open_source(struct rx_st30p_sample_ctx *s,
                                const char *file) {
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < session_num; i++) {
    ...
    ret = pthread_create(&app[i]->frame_thread, NULL, rx_st30p_frame_thread,
                         app[i]);
    if (ret < 0) {
  ...
  for (int i = 0; i < session_num; i++) {
    app[i]->stop = true;
    if (app[i]->handle)
      st30p_rx_wake_block(app[i]->handle);
