...
static void wait_frame_tx_available(struct tx_ctx* s) {
  struct split_fwd_sample_ctx* app = s->app;
...
static void wait_frame_rx_available(struct split_fwd_sample_ctx* s) {
  st_pthread_mutex_lock(&s->rx_wake_mutex);
