...
static int perf_cvt_422_10_pg2_le_to_v210(int w, int h, int frames) {
  ...
  for (int i = 0; i < frames; i++) {
    st20_rfc4175_422be10_to_v210_simd((uint8_t*)pg_be, pg_v210, w, h, ST_SIMD_LEVEL_NONE);
  }
  end = clock();
  ...
  if (cpu_level >= ST_SIMD_LEVEL_AVX512) {
    start = clock();
    for (int i = 0; i < frames; i++) {
      st20_rfc4175_422be10_to_v210_simd((uint8_t*)pg_be, pg_v210, w, h,
  ...
  for (int i = 0; i < frames; i++) {
    ...
    st20_rfc4175_422le10_to_v210_simd((uint8_t*)pg_le, pg_v210, w, h, ST_SIMD_LEVEL_NONE);
  }
  end = clock();
  duration = (float)(end - start) / CLOCKS_PER_SEC;
  printf("scalar, time: %f secs with %d frames(%dx%d,%fm)\n", duration, frames, w, h,
  ...
  if (cpu_level >= ST_SIMD_LEVEL_AVX512) {
    ...
    for (int i = 0; i < frames; i++) {
      ...
      st20_rfc4175_422le10_to_v210_simd((uint8_t*)pg_le, pg_v210, w, h,
                                        ST_SIMD_LEVEL_AVX512);
    }
    end = clock();
    float duration_simd = (float)(end - start) / CLOCKS_PER_SEC;
...
int main(int argc, char** argv) {
  perf_cvt_422_10_pg2_le_to_v210(640, 480, 60);
  perf_cvt_422_10_pg2_le_to_v210(1280, 720, 60);
  perf_cvt_422_10_pg2_le_to_v210(1920, 1080, 60);
  perf_cvt_422_10_pg2_le_to_v210(1920 * 2, 1080 * 2, 60);
  perf_cvt_422_10_pg2_le_to_v210(1920 * 4, 1080 * 4, 60);
  return 0;
}