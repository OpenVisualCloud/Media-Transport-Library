...
static int perf_cvt_422_10_pg2_be_to_v210(mtl_handle st, int w, int h,
  ...
  size_t be_1line_size =
      st_frame_size(ST_FRAME_FMT_YUV422RFC4175PG2BE10, w, 1, false);
  ...
  for (int i = 0; i < frames; i++) {
    pg_be_in = pg_be + (i % fb_cnt) * (fb_pg2_size / sizeof(*pg_be));
    pg_v210_out =
        pg_v210 + (i % fb_cnt) * (fb_pg2_size_v210 / sizeof(*pg_v210));
  ...
  if (cpu_level >= MTL_SIMD_LEVEL_AVX512) {
    ...
    if (dma) {
      ...
      for (int i = 0; i < frames; i++) {
        ...
        pg_v210_out =
            pg_v210 + (i % fb_cnt) * (fb_pg2_size_v210 / sizeof(*pg_v210));
        st20_rfc4175_422be10_to_v210_simd_dma(dma, pg_be_in, pg_be_in_iova,
                                              pg_v210_out, w, h,
                                              MTL_SIMD_LEVEL_AVX512);
      }
      end = clock();
...
static void *perf_thread(void *arg) {
  ...
  perf_cvt_422_10_pg2_be_to_v210(dev_handle, 1920 * 2, 1080 * 2, frames,
                                 fb_cnt);
  perf_cvt_422_10_pg2_be_to_v210(dev_handle, 1920 * 4, 1080 * 4, frames,
                                 fb_cnt);
