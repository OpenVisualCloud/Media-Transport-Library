...
static int perf_cvt_422_12_pg2_to_planar_le(mtl_handle st, int w, int h, int frames,
  ...
  for (int i = 0; i < frames; i++) {
    ...
    p12_u16_r_out = p12_u16_r + (i % fb_cnt) * (planar_size / sizeof(*p12_u16_r));
    st20_rfc4175_422be12_to_yuv422p12le_simd(pg_be_in, p12_u16_out, p12_u16_b_out,
                                             p12_u16_r_out, w, h, MTL_SIMD_LEVEL_NONE);
  }
  end = clock();
    ...
    if (dma) {
      ...
      for (int i = 0; i < frames; i++) {
        ...
        st20_rfc4175_422be12_to_yuv422p12le_simd_dma(
            dma, pg_be_in, pg_be_in_iova, p12_u16_out, p12_u16_b_out, p12_u16_r_out, w, h,
            MTL_SIMD_LEVEL_AVX512);
      }
      end = clock();
      float duration_simd = (float)(end - start) / CLOCKS_PER_SEC;
...
static void* perf_thread(void* arg) {
  ...
  perf_cvt_422_12_pg2_to_planar_le(dev_handle, 1920 * 2, 1080 * 2, frames, fb_cnt);
  perf_cvt_422_12_pg2_to_planar_le(dev_handle, 1920 * 4, 1080 * 4, frames, fb_cnt);
