...
static int perf_cvt_422_10_pg2_be_to_le(mtl_handle st, int w, int h, int frames,
                                        int fb_cnt) {
  ...
  if (cpu_level >= MTL_SIMD_LEVEL_AVX2) {
    ...
    for (int i = 0; i < frames * 1; i++) {
      pg_be_in = pg_be + (i % fb_cnt) * (fb_pg2_size / sizeof(*pg_be));
      pg_le_out = pg_le + (i % fb_cnt) * (fb_pg2_size / sizeof(*pg_le));
      st20_rfc4175_422be10_to_422le10_simd(pg_be_in, pg_le_out, w, h, MTL_SIMD_LEVEL_AVX2);
    }
    end = clock();
  ...
  if (cpu_level >= MTL_SIMD_LEVEL_AVX512) {
    ...
    if (dma) {
      ...
      for (int i = 0; i < frames; i++) {
        ...
        pg_le_out = pg_le + (i % fb_cnt) * (fb_pg2_size / sizeof(*pg_le));
        st20_rfc4175_422be10_to_422le10_simd_dma(dma, pg_be_in, pg_be_in_iova, pg_le_out,
...
static void* perf_thread(void* arg) {
  ...
  perf_cvt_422_10_pg2_be_to_le(dev_handle, 1280, 720, frames, fb_cnt);
  perf_cvt_422_10_pg2_be_to_le(dev_handle, 1920, 1080, frames, fb_cnt);
