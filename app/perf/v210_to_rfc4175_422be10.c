...
static int perf_cvt_v210_to_be(mtl_handle st, int w, int h, int frames, int fb_cnt) {
  ...
  uint8_t* pg_v210 = (uint8_t*)st_hp_malloc(st, fb_size_v210 * fb_cnt, MTL_PORT_P);
  mtl_iova_t pg_v210_iova = st_hp_virt2iova(st, pg_v210);
  ...
  for (int i = 0; i < frames; i++) {
    ...
    st20_v210_to_rfc4175_422be10_simd(pg_v210_in, pg_be_out, w, h, MTL_SIMD_LEVEL_NONE);
  }
  end = clock();
