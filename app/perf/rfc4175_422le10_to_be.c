...
static int perf_cvt_422_10_pg2_le_to_be(st_handle st, int w, int h, int frames,
                                        int fb_cnt) {
  size_t fb_pg2_size = w * h * 5 / 2;
  st_udma_handle dma = st_udma_create(st, 128, ST_PORT_P);
  struct st20_rfc4175_422_10_pg2_le* pg_le =
      (struct st20_rfc4175_422_10_pg2_le*)st_hp_malloc(st, fb_pg2_size * fb_cnt,
                                                       ST_PORT_P);
  st_iova_t pg_le_iova = st_hp_virt2iova(st, pg_le);
  st_iova_t pg_le_in_iova;
  ...
  for (int i = 0; i < frames * 1; i++) {
    ...
    st20_rfc4175_422le10_to_422be10_simd(pg_le_in, pg_be_out, w, h, ST_SIMD_LEVEL_NONE);
  }
  end = clock();
  ...
  if (cpu_level >= ST_SIMD_LEVEL_AVX2) {
    ...
    for (int i = 0; i < frames * 1; i++) {
      pg_le_in = pg_le + (i % fb_cnt) * (fb_pg2_size / sizeof(*pg_be));
      pg_be_out = pg_be + (i % fb_cnt) * (fb_pg2_size / sizeof(*pg_le));
      st20_rfc4175_422le10_to_422be10_simd(pg_le_in, pg_be_out, w, h, ST_SIMD_LEVEL_AVX2);
    }
    end = clock();
