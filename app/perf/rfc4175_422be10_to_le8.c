...
static int perf_cvt_422_10_pg2_be_to_le8(int w, int h, int frames) {
  ...
  struct st20_rfc4175_422_8_pg2_le* pg_8 =
      (struct st20_rfc4175_422_8_pg2_le*)malloc(w * h * 2);
  ...
  fill_422_10_pd2_data(pg_10, w, h);
  ...
  for (int i = 0; i < frames; i++) {
    st20_rfc4175_422be10_to_422le8_simd(pg_10, pg_8, w, h, ST_SIMD_LEVEL_NONE);
  }
  end = clock();
  ...
  if (cpu_level >= ST_SIMD_LEVEL_AVX512) {
    start = clock();
    for (int i = 0; i < frames; i++) {
      st20_rfc4175_422be10_to_422le8_simd(pg_10, pg_8, w, h, ST_SIMD_LEVEL_AVX512);
    }
    end = clock();
  ...
  if (cpu_level >= ST_SIMD_LEVEL_AVX512_VBMI2) {
    start = clock();
    for (int i = 0; i < frames; i++) {
      st20_rfc4175_422be10_to_422le8_simd(pg_10, pg_8, w, h, ST_SIMD_LEVEL_AVX512_VBMI2);
    }
    end = clock();
...
int main(int argc, char** argv) {
  perf_cvt_422_10_pg2_be_to_le8(640, 480, 60);
  perf_cvt_422_10_pg2_be_to_le8(1280, 720, 60);
  perf_cvt_422_10_pg2_be_to_le8(1920, 1080, 60);
  perf_cvt_422_10_pg2_be_to_le8(1920 * 2, 1080 * 2, 60);
  perf_cvt_422_10_pg2_be_to_le8(1920 * 4, 1080 * 4, 60);
  return 0;
}