...
static int perf_cvt_y210_to_be(mtl_handle st, int w, int h, int frames,
                               int fb_cnt) {
  ...
  uint16_t *pg_y210 =
      (uint16_t *)mtl_hp_malloc(st, fb_size_y210 * fb_cnt, MTL_PORT_P);
  ...
  info("scalar, time: %f secs with %d frames(%dx%d,%fm@%d buffers)\n", duration,
       frames, w, h, fb_size_y210_m, fb_cnt);

  if (cpu_level >= MTL_SIMD_LEVEL_AVX512) {
    start = clock();
    for (int i = 0; i < frames; i++) {
      pg_y210_in =
          pg_y210 + (i % fb_cnt) * (fb_size_y210 / sizeof(*pg_y210_in));
      pg_be_out = pg_be + (i % fb_cnt) * (fb_pg2_size / sizeof(*pg_be));
      st20_y210_to_rfc4175_422be10_simd(pg_y210_in, pg_be_out, w, h,
    ...
    if (dma) {
      ...
      for (int i = 0; i < frames; i++) {
        ...
        pg_be_out = pg_be + (i % fb_cnt) * (fb_pg2_size / sizeof(*pg_be));
        st20_y210_to_rfc4175_422be10_simd_dma(dma, pg_y210_in, pg_y210_in_iova,
                                              pg_be_out, w, h,
                                              MTL_SIMD_LEVEL_AVX512);
      }
      end = clock();
