...
static void fill_422_planar_le(uint16_t *y, uint16_t *b, uint16_t *r, int w,
                               int h) {
  int pg_size = w * h / 2;
...
static int perf_cvt_planar_le_to_422_10_pg2(mtl_handle st, int w, int h,
  ...
  uint16_t *p10_u16 =
      (uint16_t *)mtl_hp_malloc(st, planar_size * fb_cnt, MTL_PORT_P);
  ...
  for (int i = 0; i < frames; i++) {
    ...
    p10_u16_r_in =
        p10_u16_r + (i % fb_cnt) * (planar_size / sizeof(*p10_u16_r));
    st20_yuv422p10le_to_rfc4175_422be10_simd(p10_u16_in, p10_u16_b_in,
                                             p10_u16_r_in, pg_be_out, w, h,
                                             MTL_SIMD_LEVEL_NONE);
  }
  end = clock();
...
static void *perf_thread(void *arg) {
  ...
  perf_cvt_planar_le_to_422_10_pg2(dev_handle, 1920 * 2, 1080 * 2, frames,
                                   fb_cnt);
  perf_cvt_planar_le_to_422_10_pg2(dev_handle, 1920 * 4, 1080 * 4, frames,
                                   fb_cnt);
