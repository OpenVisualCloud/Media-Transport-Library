...
static int perf_cvt_422_10_pg2_to_planar_le(st_handle st, int w, int h, int frames,
                                            int fb_cnt) {
  ...
  for (int i = 0; i < frames; i++) {
    ...
    st20_rfc4175_422be10_to_yuv422p10le_simd(pg_be_in, p10_u16_out, p10_u16_b_out,
                                             p10_u16_r_out, w, h, ST_SIMD_LEVEL_NONE);
  }
  end = clock();
  ...
  if (cpu_level >= ST_SIMD_LEVEL_AVX512) {
    ...
    for (int i = 0; i < frames; i++) {
      ...
      p10_u16_r_out = p10_u16_r + (i % fb_cnt) * (planar_size / sizeof(*p10_u16_r));
      st20_rfc4175_422be10_to_yuv422p10le_simd(pg_be_in, p10_u16_out, p10_u16_b_out,
                                               p10_u16_r_out, w, h, ST_SIMD_LEVEL_AVX512);
    }
    end = clock();
