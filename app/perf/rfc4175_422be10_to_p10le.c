...
static int perf_cvt_422_10_pg2_to_planar_le(mtl_handle st, int w, int h,
  ...
  for (int i = 0; i < frames; i++) {
    ...
    p10_u16_b_out =
        p10_u16_b + (i % fb_cnt) * (planar_size / sizeof(*p10_u16_b));
    p10_u16_r_out =
        p10_u16_r + (i % fb_cnt) * (planar_size / sizeof(*p10_u16_r));
    st20_rfc4175_422be10_to_yuv422p10le_simd(pg_be_in, p10_u16_out,
                                             p10_u16_b_out, p10_u16_r_out, w, h,
                                             MTL_SIMD_LEVEL_NONE);
  }
  end = clock();
    ...
    if (dma) {
      ...
      for (int i = 0; i < frames; i++) {
        ...
        st20_rfc4175_422be10_to_yuv422p10le_simd_dma(
            dma, pg_be_in, pg_be_in_iova, p10_u16_out, p10_u16_b_out,
  ...
  if (cpu_level >= MTL_SIMD_LEVEL_AVX512_VBMI2) {
    ...
    if (dma) {
      ...
      for (int i = 0; i < frames; i++) {
        ...
        st20_rfc4175_422be10_to_yuv422p10le_simd_dma(
            dma, pg_be_in, pg_be_in_iova, p10_u16_out, p10_u16_b_out,
            p10_u16_r_out, w, h, MTL_SIMD_LEVEL_AVX512_VBMI2);
      }
      end = clock();
      float duration_vbmi = (float)(end - start) / CLOCKS_PER_SEC;
...
static void *perf_thread(void *arg) {
  ...
  perf_cvt_422_10_pg2_to_planar_le(dev_handle, 1920 * 2, 1080 * 2, frames,
                                   fb_cnt);
  perf_cvt_422_10_pg2_to_planar_le(dev_handle, 1920 * 4, 1080 * 4, frames,
                                   fb_cnt);
