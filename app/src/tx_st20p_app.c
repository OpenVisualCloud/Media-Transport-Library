...
static int app_tx_st20p_open_source(struct st_app_tx_st20p_session* s) {
  ...
  s->st20p_source_begin = st_hp_malloc(s->st, i.st_size, ST_PORT_P);
...
static int app_tx_st20p_init(struct st_app_context* ctx, st_json_st20p_session_t* st20p,
  ...
  memcpy(ops.port.dip_addr[ST_PORT_P],
         st20p ? st20p->base.ip[ST_PORT_P] : ctx->tx_dip_addr[ST_PORT_P], ST_IP_ADDR_LEN);
  ...
  if (ops.port.num_port > 1) {
    memcpy(ops.port.dip_addr[ST_PORT_R],
           st20p ? st20p->base.ip[ST_PORT_R] : ctx->tx_dip_addr[ST_PORT_R],
           ST_IP_ADDR_LEN);
