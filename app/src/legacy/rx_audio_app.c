...
static int app_rx_audio_close_source(struct st_app_rx_audio_session *session) {
  if (session->st30_ref_fd >= 0) {
    munmap(session->st30_ref_begin,
           session->st30_ref_end - session->st30_ref_begin);
...
static int
app_rx_audio_compare_with_ref(struct st_app_rx_audio_session *session,
                              void *frame) {
  int ret = -1;
  bool rewind = false;
  int count = 0;
  ...
  while (ret) {
    ...
    if ((session->st30_ref_cursor >= session->st30_ref_end) ||
        ((session->st30_ref_end - session->st30_ref_cursor) <
         session->st30_frame_size)) {
...
static int app_rx_audio_handle_rtp(struct st_app_rx_audio_session *s,
  ...
  if (s->st30_ref_fd > 0)
    app_rx_audio_compare_with_ref(s, payload);
  return 0;
}
...
static int app_rx_audio_timing_parser_result(void *priv,
  ...
  if (tp->compliant != ST_RX_TP_COMPLIANT_NARROW) {
    warn("%s(%d,%d), compliant %d, failed cause %s, pkts_cnt %u\n", __func__,
         s->idx, port, tp->compliant, tp->failed_cause, tp->pkts_cnt);
    warn("%s(%d,%d), tsdf %dus, ipt(ns) min %d max %d avg %f\n", __func__,
         s->idx, port, tp->tsdf, tp->ipt_min, tp->ipt_max, tp->ipt_avg);
  ...
  if (s->enable_timing_parser_meta) {
    if ((s->stat_dump_cnt % 6) == 0) {
      ...
      warn("%s(%d), COMPLIANT NARROW %d WIDE %d FAILED %d, ipt max %fus\n",
           __func__, s->idx,
           s->stat_compliant_result[ST_RX_TP_COMPLIANT_NARROW],
           s->stat_compliant_result[ST_RX_TP_COMPLIANT_WIDE],
           s->stat_compliant_result[ST_RX_TP_COMPLIANT_FAILED],
           (float)s->ipt_max / 1000);
...
static int app_rx_audio_init(struct st_app_context *ctx,
  ...
  memcpy(ops.mcast_sip_addr[MTL_SESSION_PORT_P],
         audio ? audio->base.mcast_src_ip[MTL_PORT_P]
  ...
  s->expect_fps =
      (double)NS_PER_S / st30_get_packet_time(ops.ptime) / pkt_per_frame;

  ops.framebuff_size = s->st30_frame_size;
  ops.framebuff_cnt = s->framebuff_cnt;
  ops.rtp_ring_size =
      ctx->rx_audio_rtp_ring_size ? ctx->rx_audio_rtp_ring_size : 16;
  if (audio && audio->enable_rtcp)
    ops.flags |= ST30_RX_FLAG_ENABLE_RTCP;
  ...
  /* dump */
  snprintf(s->st30_dump_url, ST_APP_URL_MAX_LEN, "st_audio_app%d_%d_%d_c%u.pcm",
           idx, st30_get_sample_rate(ops.sampling),
