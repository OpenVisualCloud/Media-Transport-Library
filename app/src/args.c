...
static struct option st_app_args_options[] = {
    ...
    {"tx_video_sessions_count", required_argument, 0,
     ST_ARG_TX_VIDEO_SESSIONS_CNT},
    {"tx_video_rtp_ring_size", required_argument, 0,
     ST_ARG_TX_VIDEO_RTP_RING_SIZE},
    {"tx_audio_url", required_argument, 0, ST_ARG_TX_AUDIO_URL},
    {"tx_audio_sessions_count", required_argument, 0,
     ST_ARG_TX_AUDIO_SESSIONS_CNT},
    {"tx_audio_rtp_ring_size", required_argument, 0,
     ST_ARG_TX_AUDIO_RTP_RING_SIZE},
    ...
    {"rx_video_sessions_count", required_argument, 0,
     ST_ARG_RX_VIDEO_SESSIONS_CNT},
    ...
    {"rx_video_rtp_ring_size", required_argument, 0,
     ST_ARG_RX_VIDEO_RTP_RING_SIZE},
    {"rx_video_multi_thread", no_argument, 0, ST_ARG_RX_VIDEO_MULTI_THREADS},
    {"rx_audio_sessions_count", required_argument, 0,
     ST_ARG_RX_AUDIO_SESSIONS_CNT},
    {"rx_audio_rtp_ring_size", required_argument, 0,
     ST_ARG_RX_AUDIO_RTP_RING_SIZE},
...
static int app_args_json(struct st_app_context *ctx, struct mtl_init_params *p,
  ...
  for (int i = 0; i < ctx->json_ctx->num_interfaces; ++i) {
    ...
    memcpy(p->sip_addr[i], ctx->json_ctx->interfaces[i].ip_addr,
           sizeof(p->sip_addr[i]));
    memcpy(p->netmask[i], ctx->json_ctx->interfaces[i].netmask,
           sizeof(p->netmask[i]));
    memcpy(p->gateway[i], ctx->json_ctx->interfaces[i].gateway,
           sizeof(p->gateway[i]));
  ...
  if (ctx->json_ctx->tx_audio_sessions_max_per_sch) {
    p->tx_audio_sessions_max_per_sch =
        ctx->json_ctx->tx_audio_sessions_max_per_sch;
  }
  if (ctx->json_ctx->rx_audio_sessions_max_per_sch) {
    p->rx_audio_sessions_max_per_sch =
        ctx->json_ctx->rx_audio_sessions_max_per_sch;
  }
  if (ctx->json_ctx->shared_tx_queues)
    p->flags |= MTL_FLAG_SHARED_TX_QUEUE;
  if (ctx->json_ctx->shared_rx_queues)
    p->flags |= MTL_FLAG_SHARED_RX_QUEUE;
  if (ctx->json_ctx->tx_no_chain)
    p->flags |= MTL_FLAG_TX_NO_CHAIN;
  if (ctx->json_ctx->rss_mode)
    p->rss_mode = ctx->json_ctx->rss_mode;
  if (ctx->json_ctx->log_file)
    st_set_mtl_log_file(ctx, ctx->json_ctx->log_file);
...
static int app_args_parse_port(struct st_app_context *ctx,
                               struct mtl_init_params *p, char *str,
                               enum mtl_port port) {
