...
RUN apt-get update -y && \
    apt-get install -y libnuma1 libjson-c5 libpcap0.8 libsdl2-2.0-0 libsdl2-ttf-2.0-0 libssl3 zlib1g libelf1 libcap-ng0 libcap2-bin && \
    apt-get clean && \
    rm -rf /var/lib/apt/lists/*
...
# Add caps for binary
RUN cd /home/imtl && \
    setcap 'cap_net_admin+ep cap_net_raw+ep' ./app/RxTxApp

USER imtl

WORKDIR /home/imtl/

ENTRYPOINT ["app/RxTxApp"]
