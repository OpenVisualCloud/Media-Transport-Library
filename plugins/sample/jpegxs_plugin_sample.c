...
#include "jpegxs_plugin_sample.h"
...
static int encode_frame(struct jpegxs_encoder_session* s,
...
static void* encode_thread(void* arg) {
  struct jpegxs_encoder_session* s = arg;
...
static st22_encode_priv encoder_create_session(void* priv, st22p_encode_session session_p,
  ...
  struct jpegxs_sample_ctx* ctx = priv;
  ...
  for (int i = 0; i < MAX_SAMPLE_ENCODER_SESSIONS; i++) {
    ...
    printf("%s(%d), max_codestream_size %ld\n", __func__, i,
...
static int encoder_free_session(void* priv, st22_encode_priv session) {
  struct jpegxs_sample_ctx* ctx = priv;
  struct jpegxs_encoder_session* encoder_session = session;
...
static int encoder_frame_available(void* priv) {
  struct jpegxs_encoder_session* s = priv;
...
static int decode_frame(struct jpegxs_decoder_session* s,
...
static void* decode_thread(void* arg) {
  struct jpegxs_decoder_session* s = arg;
...
static int decoder_free_session(void* priv, st22_decode_priv session) {
  struct jpegxs_sample_ctx* ctx = priv;
  struct jpegxs_decoder_session* decoder_session = session;
...
static int decoder_frame_available(void* priv) {
  struct jpegxs_decoder_session* s = priv;
...
st_plugin_priv st_plugin_create(st_handle st) {
  ...
  d_dev.name = "jpegxs_decoder_sample";
  d_dev.priv = ctx;
  ...
  if (!ctx->decoder_dev_handle) {
    printf("%s, decoder register fail\n", __func__);
    free(ctx);
    return NULL;
  }
  ...
  e_dev.name = "jpegxs_encoder_sample";
  e_dev.priv = ctx;
  ...
  if (!ctx->encoder_dev_handle) {
    printf("%s, encoder register fail\n", __func__);
    st22_decoder_unregister(ctx->decoder_dev_handle);
    free(ctx);
    return NULL;
  }

  printf("%s, succ with sample plugin\n", __func__);
  return ctx;
}

int st_plugin_free(st_plugin_priv handle) {
  struct jpegxs_sample_ctx* ctx = handle;
