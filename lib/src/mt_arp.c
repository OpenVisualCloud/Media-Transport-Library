...
static int arp_receive_request(struct mtl_main_impl *impl,
                               struct rte_arp_hdr *request,
                               enum mtl_port port) {
  if (!arp_is_valid_hdr(request))
    return -EINVAL;
  ...
  struct rte_arp_hdr *arp = rte_pktmbuf_mtod_offset(
      rpl_pkt, struct rte_arp_hdr *, sizeof(struct rte_ether_hdr));
...
static int arp_receive_reply(struct mtl_main_impl *impl,
                             struct rte_arp_hdr *reply, enum mtl_port port) {
  if (!arp_is_valid_hdr(reply))
    return -EINVAL;
  ...
  info_once("%s(%d), from %d.%d.%d.%d, mac: "
            "%02hhx:%02hhx:%02hhx:%02hhx:%02hhx:%02hhx\n",
            __func__, port, ip[0], ip[1], ip[2], ip[3], addr_bytes[0],
  ...
  for (i = 0; i < MT_ARP_ENTRY_MAX; i++) {
    ...
    if (entry->ip == reply->arp_data.arp_sip)
      break;
  }
  ...
  /* save to arp table */
  memcpy(entry->ea.addr_bytes, reply->arp_data.arp_sha.addr_bytes,
         RTE_ETHER_ADDR_LEN);
...
static int arp_send_req(struct mtl_main_impl *impl, enum mtl_port port,
                        uint32_t ip) {
  ...
  eth->ether_type = htons(RTE_ETHER_TYPE_ARP); // ARP_PROTOCOL
  struct rte_arp_hdr *arp = rte_pktmbuf_mtod_offset(
      req_pkt, struct rte_arp_hdr *, sizeof(struct rte_ether_hdr));
  arp->arp_hardware = htons(RTE_ARP_HRD_ETHER);
  arp->arp_protocol = htons(RTE_ETHER_TYPE_IPV4); // IP protocol
  arp->arp_hlen = RTE_ETHER_ADDR_LEN;             // size of MAC
  arp->arp_plen = 4;                              // size of fo IP
...
static int arp_get_result(struct mt_arp_impl *arp_impl,
                          struct mt_arp_entry *entry, int timeout_ms) {
  ...
  if (timeout_ms)
    max_retry = (timeout_ms / sleep_interval_ms) + 1;
...
int mt_arp_parse(struct mtl_main_impl *impl, struct rte_arp_hdr *hdr,
  ...
  default:
    err("%s, mt_arp_parse %04x unimplemented\n", __func__,
        ntohs(hdr->arp_opcode));
    return -EINVAL;
  }
}

static int mt_arp_cni_get_mac(struct mtl_main_impl *impl,
                              struct rte_ether_addr *ea, enum mtl_port port,
                              uint32_t ip, int timeout_ms) {
  struct mt_arp_impl *arp_impl = get_arp(impl, port);
  struct mt_arp_entry *entry = NULL;
  int ret;
  ...
  arp_start_arp_timer(
      arp_impl); /* start the timer to monitor if send arp again */
...
int mt_arp_get_mac(struct mtl_main_impl *impl, uint8_t dip[MTL_IP_ADDR_LEN],
  ...
  if (mt_drv_use_kernel_ctl(impl, port) || mt_has_virtio_user(impl, port)) {
    ret = mt_socket_get_mac(impl, mt_kernel_if_name(impl, port), dip, ea,
                            timeout_ms);
    if (ret < 0) {
