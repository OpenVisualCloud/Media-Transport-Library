...
static int tx_ancillary_session_reset_pacing_epoch(
...
st40_tx_handle st40_tx_create(mtl_handle mt, struct st40_tx_ops* ops) {
  ...
  ret = tx_ancillary_ops_prune_down_ports(impl, ops);
  if (ret < 0) {
    err("%s, tx_ancillary_ops_prune_down_ports fail %d\n", __func__, ret);
    return NULL;
  }
...
void* st40_tx_get_mbuf(st40_tx_handle handle, void** usrptr) {
  ...
  MT_HANDLE_GUARD(s_impl, MT_HANDLE_TX_ANC, NULL);
...
int st40_tx_put_mbuf(st40_tx_handle handle, void* mbuf, uint16_t len) {
  ...
  MT_HANDLE_GUARD(s_impl, MT_HANDLE_TX_ANC, -EIO);
...
int st40_tx_free(st40_tx_handle handle) {
  ...
  int _gd =
      mt_handle_begin_destroy(&s_impl->lc_destroying, &s_impl->type, MT_HANDLE_TX_ANC);
  if (_gd < 0) {
...
int st40_tx_get_session_stats(st40_tx_handle handle, struct st40_tx_user_stats* stats) {
  ...
  MT_HANDLE_GUARD(s_impl, MT_HANDLE_TX_ANC, -EINVAL);
