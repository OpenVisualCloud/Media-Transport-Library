...
static void tx_ancillary_validate_user_timestamp(struct st_tx_ancillary_session_impl* s,
  ...
  } else if (requested_epoch >
             current_epoch + (NS_PER_S / s->pacing.frame_time)) {
    dbg("%s(%d), requested epoch %" PRIu64 " too far in the future, current epoch %" PRIu64
        "\n",
        __func__, s->idx, requested_epoch, current_epoch);
    ST_SESSION_STAT_INC(s, port_user_stats.common, stat_error_user_timestamp);
  }
...
static inline uint64_t tx_ancillary_calc_epoch(struct st_tx_ancillary_session_impl* s,
  ...
  if (current_epoch <= next_free_epoch) {
    if (next_free_epoch - current_epoch > pacing->max_onward_epochs) {
      dbg("%s(%d), onward range exceeded, next_free_epoch %" PRIu64 ", current_epoch %" PRIu64
          "\n",
...
static int tx_ancillary_session_sync_pacing(struct mtl_main_impl* impl,
  ...
  if ((s->ops.flags & ST40_TX_FLAG_EXACT_USER_PACING) && required_tai) {
    ...
    uint32_t user_rtp_ts =
        st10_tai_to_media_clk(required_tai, s->fps_tm.sampling_clock_rate);
    pacing->pacing_time_stamp = user_rtp_ts;
    pacing->rtp_time_stamp = user_rtp_ts;
  } else {
    start_time_tai = tx_ancillary_pacing_time(pacing, pacing->cur_epochs);
    pacing->pacing_time_stamp = tx_ancillary_pacing_time_stamp(pacing, pacing->cur_epochs);
  ...
  pacing->cur_epoch_time = start_time_tai;
  pacing->tsc_time_cursor = (double)cur_tsc + (double)time_to_tx_ns;
  dbg("%s(%d), epochs %" PRIu64 " time_stamp %u time_cursor %f time_to_tx_ns %" PRId64 "\n",
      __func__, s->idx, pacing->cur_epochs, pacing->pacing_time_stamp,
      pacing->tsc_time_cursor, time_to_tx_ns);
