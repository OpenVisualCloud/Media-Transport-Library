...
struct st_frame* st22p_tx_get_frame(st22p_tx_handle handle) {
  ...
  if (!framebuff && ctx->block_get) { /* wait here */
    ...
    /* Re-attempt the real claim on every wake/timeout. A sticky notify flag is
     * set by every frame done, also when no thread waits, so a stale flag made
     * the next wait return at once with no free frame. */
    ...
    if (atomic_load_explicit(&ctx->lc_destroying, memory_order_acquire)) {
      if (framebuff) {
        ...
        atomic_store_explicit(&framebuff->stat, ST22P_TX_FRAME_FREE,
