...
static struct st20p_rx_frame* rx_st20p_claim_available(
    struct st20p_rx_ctx* ctx, uint16_t idx_start, enum st20p_rx_frame_status desired,
    enum st20p_rx_frame_status claimed) {
  struct st20p_rx_frame* framebuff;

  while ((framebuff = rx_st20p_next_available(ctx, idx_start, desired))) {
...
static int rx_st20p_packet_convert(void* priv, void* frame,
  ...
  if (meta->row_number == 0 && meta->row_offset == 0) {
    ...
    if (framebuff) {
      __atomic_store_n(&framebuff->stat, ST20P_RX_FRAME_IN_CONVERTING, __ATOMIC_RELEASE);
...
static int rx_st20p_frame_ready(void* priv, void* frame,
  ...
  if (ctx->derive || (ctx->ops.flags & ST20P_RX_FLAG_PKT_CONVERT)) {
    ...
    __atomic_store_n(&framebuff->stat, ST20P_RX_FRAME_CONVERTED, __ATOMIC_RELEASE);
    /* point to next */
    ctx->framebuff_producer_idx = rx_st20p_next_idx(ctx, framebuff->idx);
    rx_st20p_notify_frame_available(ctx);
    return 0;
  }
  __atomic_store_n(&framebuff->stat, ST20P_RX_FRAME_READY, __ATOMIC_RELEASE);
...
static struct st20_convert_frame_meta* rx_st20p_convert_get_frame(void* priv) {
  ...
  /* Claim READY -> IN_CONVERTING. rx_st20p_claim_available() retries across the
  ...
  framebuff = rx_st20p_claim_available(ctx, ctx->framebuff_convert_idx,
                                       ST20P_RX_FRAME_READY, ST20P_RX_FRAME_IN_CONVERTING);
  /* not any ready frame */
  if (!framebuff) {
    goto out;
  }
...
static int rx_st20p_convert_put_frame(void* priv, struct st20_convert_frame_meta* frame,
  ...
  if (ST20P_RX_FRAME_IN_CONVERTING !=
  ...
  if (result < 0) {
    ...
    __atomic_store_n(&framebuff->stat, ST20P_RX_FRAME_FREE, __ATOMIC_RELEASE);
...
static int rx_st20p_init_dst_fbs(struct mtl_main_impl* impl, struct st20p_rx_ctx* ctx,
  ...
  for (uint16_t i = 0; i < ctx->framebuff_cnt; i++) {
    __atomic_store_n(&frames[i].stat, ST20P_RX_FRAME_FREE, __ATOMIC_RELAXED);
    frames[i].idx = i;
...
struct st_frame* st20p_rx_get_frame(st20p_rx_handle handle) {
  ...
  if (ctx->internal_converter) { /* convert internal */
     ...
     * slot. rx_st20p_claim_available() retries across the ring on a lost CAS
     * race, so it only returns NULL once every slot has actually been checked. */
...
int st20p_rx_put_frame(st20p_rx_handle handle, struct st_frame* frame) {
  ...
  if (ST20P_RX_FRAME_IN_USER != __atomic_load_n(&framebuff->stat, __ATOMIC_ACQUIRE)) {
