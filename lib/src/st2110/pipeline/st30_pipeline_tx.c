...
static int st30p_tx_late_frame_spurn(void* handle, uint64_t epoch_skipped) {
...
static int tx_st30p_frame_done(void* priv, uint16_t frame_idx,
  ...
  MT_USDT_ST30P_TX_FRAME_SPURN(ctx->idx, frame_idx, frame->rtp_timestamp);
  return ret;
}
...
static int tx_st30p_create_transport(struct mtl_main_impl* impl, struct st30p_tx_ctx* ctx,
  ...
  if (ops->flags & ST30P_TX_FLAG_DROP_WHEN_LATE) {
    ops_tx.notify_frame_late = st30p_tx_late_frame_spurn;
  }
...
struct st30_frame* st30p_tx_get_frame(st30p_tx_handle handle) {
  ...
  mt_pthread_mutex_lock(&ctx->lock);
  framebuff = tx_st30p_next_available(ctx, 0, ST30P_TX_FRAME_FREE);
