...
int st20p_tx_late_frame_spurn(void* handle, uint64_t epoch_skipped) {
  ...
  MT_USDT_ST20P_TX_FRAME_DONE(ctx->idx, framebuff->idx, framebuff->dst.rtp_timestamp);
  return 0;
}
...
static int tx_st20p_frame_done(void* priv, uint16_t frame_idx,
  ...
  if (ST20P_TX_FRAME_IN_TRANSMITTING == framebuff->stat) {
    ret = 0;
    framebuff->stat = ST20P_TX_FRAME_FREE;
    dbg("%s(%d), done_idx %u, frame_idx: %u\n", __func__, ctx->idx, frame_idx, frame_idx);
  } else {
    ret = -EIO;
...
static int tx_st20p_create_transport(struct mtl_main_impl* impl, struct st20p_tx_ctx* ctx,
  ...
  if (ops->flags & ST20P_TX_FLAG_DROP_WHEN_LATE) {
    ops_tx.notify_frame_late = st20p_tx_late_frame_spurn;
  }
...
struct st_frame* st20p_tx_get_frame(st20p_tx_handle handle) {
  ...
  mt_pthread_mutex_lock(&ctx->lock);
  framebuff = tx_st20p_next_available(ctx, 0, ST20P_TX_FRAME_FREE);
  ...
  framebuff->seq_number = ctx->framebuff_seqence_number++;
  mt_pthread_mutex_unlock(&ctx->lock);
