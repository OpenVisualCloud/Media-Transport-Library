...
static inline struct st_rx_video_session_impl *
rx_video_session_get(struct st_rx_video_sessions_mgr *mgr, int idx) {
...
/* call rx_video_session_put always if get successfully */
static inline struct st_rx_video_session_impl *
rx_video_session_get_timeout(struct st_rx_video_sessions_mgr *mgr, int idx,
                             int timeout_us) {
...
/* call rx_video_session_put always if get successfully */
static inline struct st_rx_video_session_impl *
rx_video_session_try_get(struct st_rx_video_sessions_mgr *mgr, int idx) {
...
/* call rx_video_session_put always if get successfully */
static inline bool
rx_video_session_get_empty(struct st_rx_video_sessions_mgr *mgr, int idx) {
  rte_spinlock_lock(&mgr->mutex[idx]);
  struct st_rx_video_session_impl *s = mgr->sessions[idx];
  if (s) {
...
static inline void rx_video_session_put(struct st_rx_video_sessions_mgr *mgr,
                                        int idx) {
  rte_spinlock_unlock(&mgr->mutex[idx]);
}
...
static inline bool
rx_video_session_is_cpu_busy(struct st_rx_video_session_impl *s) {
  if (s->dma_dev && (s->dma_busy_score > 90))
    return true;
  if (s->imiss_busy_score > 95.0)
    return true;
  if (s->cpu_busy_score > 95.0)
    return true;
...
static inline float
rx_video_session_get_cpu_busy(struct st_rx_video_session_impl *s) {
  return s->cpu_busy_score;
}

static inline bool
rx_video_session_can_migrate(struct st_rx_video_session_impl *s) {
