...
int st40p_tx_late_frame_spurn(void* handle, uint64_t epoch_skipped) {
  ...
  MT_USDT_ST40P_TX_FRAME_SPURN(ctx->idx, framebuff->idx,
...
static int tx_st40p_create_transport(struct mtl_main_impl* impl, struct st40p_tx_ctx* ctx,
  ...
  if (ops->flags & ST40P_TX_FLAG_DROP_WHEN_LATE) {
    ops_tx.notify_frame_late = st40p_tx_late_frame_spurn;
  }
...
struct st40_frame_info* st40p_tx_get_frame(st40p_tx_handle handle) {
  ...
  mt_pthread_mutex_lock(&ctx->lock);
  framebuff = tx_st40p_next_available(ctx, 0, ST40P_TX_FRAME_FREE);
