...
static int rx_fastmetadata_session_handle_pkt(struct mtl_main_impl* impl,
  ...
  if (ops->payload_type && (payload_type != ops->payload_type)) {
    ...
    s->st41_stat_pkts_wrong_pt_dropped++;
    return -EINVAL;
  }
  if (ops->ssrc) {
  ...
  MT_USDT_ST41_RX_MBUF_AVAILABLE(s->mgr->idx, s->idx, mbuf, tmstamp, pkt_len);
  return 0;
}
...
static int rx_fastmetadata_sessions_tasklet_handler(void* priv) {
  ...
  for (int sidx = 0; sidx < mgr->max_idx; sidx++) {
    s = rx_fastmetadata_session_try_get(mgr, sidx);
    if (!s) continue;
    if (time_measure) tsc_s = mt_get_tsc(impl);

    pending += rx_fastmetadata_session_tasklet(s);

    if (time_measure) {
      uint64_t delta_ns = mt_get_tsc(impl) - tsc_s;
      mt_stat_u64_update(&s->stat_time, delta_ns);
    }
    rx_fastmetadata_session_put(mgr, sidx);
  }
...
static int rx_fastmetadata_session_init_hw(struct mtl_main_impl* impl,
  ...
  for (int i = 0; i < num_port; i++) {
    ...
    if (s->ops.flags & ST41_RX_FLAG_DATA_PATH_ONLY) {
    ...
    info("%s(%d), port(l:%d,p:%d), queue %d udp %d\n", __func__, idx, i, port,
         rx_fastmetadata_queue_id(s, i), flow.dst_port);
  }
...
static int rx_fastmetadata_session_init_sw(struct st_rx_fastmetadata_sessions_mgr* mgr,
  ...
  snprintf(ring_name, 32, "%sM%dS%d_PKT", ST_RX_FASTMETADATA_PREFIX, mgr_idx, idx);
...
static int rx_fastmetadata_session_attach(struct mtl_main_impl* impl,
  ...
  s->latest_seq_id = -1;
  s->st41_stat_pkts_received = 0;
  s->st41_stat_pkts_dropped = 0;
  s->st41_stat_last_time = mt_get_monotonic_time();
  rte_atomic32_set(&s->st41_stat_frames_received, 0);
  ...
  ret = rx_fastmetadata_session_init_sw(mgr, s);
  if (ret < 0) {
    err("%s(%d), rx_fastmetadata_session_init_rtps fail %d\n", __func__, idx, ret);
    rx_fastmetadata_session_uinit(impl, s);
    return ret;
  }

  ret = rx_fastmetadata_session_init_mcast(impl, s);
  if (ret < 0) {
    err("%s(%d), rx_fastmetadata_session_init_mcast fail %d\n", __func__, idx, ret);
    rx_fastmetadata_session_uinit(impl, s);
    return -EIO;
  }
...
static void rx_fastmetadata_session_stat(struct st_rx_fastmetadata_session_impl* s) {
  ...
  int frames_received = rte_atomic32_read(&s->st41_stat_frames_received);
  double framerate = frames_received / time_sec;

  rte_atomic32_set(&s->st41_stat_frames_received, 0);

  notice("RX_FMD_SESSION(%d:%s): fps %f frames %d pkts %d\n", idx, s->ops_name, framerate,
         frames_received, s->st41_stat_pkts_received);
  s->st41_stat_pkts_received = 0;
  s->st41_stat_last_time = cur_time_ns;

  if (s->st41_stat_pkts_redundant) {
    notice("RX_FMD_SESSION(%d): redundant pkts %d\n", idx, s->st41_stat_pkts_redundant);
    s->st41_stat_pkts_redundant = 0;
  }
  if (s->st41_stat_pkts_dropped) {
    notice("RX_FMD_SESSION(%d): dropped pkts %d\n", idx, s->st41_stat_pkts_dropped);
    s->st41_stat_pkts_dropped = 0;
  }
  if (s->st41_stat_pkts_out_of_order) {
    warn("RX_FMD_SESSION(%d): out of order pkts %d\n", idx,
         s->st41_stat_pkts_out_of_order);
    s->st41_stat_pkts_out_of_order = 0;
  }
  if (s->st41_stat_pkts_wrong_pt_dropped) {
    notice("RX_FMD_SESSION(%d): wrong hdr payload_type dropped pkts %d\n", idx,
           s->st41_stat_pkts_wrong_pt_dropped);
    s->st41_stat_pkts_wrong_pt_dropped = 0;
  }
  if (s->st41_stat_pkts_wrong_ssrc_dropped) {
    notice("RX_FMD_SESSION(%d): wrong hdr ssrc dropped pkts %d\n", idx,
           s->st41_stat_pkts_wrong_ssrc_dropped);
    s->st41_stat_pkts_wrong_ssrc_dropped = 0;
  }
  ...
  if (s->st41_stat_pkts_enqueue_fail) {
    notice("RX_FMD_SESSION(%d): enqueue failed pkts %d\n", idx,
           s->st41_stat_pkts_enqueue_fail);
    s->st41_stat_pkts_enqueue_fail = 0;
  }
  if (s->ops.interlaced) {
  ...
  if (s->stat_max_notify_rtp_us > 8) {
    notice("RX_FMD_SESSION(%d): notify rtp max %uus\n", idx, s->stat_max_notify_rtp_us);
  }
...
static int rx_fastmetadata_sessions_mgr_update_src(
  ...
  ret = rx_fastmetadata_session_update_src(mgr->parent, s, src);
  rx_fastmetadata_session_put(mgr, idx);
  if (ret < 0) {
...
static int st_rx_fastmetadata_sessions_stat(void* priv) {
  ...
  for (int j = 0; j < mgr->max_idx; j++) {
    ...
    if (!s) continue;
    rx_fastmetadata_session_stat(s);
    rx_fastmetadata_session_put(mgr, j);
  }
...
static int rx_fastmetadata_sessions_mgr_init(
  ...
  ops.priv = mgr;
  ops.name = "rx_fmd_sessions_mgr";
  ops.start = rx_fastmetadata_sessions_tasklet_start;
  ops.stop = rx_fastmetadata_sessions_tasklet_stop;
  ops.handler = rx_fastmetadata_sessions_tasklet_handler;
  ...
  mt_stat_register(mgr->parent, st_rx_fastmetadata_sessions_stat, mgr, "rx_fmd");
  info("%s(%d), succ\n", __func__, idx);
  return 0;
}
...
static struct st_rx_fastmetadata_session_impl* rx_fastmetadata_sessions_mgr_attach(
  ...
  for (int i = 0; i < ST_MAX_RX_FMD_SESSIONS; i++) {
    if (!rx_fastmetadata_session_get_empty(mgr, i)) continue;

    s = mt_rte_zmalloc_socket(sizeof(*s), socket);
    if (!s) {
      err("%s(%d), session malloc fail on %d\n", __func__, midx, i);
      rx_fastmetadata_session_put(mgr, i);
      return NULL;
    }
    s->socket_id = socket;
    ret = rx_fastmetadata_session_init(mgr, s, i);
    if (ret < 0) {
    ...
    ret = rx_fastmetadata_session_attach(mgr->parent, mgr, s, ops);
    if (ret < 0) {
...
static int rx_fastmetadata_sessions_mgr_detach(
  ...
  rx_fastmetadata_session_detach(mgr->parent, s);
...
static int rx_fastmetadata_sessions_mgr_uinit(
  ...
  for (int i = 0; i < ST_MAX_RX_FMD_SESSIONS; i++) {
    s = rx_fastmetadata_session_get(mgr, i);
    if (!s) continue;

    warn("%s(%d), session %d still attached\n", __func__, m_idx, i);
    rx_fastmetadata_sessions_mgr_detach(mgr, s);
    rx_fastmetadata_session_put(mgr, i);
  }
...
static int st_rx_fmd_init(struct mtl_main_impl* impl, struct mtl_sch_impl* sch) {
  ...
  /* create rx fastmetadata context */
  ret = rx_fastmetadata_sessions_mgr_init(impl, sch, &sch->rx_fmd_mgr);
  if (ret < 0) {
    err("%s, rx_fastmetadata_sessions_mgr_init fail\n", __func__);
    return ret;
  }

  sch->rx_fmd_init = true;
  return 0;
}
...
st41_rx_handle st41_rx_create(mtl_handle mt, struct st41_rx_ops* ops) {
  ...
  ret = st_rx_fmd_init(impl, sch);
  mt_pthread_mutex_unlock(&sch->rx_fmd_mgr_mutex);
  if (ret < 0) {
    err("%s, st_rx_fmd_init fail %d\n", __func__, ret);
  ...
  mt_pthread_mutex_lock(&sch->rx_fmd_mgr_mutex);
  s = rx_fastmetadata_sessions_mgr_attach(sch, ops);
  mt_pthread_mutex_unlock(&sch->rx_fmd_mgr_mutex);
  if (!s) {
    err("%s, rx_fastmetadata_sessions_mgr_attach fail\n", __func__);
    mt_sch_put(sch, quota_mbs);
    mt_rte_free(s_impl);
    return NULL;
  }
...
int st41_rx_free(st41_rx_handle handle) {
  ...
  mt_pthread_mutex_lock(&sch->rx_fmd_mgr_mutex);
  ret = rx_fastmetadata_sessions_mgr_detach(&sch->rx_fmd_mgr, s);
  mt_pthread_mutex_unlock(&sch->rx_fmd_mgr_mutex);
