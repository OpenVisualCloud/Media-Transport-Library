...
/* total size: 62 skolelis tbd: fix the number after updating the structure data */
struct st_rfc8331_fmd_hdr {
  struct rte_ether_hdr eth;        /* size: 14 */
  struct rte_ipv4_hdr ipv4;        /* size: 20 */
  struct rte_udp_hdr udp;          /* size: 8 */
  struct st41_rfc8331_rtp_hdr rtp; /* size: 20 */
} __attribute__((__packed__)) __rte_aligned(2);
...
#define ST_PKT_FMD_HDR_LEN \
  (sizeof(struct st_rfc8331_fmd_hdr) - sizeof(struct rte_ether_hdr))
