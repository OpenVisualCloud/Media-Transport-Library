...
/* FRAME_LEVEL assembler — ports the body of pipeline rx_st40p_rtp_ready()
 * into the transport. Owns the frame slot pool; on marker, fills cur_meta
 * and calls ops->notify_frame_ready(priv, addr, &meta).
 *
 ...
 *
 * slot->state is a C11 _Atomic enum (memory_order_seq_cst): the tasklet
 * thread transitions FREE→RECEIVING→...→IN_USER while the app thread
 * concurrently sets IN_USER→FREE from st40_rx_put_framebuff().
 */
...
static struct st_rx_anc_frame_slot* rx_anc_get_free_slot(
  ...
  for (uint16_t i = 0; i < s->frame_slots_cnt; i++) {
    ...
    if (atomic_load_explicit(&slot->state, memory_order_seq_cst) == ST_RX_ANC_SLOT_FREE)
      return slot;
  }
  return NULL;
}
...
static void rx_anc_slot_init_frame(struct st_rx_anc_frame_slot* slot, uint32_t tmstamp,
  ...
  atomic_store_explicit(&slot->state, ST_RX_ANC_SLOT_RECEIVING, memory_order_seq_cst);
...
/* record one accepted packet's seq into the slot's per-frame trackers.
...
/* Parse one RTP packet's ANC chunks into the slot. Best-effort: partial
 * successes are kept on validation failure (matches pipeline behavior). */
static void rx_anc_slot_parse_pkt(struct st_rx_ancillary_session_impl* s,
  ...
  for (uint32_t anc_idx = 0; anc_idx < anc_count; anc_idx++) {
    ...
    uint32_t udw_offset = slot->udw_buffer_fill;
    uint32_t consumed = 0;
    enum st40_rfc8331_decode_result ret = st40_rfc8331_decode_packet(
        payload + payload_offset, payload_room - payload_offset, meta_entry,
        &slot->udw_buf[udw_offset], (uint32_t)(slot->udw_buf_size - udw_offset),
        &consumed);

    if (ret == ST40_RFC8331_DECODE_SHORT_BUFFER) {
      warn("%s(%d), anc payload exceeds available room anc=%u offset=%u room=%u\n",
           __func__, s->idx, anc_idx, payload_offset, payload_room);
      s->stat_anc_pkt_parse_err++;
      break;
    }
    if (ret == ST40_RFC8331_DECODE_PARITY_FAIL) {
      warn("%s(%d), UDW parity fail anc=%u\n", __func__, s->idx, anc_idx);
      s->stat_anc_pkt_parse_err++;
      break;
    }
    if (ret == ST40_RFC8331_DECODE_CHECKSUM_FAIL) {
      warn("%s(%d), checksum mismatch anc=%u\n", __func__, s->idx, anc_idx);
      s->stat_anc_pkt_parse_err++;
      break;
    }
...
static void rx_anc_slot_deliver(struct mtl_main_impl* impl,
  ...
  meta->timestamp_first_pkt = slot->receive_timestamp;
  /* derive seq_lost / seq_discont from this slot's own per-frame bitmap.
  ...
  /* stat_frames_received is bumped once per unique wire timestamp in
   * handle_pkt(); do not double-count it here. */

  /* Hand off to app. Slot ownership transfers until put_framebuff. */
  atomic_store_explicit(&slot->state, ST_RX_ANC_SLOT_IN_USER, memory_order_seq_cst);
...
/* Force-deliver a parked PENDING slot that will not receive its late marker
 * (evicted by a rollover, or stale during pool exhaustion). */
static void rx_anc_force_deliver_pending(struct mtl_main_impl* impl,
                                         struct st_rx_ancillary_session_impl* s) {
  struct st_rx_anc_frame_slot* prev_pending = s->anc_pending_slot;
  s->anc_pending_slot = NULL;
  atomic_store_explicit(&prev_pending->state, ST_RX_ANC_SLOT_READY, memory_order_seq_cst);
  rx_anc_slot_deliver(impl, s, prev_pending);
}

/* Pick or allocate the target slot for tmstamp, handling the
 * pending/inflight/eviction state machine. Returns NULL if the frame slot
 * pool is exhausted and the packet must be dropped. */
static struct st_rx_anc_frame_slot* rx_anc_select_slot(
    struct mtl_main_impl* impl, struct st_rx_ancillary_session_impl* s, uint32_t tmstamp,
    bool interlaced, bool second_field, uint64_t recv_ts) {
  struct st_rx_anc_frame_slot* slot = NULL;
  /* Snapshot to distinguish a pending slot that predates this call (safe to
   * force-deliver on pool exhaustion) from one created by a rollover below in
   * this same call (must still get its late-marker grace period). */
  struct st_rx_anc_frame_slot* pending_at_entry = s->anc_pending_slot;
  ...
  if (s->anc_pending_slot && s->anc_pending_slot->rtp_timestamp == tmstamp) {
    ...
    if (s->anc_inflight_slot->rtp_timestamp == tmstamp) {
      ...
      if (s->ops.num_port > 1) {
        ...
        if (s->anc_pending_slot) rx_anc_force_deliver_pending(impl, s);
        atomic_store_explicit(&old->state, ST_RX_ANC_SLOT_PENDING, memory_order_seq_cst);
        s->anc_pending_slot = old;
      } else {
        atomic_store_explicit(&old->state, ST_RX_ANC_SLOT_READY, memory_order_seq_cst);
  ...
  if (!slot) {
    ...
    if (!slot && s->anc_pending_slot && s->anc_pending_slot == pending_at_entry) {
      /* Pool exhausted with a stale pending parked and no inflight to evict
       * it (e.g. inflight cleared by a plain marker delivery) — force-deliver
       * it now instead of stalling until inflight is repopulated. Only a
       * pending slot that predates this call qualifies: one just rolled over
       * above still deserves its late-marker grace period. */
      rx_anc_force_deliver_pending(impl, s);
      slot = rx_anc_get_free_slot(s);
    }
    if (!slot) {
    ...
    rx_anc_slot_init_frame(slot, tmstamp, interlaced, second_field, recv_ts);
    ...
    if (slot->interlaced != interlaced || slot->second_field != second_field) {
      slot->interlaced = interlaced;
      slot->second_field = second_field;
    }
  }
...
static int rx_ancillary_session_assemble_pkt(struct mtl_main_impl* impl,
  ...
  if (marker) {
    ...
    if (slot == s->anc_pending_slot)
      ...
      s->anc_inflight_slot = NULL;
    atomic_store_explicit(&slot->state, ST_RX_ANC_SLOT_READY, memory_order_seq_cst);
...
accept_pkt:
  ...
  if (s->ops.type == ST40_TYPE_FRAME_LEVEL) {
    /* dispatch only — assembler stub does not take an mbuf refcnt; the
     * caller's burst-free reclaims it. RTP-style notify is skipped. */
    rx_ancillary_session_assemble_pkt(impl, s, mbuf, seq_id, tmstamp, s_port);
  } else {
...
static void rx_ancillary_session_stat(struct st_rx_ancillary_session_impl* s) {
  ...
  /* assembler observability for FRAME_LEVEL transport */
  ...
  if (s->stat_max_notify_us > 8) {
    notice("RX_ANC_SESSION(%d): notify max %uus\n", idx, s->stat_max_notify_us);
  }
  s->stat_max_notify_us = 0;
}
