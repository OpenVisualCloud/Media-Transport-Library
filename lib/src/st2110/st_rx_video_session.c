...
static int rv_attach(struct mtl_main_impl *impl, struct st_rx_video_sessions_mgr *mgr,
  ...
  info(
      "%s(%d), w %u h %u fmt %s packing %d pt %d flags 0x%x frame time %fms "
      "fps %f\n",
...
static void rv_stat(struct st_rx_video_sessions_mgr* mgr,
  ...
  if (s->stat_frames_dropped || s->stat_pkts_idx_dropped || s->stat_pkts_offset_dropped) {
    notice(
        "RX_VIDEO_SESSION(%d,%d): incomplete frames %d, pkts (idx error: "
        "%d, offset "
  ...
  if (stat_time->cnt) {
    ...
    notice(
        "RX_VIDEO_SESSION(%d,%d): tasklet time avg %.2fus max %.2fus min "
        "%.2fus\n",
