...
static uint8_t be10_to_ple_permute_tbl_512[16 * 4] = {
    1,      0,      6,      5,
    1 + 10, 0 + 10, 6 + 10, 5 + 10, /* b0, b1, b2, b3 */
    1 + 20, 0 + 20, 6 + 20, 5 + 20,
    1 + 30, 0 + 30, 6 + 30, 5 + 30, /* b4, b5, b6, b7 */
    3,      2,      8,      7,
    3 + 10, 2 + 10, 8 + 10, 7 + 10, /* r0, r1, r2, r3 */
    3 + 20, 2 + 20, 8 + 20, 7 + 20,
    3 + 30, 2 + 30, 8 + 30, 7 + 30, /* r4, r5, r6, r7 */
    2,      1,      4,      3,
    7,      6,      9,      8, /* y0 - y3 */
    2 + 10, 1 + 10, 4 + 10, 3 + 10,
    7 + 10, 6 + 10, 9 + 10, 8 + 10, /* y4 - y7 */
    2 + 20, 1 + 20, 4 + 20, 3 + 20,
    7 + 20, 6 + 20, 9 + 20, 8 + 20, /* y8 - y11 */
    2 + 30, 1 + 30, 4 + 30, 3 + 30,
    7 + 30, 6 + 30, 9 + 30, 8 + 30, /* y12 - y15 */
};
...
static uint16_t be10_to_ple_and_tbl_512[8 * 4] = {
    ...
    0x03ff, 0x03ff, 0x03ff, 0x03ff, /* b0 - b7 */
    0x03ff, 0x03ff, 0x03ff, 0x03ff,
    0x03ff, 0x03ff, 0x03ff, 0x03ff, /* r0 - r7 */
    0x03ff, 0x03ff, 0x03ff, 0x03ff,
    0x03ff, 0x03ff, 0x03ff, 0x03ff, /* y0 - y7 */
    0x03ff, 0x03ff, 0x03ff, 0x03ff,
    0x03ff, 0x03ff, 0x03ff, 0x03ff, /* y8 - y15 */
};

int st20_rfc4175_422be10_to_yuv422p10le_avx512_vbmi(
...
int st20_rfc4175_422be10_to_yuv422p10le_avx512_vbmi_dma(
  ...
  if (!be_caches || !ctx) {
    ...
    return st20_rfc4175_422be10_to_yuv422p10le_avx512_vbmi(pg_be, y, b, r, w,
                                                           h);
  }
  ...
  for (int i = 0; i < cache_batch; i++) {
    ...
    for (int j = 0; j < batch; j++) {
      __m512i input = _mm512_maskz_loadu_epi8(k, pg);
      __m512i permute_le_result =
          _mm512_permutexvar_epi8(permute_le_mask, input);
      __m512i srlv_le_result =
          _mm512_srlv_epi16(permute_le_result, srlv_le_mask);
      __m512i stage_m512i = _mm512_and_si512(srlv_le_result, srlv_and_mask);
...
static uint8_t be10_to_le_permute_l0_tbl_512[64] = {
    1,       0,
    3,       2, /* 4 bytes from pg0 */
    6,       5,
    8,       7, /* 4 bytes from pg1 */
    11,      10,
    13,      12, /* 4 bytes from pg2 */
    0,       5,
    10,      63, /* 5th bytes from pg0,pg1,pg2, and a padding */
    1 + 15,  0 + 15,
    3 + 15,  2 + 15, /* 4 bytes from pg3 */
    6 + 15,  5 + 15,
    8 + 15,  7 + 15, /* 4 bytes from pg4 */
    11 + 15, 10 + 15,
    13 + 15, 12 + 15, /* 4 bytes from pg5 */
    0 + 15,  5 + 15,
    10 + 15, 63, /* 5th bytes from pg3,pg4,pg5, and a padding */
    1 + 30,  0 + 30,
    3 + 30,  2 + 30, /* 4 bytes from pg6 */
    6 + 30,  5 + 30,
    8 + 30,  7 + 30, /* 4 bytes from pg7 */
    11 + 30, 10 + 30,
    13 + 30, 12 + 30, /* 4 bytes from pg8 */
    0 + 30,  5 + 30,
    10 + 30, 63, /* 5th bytes from pg6,pg7,pg8, and a padding */
    1 + 45,  0 + 45,
    3 + 45,  2 + 45, /* 4 bytes from pg9 */
    6 + 45,  5 + 45,
    8 + 45,  7 + 45, /* 4 bytes from pg10 */
    11 + 45, 10 + 45,
    13 + 45, 12 + 45, /* 4 bytes from pg11 */
    0 + 45,  5 + 45,
    10 + 45, 63, /* 5th bytes from pg9,pg10,pg11, and a padding */
};

static uint8_t be10_to_le_and_l0_tbl_512[64] = {
    0x00, 0xFF, 0xF0, 0x3F, 0x00, 0xFF, 0xF0, 0x3F, 0x00, 0xFF, 0xF0,
    0x3F, 0x00, 0x03, 0x03, 0x03, 0x00, 0xFF, 0xF0, 0x3F, 0x00, 0xFF,
    0xF0, 0x3F, 0x00, 0xFF, 0xF0, 0x3F, 0x00, 0x03, 0x03, 0x03, 0x00,
    0xFF, 0xF0, 0x3F, 0x00, 0xFF, 0xF0, 0x3F, 0x00, 0xFF, 0xF0, 0x3F,
    0x00, 0x03, 0x03, 0x03, 0x00, 0xFF, 0xF0, 0x3F, 0x00, 0xFF, 0xF0,
    0x3F, 0x00, 0xFF, 0xF0, 0x3F, 0x00, 0x03, 0x03, 0x03,
};

static uint8_t be10_to_le_permute_r0_tbl_512[64] = {
    2,       1,
    4,       3, /* 4 bytes from pg0 */
    7,       6,
    9,       8, /* 4 bytes from pg1 */
    12,      11,
    14,      13, /* 4 bytes from pg2 */
    63,      4,
    9,       14, /* 1st bytes from pg0,pg1,pg2, and a padding */
    2 + 15,  1 + 15,
    4 + 15,  3 + 15, /* 4 bytes from pg3 */
    7 + 15,  6 + 15,
    9 + 15,  8 + 15, /* 4 bytes from pg4 */
    12 + 15, 11 + 15,
    14 + 15, 13 + 15, /* 4 bytes from pg5 */
    63,      4 + 15,
    9 + 15,  14 + 15, /* 1st bytes from pg3,pg4,pg5, and a padding */
    2 + 30,  1 + 30,
    4 + 30,  3 + 30, /* 4 bytes from pg6 */
    7 + 30,  6 + 30,
    9 + 30,  8 + 30, /* 4 bytes from pg7 */
    12 + 30, 11 + 30,
    14 + 30, 13 + 30, /* 4 bytes from pg8 */
    63,      4 + 30,
    9 + 30,  14 + 30, /* 1st bytes from pg6,pg7,pg8, and a padding */
    2 + 45,  1 + 45,
    4 + 45,  3 + 45, /* 4 bytes from pg6 */
    7 + 45,  6 + 45,
    9 + 45,  8 + 45, /* 4 bytes from pg7 */
    12 + 45, 11 + 45,
    14 + 45, 13 + 45, /* 4 bytes from pg8 */
    63,      4 + 45,
    9 + 45,  14 + 45, /* 1st bytes from pg9,pg10,pg11, and a padding */
};

static uint8_t be10_to_le_and_r0_tbl_512[64] = {
    0xFC, 0x0F, 0xFF, 0x00, 0xFC, 0x0F, 0xFF, 0x00, 0xFC, 0x0F, 0xFF,
    0x00, 0xC0, 0xC0, 0xC0, 0x00, 0xFC, 0x0F, 0xFF, 0x00, 0xFC, 0x0F,
    0xFF, 0x00, 0xFC, 0x0F, 0xFF, 0x00, 0xC0, 0xC0, 0xC0, 0x00, 0xFC,
    0x0F, 0xFF, 0x00, 0xFC, 0x0F, 0xFF, 0x00, 0xFC, 0x0F, 0xFF, 0x00,
    0xC0, 0xC0, 0xC0, 0x00, 0xFC, 0x0F, 0xFF, 0x00, 0xFC, 0x0F, 0xFF,
    0x00, 0xFC, 0x0F, 0xFF, 0x00, 0xC0, 0xC0, 0xC0, 0x00,
};
...
int st20_rfc4175_422be10_to_422le10_avx512_vbmi(
    struct st20_rfc4175_422_10_pg2_be *pg_be,
    struct st20_rfc4175_422_10_pg2_le *pg_le, uint32_t w, uint32_t h) {
  __m512i permute_l0 =
      _mm512_loadu_si512((__m512i *)be10_to_le_permute_l0_tbl_512);
  __m512i permute_r0 =
      _mm512_loadu_si512((__m512i *)be10_to_le_permute_r0_tbl_512);
  ...
  __m512i permute_l1 =
      _mm512_loadu_si512((__m512i *)be10_to_le_permute_l1_tbl_512);
  __m512i permute_r1 =
      _mm512_loadu_si512((__m512i *)be10_to_le_permute_r1_tbl_512);
  ...
  for (int i = 0; i < cache_batch; i++) {
    ...
    for (int j = 0; j < batch; j++) {
      ...
      __m512i rl_result_shuffle =
          _mm512_permutexvar_epi8(permute_l1, rl_result);
      __m512i rr_result_shuffle =
          _mm512_permutexvar_epi8(permute_r1, rr_result);
      __m512i result = _mm512_or_si512(rl_result_shuffle, rr_result_shuffle);
...
int st20_rfc4175_422be10_to_422le8_avx512_vbmi(
  ...
  __m512i multishift_mask =
      _mm512_loadu_si512((__m512i *)be10_to_le8_multishift_tbl_512);
...
static uint8_t le10_to_v210_permute_tbl_512[16 * 4] = {
    0,      1,      2,       3,       4,       5,       6,       7,      7,
    8,      9,      10,      11,      12,      13,      14, /* pg0-2 */
    0 + 15, 1 + 15, 2 + 15,  3 + 15,  4 + 15,  5 + 15,  6 + 15,  7 + 15, 7 + 15,
    8 + 15, 9 + 15, 10 + 15, 11 + 15, 12 + 15, 13 + 15, 14 + 15, /* pg3-5 */
    0 + 30, 1 + 30, 2 + 30,  3 + 30,  4 + 30,  5 + 30,  6 + 30,  7 + 30, 7 + 30,
    8 + 30, 9 + 30, 10 + 30, 11 + 30, 12 + 30, 13 + 30, 14 + 30, /* pg6-8 */
    0 + 45, 1 + 45, 2 + 45,  3 + 45,  4 + 45,  5 + 45,  6 + 45,  7 + 45, 7 + 45,
    8 + 45, 9 + 45, 10 + 45, 11 + 45, 12 + 45, 13 + 45, 14 + 45, /* pg9-11 */
};
...
static uint8_t le10_to_v210_and_tbl_512[16 * 4] = {
    ...
    0xFF, 0x3F, 0xFF, 0xFF, 0xFF, 0x3F, 0xFF, 0xFF, 0xFF, 0x3F, 0xFF,
    0xFF, 0xFF, 0x3F, 0xFF, 0xFF, 0xFF, 0x3F, 0xFF, 0xFF, 0xFF, 0x3F,
    0xFF, 0xFF, 0xFF, 0x3F, 0xFF, 0xFF, 0xFF, 0x3F, 0xFF, 0xFF, 0xFF,
    0x3F, 0xFF, 0xFF, 0xFF, 0x3F, 0xFF, 0xFF, 0xFF, 0x3F,
};

int st20_rfc4175_422le10_to_v210_avx512_vbmi(uint8_t *pg_le, uint8_t *pg_v210,
                                             uint32_t w, uint32_t h) {
  __m512i permute_mask =
      _mm512_loadu_si512((__m512i *)le10_to_v210_permute_tbl_512);
  __m512i multishift_mask =
      _mm512_loadu_si512((__m512i *)le10_to_v210_multishift_tbl_512);
  __m512i padding_mask =
      _mm512_loadu_si512((__m512i *)le10_to_v210_and_tbl_512);
  __mmask16 k = 0x7FFF; /* each __m512i with 12 pg group, 60 bytes */
...
static uint8_t be10_to_v210_permute0_tbl_512[16 * 4] = {
    ...
    7,       6,       8,       7,       11,      10,
    12,      11,      14,      13, /* pg 0-2 */
    1 + 15,  0 + 15,  3 + 15,  2 + 15,  4 + 15,  3 + 15,
    7 + 15,  6 + 15,  8 + 15,  7 + 15,  11 + 15, 10 + 15,
    12 + 15, 11 + 15, 14 + 15, 13 + 15, /* pg 3-5 */
    1 + 30,  0 + 30,  3 + 30,  2 + 30,  4 + 30,  3 + 30,
    7 + 30,  6 + 30,  8 + 30,  7 + 30,  11 + 30, 10 + 30,
    12 + 30, 11 + 30, 14 + 30, 13 + 30, /* pg 6-8 */
    1 + 45,  0 + 45,  3 + 45,  2 + 45,  4 + 45,  3 + 45,
    7 + 45,  6 + 45,  8 + 45,  7 + 45,  11 + 45, 10 + 45,
    12 + 45, 11 + 45, 14 + 45, 13 + 45, /* pg 9-11 */
};
...
static uint8_t be10_to_v210_and0_tbl_512[16 * 4] = {
    0xFF, 0x03, 0xF0, 0x3F, 0xFF, 0x03, 0xF0, 0x3F, 0xFF, 0x03, 0xF0,
    0x3F, 0xFF, 0x03, 0xF0, 0x3F, 0xFF, 0x03, 0xF0, 0x3F, 0xFF, 0x03,
    0xF0, 0x3F, 0xFF, 0x03, 0xF0, 0x3F, 0xFF, 0x03, 0xF0, 0x3F, 0xFF,
    0x03, 0xF0, 0x3F, 0xFF, 0x03, 0xF0, 0x3F, 0xFF, 0x03, 0xF0, 0x3F,
    0xFF, 0x03, 0xF0, 0x3F, 0xFF, 0x03, 0xF0, 0x3F, 0xFF, 0x03, 0xF0,
    0x3F, 0xFF, 0x03, 0xF0, 0x3F, 0xFF, 0x03, 0xF0, 0x3F,
};
...
static uint8_t be10_to_v210_and1_tbl_512[16 * 4] = {
    0x00, 0xFC, 0x0F, 0x00, 0x00, 0xFC, 0x0F, 0x00, 0x00, 0xFC, 0x0F,
    0x00, 0x00, 0xFC, 0x0F, 0x00, 0x00, 0xFC, 0x0F, 0x00, 0x00, 0xFC,
    0x0F, 0x00, 0x00, 0xFC, 0x0F, 0x00, 0x00, 0xFC, 0x0F, 0x00, 0x00,
    0xFC, 0x0F, 0x00, 0x00, 0xFC, 0x0F, 0x00, 0x00, 0xFC, 0x0F, 0x00,
    0x00, 0xFC, 0x0F, 0x00, 0x00, 0xFC, 0x0F, 0x00, 0x00, 0xFC, 0x0F,
    0x00, 0x00, 0xFC, 0x0F, 0x00, 0x00, 0xFC, 0x0F, 0x00,
};

int st20_rfc4175_422be10_to_v210_avx512_vbmi(
    struct st20_rfc4175_422_10_pg2_be *pg_be, uint8_t *pg_v210, uint32_t w,
    uint32_t h) {
  __m512i permute0_mask =
      _mm512_loadu_si512((__m512i *)be10_to_v210_permute0_tbl_512);
  ...
  __m512i permute1_mask =
      _mm512_loadu_si512((__m512i *)be10_to_v210_permute1_tbl_512);
...
int st20_rfc4175_422be10_to_v210_avx512_vbmi_dma(
    struct mtl_dma_lender_dev *dma, struct st20_rfc4175_422_10_pg2_be *pg_be,
...
static uint8_t ple_to_be10_permute_hi_tbl_512[16 * 4] =
    {
        1 + (2 * 0),  0 + (2 * 0), 16 + (4 * 0), 32 + (2 * 0),
        18 + (4 * 0), /* pg0 */
        1 + (2 * 1),  0 + (2 * 1), 16 + (4 * 1), 32 + (2 * 1),
        18 + (4 * 1), /* pg1 */
        1 + (2 * 2),  0 + (2 * 2), 16 + (4 * 2), 32 + (2 * 2),
        18 + (4 * 2), /* pg2 */
        1 + (2 * 3),  0 + (2 * 3), 16 + (4 * 3), 32 + (2 * 3),
        18 + (4 * 3), /* pg3 */
        1 + (2 * 4),  0 + (2 * 4), 48 + (4 * 0), 32 + (2 * 4),
        50 + (4 * 0), /* pg4 */
        1 + (2 * 5),  0 + (2 * 5), 48 + (4 * 1), 32 + (2 * 5),
        50 + (4 * 1), /* pg5 */
        1 + (2 * 6),  0 + (2 * 6), 48 + (4 * 2), 32 + (2 * 6),
        50 + (4 * 2), /* pg6 */
        1 + (2 * 7),  0 + (2 * 7), 48 + (4 * 3), 32 + (2 * 7),
        50 + (4 * 3), /* pg7 */
        0x00,         0x00,        0x00,         0x00,
        0x00,         0x00,        0x00,         0x00, /* 40-48 */
        0x00,         0x00,        0x00,         0x00,
        0x00,         0x00,        0x00,         0x00, /* 49-53 */
        0x00,         0x00,        0x00,         0x00,
        0x00,         0x00,        0x00,         0x00, /* 54-63 */
};
...
int st20_yuv422p10le_to_rfc4175_422be10_vbmi(
  ...
  while (pg_cnt >= 8) {
    ...
    __m512i permute_hi_result =
        _mm512_permutexvar_epi8(permute_hi_mask, srlv_le_result);
    __m512i permute_lo_result =
        _mm512_permutexvar_epi8(permute_lo_mask, srlv_le_result);
...
int st20_rfc4175_422le10_to_422be10_vbmi(
    struct st20_rfc4175_422_10_pg2_le *pg_le,
    struct st20_rfc4175_422_10_pg2_be *pg_be, uint32_t w, uint32_t h) {
  __m512i permute_l0 =
      _mm512_loadu_si512((__m512i *)le10_to_be_permute_l0_tbl_512);
  __m512i permute_r0 =
      _mm512_loadu_si512((__m512i *)le10_to_be_permute_r0_tbl_512);
  ...
  __m512i permute_l1 =
      _mm512_loadu_si512((__m512i *)le10_to_be_permute_l1_tbl_512);
  __m512i permute_r1 =
      _mm512_loadu_si512((__m512i *)le10_to_be_permute_r1_tbl_512);
  __mmask16 k = 0x7FFF; /* each __m512i with 12 pg group, 60 bytes */
...
int st20_v210_to_rfc4175_422be10_avx512_vbmi(
  ...
  __m512i shuffle_mask =
      _mm512_loadu_si512((__m512i *)v210_to_be10_shuffle_tbl_512);
  __m512i multishift1_mask =
  ...
  for (int i = 0; i < batch; i++) {
    ...
    __m512i multishift0_result =
        _mm512_multishift_epi64_epi8(multishift0_mask, input);
...
int st20_v210_to_rfc4175_422be10_avx512_vbmi_dma(
  ...
  dbg("%s, pg_cnt %d cache_3_pg_cnt %d caches_num %d cache_batch %d\n",
      __func__, pg_cnt, cache_3_pg_cnt, caches_num, cache_batch);
  for (int i = 0; i < cache_batch; i++) {
    uint8_t *v210_cache =
        v210_caches + (i % caches_num) * (cache_3_pg_cnt * 16);
    dbg("%s, cache batch idx %d\n", __func__, i);
...
int st20_downsample_rfc4175_422be10_wh_half_avx512_vbmi(uint8_t *pg_old,
                                                        uint8_t *pg_new,
                                                        uint32_t w, uint32_t h,
...
static uint8_t be12_to_ple_permute_tbl_512[16 * 4] = {
    1,      0,      1 + 6,  0 + 6,
    1 + 12, 0 + 12, 1 + 18, 0 + 18, /* b0, b1, b2, b3 */
    1 + 24, 0 + 24, 1 + 30, 0 + 30,
    1 + 36, 0 + 36, 1 + 42, 0 + 42, /* b4, b5, b6, b7 */
    4,      3,      4 + 6,  3 + 6,
    4 + 12, 3 + 12, 4 + 18, 3 + 18, /* r0, r1, r2, r3 */
    4 + 24, 3 + 24, 4 + 30, 3 + 30,
    4 + 36, 3 + 36, 4 + 42, 3 + 42, /* r4, r5, r6, r7 */
    2,      1,      5,      4,
    2 + 6,  1 + 6,  5 + 6,  4 + 6, /* y0 - y3 */
    2 + 12, 1 + 12, 5 + 12, 4 + 12,
    2 + 18, 1 + 18, 5 + 18, 4 + 18, /* y4 - y7 */
    2 + 24, 1 + 24, 5 + 24, 4 + 24,
    2 + 30, 1 + 30, 5 + 30, 4 + 30, /* y8 - y11 */
    2 + 36, 1 + 36, 5 + 36, 4 + 36,
    2 + 42, 1 + 42, 5 + 42, 4 + 42, /* y12 - y15 */
};
...
static uint16_t be12_to_ple_and_tbl_512[8 * 4] = {
    ...
    0x0fff, 0x0fff, 0x0fff, 0x0fff, /* b0 - b7 */
    0x0fff, 0x0fff, 0x0fff, 0x0fff,
    0x0fff, 0x0fff, 0x0fff, 0x0fff, /* r0 - r7 */
    0x0fff, 0x0fff, 0x0fff, 0x0fff,
    0x0fff, 0x0fff, 0x0fff, 0x0fff, /* y0 - y7 */
    0x0fff, 0x0fff, 0x0fff, 0x0fff,
    0x0fff, 0x0fff, 0x0fff, 0x0fff, /* y8 - y15 */
};

int st20_rfc4175_422be12_to_yuv422p12le_avx512_vbmi(
...
int st20_rfc4175_422be12_to_yuv422p12le_avx512_vbmi_dma(
  ...
  if (!be_caches || !ctx) {
    ...
    if (ctx)
      mt_cvt_dma_ctx_uinit(ctx);
    return st20_rfc4175_422be12_to_yuv422p12le_avx512_vbmi(pg_be, y, b, r, w,
                                                           h);
  }
  rte_iova_t be_caches_iova = rte_malloc_virt2iova(be_caches);
