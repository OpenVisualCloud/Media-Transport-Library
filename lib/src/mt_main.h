...
struct mt_ptp_impl {
  ...
  /* async read state for the delay_req tx timestamp (t3) */
  bool t3_pending;
  uint16_t t3_pending_seq;
  uint64_t t3_deadline_ns;
  /* single-owner claim so t3 and t4 completing on two threads apply the
   * cycle's result exactly once; reset per cycle in ptp_parse_sync */
  rte_atomic32_t result_claimed;
...
struct mtl_main_impl {
  ...
  /* Client of the MTL Manager, a mtlm_client*, or NULL in single instance mode.
   * It is void* so this header needs no manager header. */
  void* instance_client;
};
...
static inline bool mt_is_manager_connected(struct mtl_main_impl* impl) {
  return impl->instance_client != NULL;
}
