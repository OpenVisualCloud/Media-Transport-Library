...
#ifndef _ST_LIB_QUIRK_HEAD_H_
#define _ST_LIB_QUIRK_HEAD_H_

static inline unsigned int st_rte_ring_sc_dequeue_bulk(struct rte_ring* r,
                                                       void** obj_table, unsigned int n,
                                                       unsigned int* available) {
#if defined(__clang__) /* fix for clang release build */
  /* not sure why clang has issue with variable n, porbally optimised code sequence */
  if (n == 4)
