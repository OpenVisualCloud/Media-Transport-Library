...
static uint16_t rx_socket_burst(struct mt_rxq_entry *entry,
                                struct rte_mbuf **rx_pkts,
                                const uint16_t nb_pkts) {
  return mt_rx_socket_burst(entry->rx_socket_q, rx_pkts, nb_pkts);
}

static uint16_t rx_xdp_burst(struct mt_rxq_entry *entry,
                             struct rte_mbuf **rx_pkts,
                             const uint16_t nb_pkts) {
  return mt_rx_xdp_burst(entry->rx_xdp_q, rx_pkts, nb_pkts);
}

static uint16_t rx_rdma_burst(struct mt_rxq_entry *entry,
                              struct rte_mbuf **rx_pkts,
                              const uint16_t nb_pkts) {
  return mt_rx_rdma_burst(entry->rx_rdma_q, rx_pkts, nb_pkts);
}

static uint16_t rx_srss_burst(struct mt_rxq_entry *entry,
                              struct rte_mbuf **rx_pkts,
                              const uint16_t nb_pkts) {
  return mt_srss_burst(entry->srss, rx_pkts, nb_pkts);
}

static uint16_t rx_rsq_burst(struct mt_rxq_entry *entry,
                             struct rte_mbuf **rx_pkts,
                             const uint16_t nb_pkts) {
  return mt_rsq_burst(entry->rsq, rx_pkts, nb_pkts);
}

static uint16_t rx_csq_burst(struct mt_rxq_entry *entry,
                             struct rte_mbuf **rx_pkts,
                             const uint16_t nb_pkts) {
  return mt_csq_burst(entry->csq, rx_pkts, nb_pkts);
}

static uint16_t rx_dpdk_burst(struct mt_rxq_entry *entry,
                              struct rte_mbuf **rx_pkts,
                              const uint16_t nb_pkts) {
...
static uint16_t tx_socket_burst(struct mt_txq_entry *entry,
                                struct rte_mbuf **tx_pkts, uint16_t nb_pkts) {
  return mt_tx_socket_burst(entry->tx_socket_q, tx_pkts, nb_pkts);
}

static uint16_t tx_xdp_burst(struct mt_txq_entry *entry,
                             struct rte_mbuf **tx_pkts, uint16_t nb_pkts) {
  return mt_tx_xdp_burst(entry->tx_xdp_q, tx_pkts, nb_pkts);
}

static uint16_t tx_rdma_burst(struct mt_txq_entry *entry,
                              struct rte_mbuf **tx_pkts, uint16_t nb_pkts) {
  return mt_tx_rdma_burst(entry->tx_rdma_q, tx_pkts, nb_pkts);
}

static uint16_t tx_tsq_burst(struct mt_txq_entry *entry,
                             struct rte_mbuf **tx_pkts, uint16_t nb_pkts) {
  return mt_tsq_burst(entry->tsq, tx_pkts, nb_pkts);
}

static uint16_t tx_dpdk_burst(struct mt_txq_entry *entry,
                              struct rte_mbuf **tx_pkts, uint16_t nb_pkts) {
...
int mt_txq_fatal_error(struct mt_txq_entry *entry) {
  if (entry->txq)
    mt_dev_tx_queue_fatal_error(entry->parent, entry->txq);
  if (entry->tsq)
    mt_tsq_fatal_error(entry->tsq);
  return 0;
}

int mt_txq_done_cleanup(struct mt_txq_entry *entry) {
  if (entry->txq)
    mt_dev_tx_done_cleanup(entry->parent, entry->txq);
  if (entry->tsq)
    mt_tsq_done_cleanup(entry->tsq);
  return 0;
}
