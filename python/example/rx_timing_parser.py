...
class Dashboard:
    def __init__(self, max_histories):
        ...
        self.text_narrow = self.ax.text(
            0.95, 1.0, f"Narrow: {self.narrow_count}", transform=self.ax.transAxes
        )
        self.text_wide = self.ax.text(
            0.95, 0.95, f"Wide: {self.wide_count}", transform=self.ax.transAxes
        )
        self.text_fail = self.ax.text(
            0.95, 0.90, f"Failed: {self.fail_count}", transform=self.ax.transAxes
        )
        # init cleat button
...
def main():
    ...
    # initial plot
    plot = Dashboard(max_histories=200)
