...
static int rdma_rx_handle_wc_recv_imm(struct mt_rdma_rx_ctx *ctx,
                                      struct ibv_wc *wc) {
  struct mtl_rdma_rx_ops *ops = &ctx->ops;
  ...
  if (ops->notify_buffer_ready)
    ops->notify_buffer_ready(ops->priv, &rx_buffer->buffer);
  pthread_mutex_unlock(&rx_buffer->lock);
  ...
  while (!ctx->connect_stop) {
    ...
    if (!ret) {
      ...
      case RDMA_CM_EVENT_ESTABLISHED:
        ...
        ret = pthread_create(&ctx->cq_poll_thread, NULL, rdma_rx_cq_poll_thread,
                             ctx);
        if (ret) {
